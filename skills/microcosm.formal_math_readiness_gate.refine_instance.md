# Microcosm Formal Math Readiness Gate Refine Instance

Use this public-safe skill when `std_microcosm_formal_math_readiness_gate` declares the `refine_instance` triad route for `formal_math_readiness_gate`. It is a governed route source for the declared skill id. It does not upgrade the standard from `public_microcosm_standard_v1` / `accepted`, prove runtime correctness, authorize release, publication, provider calls, private-body export, source mutation, or whole-system correctness.
...
```json
{
  ...
  "trigger_summary": [
    "The Formal math readiness gate standard declares this refine instance skill id with status planned.",
  ...
  "mapping_basis": "The standard JSON at standards/std_microcosm_formal_math_readiness_gate.json declares this refine_instance skill route for formal_math_readiness_gate, and core/organ_atlas.json::formal_math_readiness_gate names mechanism_refs=mechanism.formal_math_readiness_gate.validates_public_readiness_boundary and concept_refs=concept.formal_math_and_proof_witness_bundle. This source mapping binds the skill to those accepted organ neighbours without inferring from prose, generated projections, endpoint coverage, or release claims."
}
```
