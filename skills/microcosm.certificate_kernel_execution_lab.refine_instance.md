# Microcosm Certificate Kernel Execution Lab Refine Instance

Use this public-safe skill when `std_microcosm_certificate_kernel_execution_lab` declares the `refine_instance` triad route for `certificate_kernel_execution_lab`. It is a governed route source for the declared skill id. It does not upgrade the standard from `public_microcosm_standard_v1` / `draft`, prove runtime correctness, authorize release, publication, provider calls, private-body export, source mutation, or whole-system correctness.
...
```json
{
  ...
  "operates_standard": "std_microcosm_certificate_kernel_execution_lab",
  "acts_on_kind": "certificate_kernel_execution_lab",
  "trigger_summary": [
    "The Certificate kernel execution lab standard declares this refine instance skill id with status planned.",
  ...
  "workflow_summary": [
    "Read std_microcosm_certificate_kernel_execution_lab JSON, its authority ceiling, validator refs, receipt refs, and lattice health before editing certificate_kernel_execution_lab artifacts.",
  ...
  "mapping_basis": "The standard JSON at standards/std_microcosm_certificate_kernel_execution_lab.json declares this refine_instance skill route for certificate_kernel_execution_lab, and core/organ_atlas.json::certificate_kernel_execution_lab names mechanism_refs=mechanism.certificate_kernel_execution_lab.validates_public_certificate_kernel_execution and concept_refs=concept.formal_math_and_proof_witness_bundle. This source mapping binds the skill to those accepted organ neighbours without inferring from prose, generated projections, endpoint coverage, or release claims."
}
```
