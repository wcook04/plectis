# Microcosm Spatial World Model Counterfactual Simulation Replay Refine Standard And Propagate

Use this public-safe skill when `std_microcosm_spatial_world_model_counterfactual_simulation_replay` declares the `refine_standard_and_propagate` triad route for `spatial_world_model_counterfactual_simulation_replay`. It is a governed route source for the declared skill id. It does not upgrade the standard from `public_microcosm_standard_v1` / `active_source_body_import`, prove runtime correctness, authorize release, publication, provider calls, private-body export, source mutation, or whole-system correctness.
...
```json
{
  ...
  "operates_standard": "std_microcosm_spatial_world_model_counterfactual_simulation_replay",
  "acts_on_kind": "spatial_world_model_counterfactual_simulation_replay",
  "trigger_summary": [
    "The Spatial world-model counterfactual simulation replay standard declares this refine standard and propagate skill id with status planned.",
  ...
  "workflow_summary": [
    "Read std_microcosm_spatial_world_model_counterfactual_simulation_replay JSON, its authority ceiling, validator refs, receipt refs, and lattice health before editing spatial_world_model_counterfactual_simulation_replay artifacts.",
  ...
  "mapping_basis": "The standard JSON at standards/std_microcosm_spatial_world_model_counterfactual_simulation_replay.json declares this refine_standard_and_propagate skill route for spatial_world_model_counterfactual_simulation_replay, and core/organ_atlas.json::spatial_world_model_counterfactual_simulation_replay names mechanism_refs=mechanism.spatial_world_model_counterfactual_simulation_replay.validates_public_spatial_world_model_counterfactual_simulation_replay and concept_refs=concept.research_and_science_replay_evidence_bundle. This source mapping binds the skill to those accepted organ neighbours without inferring from prose, generated projections, endpoint coverage, or release claims."
}
```
