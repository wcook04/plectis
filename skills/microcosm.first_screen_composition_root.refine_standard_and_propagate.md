# Microcosm First Screen Composition Root Refine Standard And Propagate

Use this public-safe skill when `std_microcosm_first_screen_composition_root` declares the `refine_standard_and_propagate` triad route for `first_screen_composition_root`. It is a governed route source for the declared skill id. It does not upgrade the standard from `public_microcosm_standard_v1` / `active`, prove runtime correctness, authorize release, publication, provider calls, private-body export, source mutation, or whole-system correctness.
...
```json
{
  ...
  "operates_standard": "std_microcosm_first_screen_composition_root",
  "acts_on_kind": "first_screen_composition_root",
  "trigger_summary": [
    "The First-screen composition root standard declares this refine standard and propagate skill id with status planned.",
  ...
  "workflow_summary": [
    "Read std_microcosm_first_screen_composition_root JSON, its authority ceiling, validator refs, receipt refs, and lattice health before editing first_screen_composition_root artifacts.",
  ...
  "mapping_basis": "The standard JSON at standards/std_microcosm_first_screen_composition_root.json declares this skill route; relationships.used_by_organs=[\"cold_reader_route_map\", \"public_reveal_walkthrough\"] names organ atlas rows whose mechanism_refs=[\"mechanism.cold_reader_route_map.validates_public_first_run_route_map\", \"mechanism.public_reveal_walkthrough.validates_public_reveal_walkthrough\"] and concept_refs=[\"concept.entry_and_reveal_route_readiness_bundle\"] are the source basis for these selective skill edges. This binds only source-declared lattice neighbours and does not prove runtime correctness, release readiness, provider authority, private-body coverage, or whole-system completeness."
}
```
