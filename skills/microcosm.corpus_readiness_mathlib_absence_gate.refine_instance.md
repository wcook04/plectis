# Microcosm Corpus Readiness Mathlib Absence Gate Refine Instance

Use this public-safe skill when `std_microcosm_corpus_readiness_mathlib_absence_gate` declares the `refine_instance` triad route for `corpus_readiness_mathlib_absence_gate`. It is a governed route source for the declared skill id. It does not upgrade the standard from `public_microcosm_standard_v1` / `draft`, prove runtime correctness, authorize release, publication, provider calls, private-body export, source mutation, or whole-system correctness.
...
```json
{
  ...
  "operates_standard": "std_microcosm_corpus_readiness_mathlib_absence_gate",
  "acts_on_kind": "corpus_readiness_mathlib_absence_gate",
  "trigger_summary": [
    "The Corpus readiness Mathlib absence gate standard declares this refine instance skill id with status planned.",
  ...
  "workflow_summary": [
    "Read std_microcosm_corpus_readiness_mathlib_absence_gate JSON, its authority ceiling, validator refs, receipt refs, and lattice health before editing corpus_readiness_mathlib_absence_gate artifacts.",
  ...
  "mapping_basis": "The standard JSON at standards/std_microcosm_corpus_readiness_mathlib_absence_gate.json declares this refine_instance skill route for corpus_readiness_mathlib_absence_gate, and core/organ_atlas.json::corpus_readiness_mathlib_absence_gate names mechanism_refs=mechanism.corpus_readiness_mathlib_absence_gate.validates_public_mathlib_absence_boundary and concept_refs=concept.formal_math_and_proof_witness_bundle. This source mapping binds the skill to those accepted organ neighbours without inferring from prose, generated projections, endpoint coverage, or release claims."
}
```
