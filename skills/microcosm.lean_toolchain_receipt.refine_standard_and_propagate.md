# Microcosm Lean Toolchain Receipt Refine Standard And Propagate

Use this public-safe skill when `std_microcosm_lean_toolchain_receipt` declares the `refine_standard_and_propagate` triad route for `lean_toolchain_receipt`. It is a governed route source for the declared skill id. It does not upgrade the standard from `public_microcosm_standard_v1` / `draft`, prove runtime correctness, authorize release, publication, provider calls, private-body export, source mutation, or whole-system correctness.
...
```json
{
  ...
  "operates_standard": "std_microcosm_lean_toolchain_receipt",
  "acts_on_kind": "lean_toolchain_receipt",
  "trigger_summary": [
    "The Lean toolchain receipt standard declares this refine standard and propagate skill id with status planned.",
  ...
  "workflow_summary": [
    "Read std_microcosm_lean_toolchain_receipt JSON, its authority ceiling, validator refs, receipt refs, and lattice health before editing lean_toolchain_receipt artifacts.",
  ...
  "mapping_basis": "The standard JSON at standards/std_microcosm_lean_toolchain_receipt.json declares this skill route; relationships.used_by_organs=[\"formal_math_lean_proof_witness\", \"proof_diagnostic_evidence_spine\"] names organ atlas rows whose mechanism_refs=[\"mechanism.formal_math_lean_proof_witness.validates_public_lean_witness\", \"mechanism.proof_diagnostic_evidence_spine.validates_ring2_diagnostic_evidence_membrane\"] and concept_refs=[\"concept.formal_math_and_proof_witness_bundle\"] are the source basis for these selective skill edges. This binds only source-declared lattice neighbours and does not prove runtime correctness, release readiness, provider authority, private-body coverage, or whole-system completeness."
}
```
