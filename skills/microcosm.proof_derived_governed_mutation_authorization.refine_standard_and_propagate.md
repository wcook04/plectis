# Microcosm Proof Derived Governed Mutation Authorization Refine Standard And Propagate

Use this public-safe skill when `std_microcosm_proof_derived_governed_mutation_authorization` declares the `refine_standard_and_propagate` triad route for `proof_derived_governed_mutation_authorization`. It is a governed route source for the declared skill id. It does not upgrade the standard from `public_microcosm_standard_v1` / `draft`, prove runtime correctness, authorize release, publication, provider calls, private-body export, source mutation, or whole-system correctness.
...
```json
{
  ...
  "operates_standard": "std_microcosm_proof_derived_governed_mutation_authorization",
  "acts_on_kind": "proof_derived_governed_mutation_authorization",
  "trigger_summary": [
    "The Proof-derived governed mutation authorization standard declares this refine standard and propagate skill id with status planned.",
  ...
  "workflow_summary": [
    "Read std_microcosm_proof_derived_governed_mutation_authorization JSON, its authority ceiling, validator refs, receipt refs, and lattice health before editing proof_derived_governed_mutation_authorization artifacts.",
  ...
  "mapping_basis": "The standard JSON at standards/std_microcosm_proof_derived_governed_mutation_authorization.json declares this refine_standard_and_propagate skill route for proof_derived_governed_mutation_authorization, and core/organ_atlas.json::proof_derived_governed_mutation_authorization names mechanism_refs=mechanism.proof_derived_governed_mutation_authorization.validates_synthetic_governed_mutation_authorization and concept_refs=concept.formal_math_and_proof_witness_bundle. This source mapping binds the skill to those accepted organ neighbours without inferring from prose, generated projections, endpoint coverage, or release claims."
}
```
