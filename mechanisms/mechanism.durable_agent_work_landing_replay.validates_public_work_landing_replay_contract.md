# mechanism.durable_agent_work_landing_replay.validates_public_work_landing_replay_contract validates public work landing replay contract
...
The durable agent work-landing replay organ validates public work-landing replay rows by checking owned-path claim evidence, owner-native validation-before-commit ordering, HEAD-advance evidence for landed language, metadata-blocked closeout capture, Work Ledger finalizer refs, copied macro source-module digests, and body-free receipt boundaries before writing bounded replay receipts.

## Lattice Neighbours

- `grounded_in` -> `code_locus:src/microcosm_core/organs/durable_agent_work_landing_replay.py` (resolved_code_locus)
- `runs_in` -> `organ:durable_agent_work_landing_replay` (resolved_registry_or_atlas_target)
