# mechanism.engine_room_navigation_fitness_benchmark.validates_public_navigation_fitness_benchmark validates public navigation fitness benchmark
...
The Engine Room navigation fitness benchmark mechanism validates public route-packet benchmark fixtures by checking expected stable-id recall, packet precision, forbidden first routes, latency budgets, scent terms, and debt-candidate summaries without running the private kernel, validating embeddings, claiming universal navigation benchmark authority, release, or private-root authority.

## Lattice Neighbours

- `grounded_in` -> `code_locus:src/microcosm_core/engine_room/navigation_fitness_benchmark.py` (resolved_code_locus)
