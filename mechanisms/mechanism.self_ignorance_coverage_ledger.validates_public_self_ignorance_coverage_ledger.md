# mechanism.self_ignorance_coverage_ledger.validates_public_self_ignorance_coverage_ledger validates public self ignorance coverage ledger
...
The self-ignorance coverage ledger organ validates known Kind Atlas coverage-debt fixtures through declared gap rows, missing-category checks, source-module digest anchors, negative cases, body-free receipts, and authority ceilings while refusing unknown-unknown omniscience, absence proof, total-search proof, source mutation, release, publication, or whole-system correctness claims.

## Lattice Neighbours

- `grounded_in` -> `code_locus:src/microcosm_core/organs/self_ignorance_coverage_ledger.py` (resolved_code_locus)
- `runs_in` -> `organ:self_ignorance_coverage_ledger` (resolved_registry_or_atlas_target)
