# mechanism.engine_room_derived_fact_provider_engine.validates_public_derived_fact_provider_engine validates public derived fact provider engine
...
The Engine Room derived fact provider engine mechanism validates registry-backed fact resolution over public fixture roots by resolving JSON pointers, glob counts, and git-backed callable facts, then turning provider failures into repairable error rows without claiming doctrine truth auditing, full macro fact registry export, semantic claim validation, release, or private-root authority.

## Lattice Neighbours

- `grounded_in` -> `code_locus:src/microcosm_core/engine_room/derived_fact_provider_engine.py` (resolved_code_locus)
