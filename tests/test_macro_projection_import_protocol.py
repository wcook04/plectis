...
def test_macro_projection_import_protocol_observes_negative_cases(tmp_path: Path) -> None:
    ...
    assert result["projection_cell_count"] == 18
    assert result["ready_projection_cell_count"] == 18
    ...
    assert result["public_safe_body_material_count"] == 50
    ...
    assert result["projection_intake_board"]["ready_cell_count"] == 18
    ...
    assert result["projection_intake_board"]["landed_cell_count"] == 18
    assert result["projection_intake_board"]["projection_status_counts"] == {
        "public_runtime_import_landed": 16,
    ...
    assert result["projection_intake_board"]["public_safe_body_import_count"] == 50
    assert result["projection_intake_board"]["public_safe_body_import_routes"] == {
        "verified_light_edit": 50
    }
    by_material = {
        row["material_id"]: row
...
def test_macro_projection_exported_bundle_validates_runtime_shape(tmp_path: Path) -> None:
    ...
    assert result["public_safe_body_digest_count"] == 50
