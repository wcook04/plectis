...
def test_macro_projection_exported_bundle_validates_runtime_shape(tmp_path: Path) -> None:
    ...
    assert result["projection_cell_count"] == 74
    assert result["projection_intake_board"]["ready_cell_count"] == 74
    assert result["projection_intake_board"]["open_actionable_cell_count"] == 0
    ...
    assert result["public_safe_body_material_count"] == 227
    assert result["projection_intake_board"]["public_safe_body_import_count"] == 227
    assert result["runtime_severance_status"] == "pass"
    ...
    assert result["public_safe_body_digest_count"] == 227
