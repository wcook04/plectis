...
def test_health_projection_counts_doctrine_routing_floor() -> None:
    ...
    assert paper_modules["total_objects"] == 103
    assert paper_modules["ready_objects"] == 103
    assert paper_modules["source_authority_counts"] == {
        "json_capsule": 103,
    }
    assert paper_modules["required_residual_relation_count"] == 0
    assert paper_modules["selective_residual_relation_count"] == 10
    assert paper_modules["residual_relation_count"] == 10
...
def test_sources_of_record_resolve_inside_microcosm_root() -> None:
    ...
    for source in declared:
        token = source.split()[0].split("::", 1)[0]
