...
def test_pattern_assimilation_exported_bundle_validates_runtime_shape(
    ...
    assert result["organ_landing_count"] == 38
    ...
    assert result["refinement_receipt_count"] == 33
    ...
    assert result["next_best_lane_result"] == (
        "agent_sandbox_policy_escape_replay_compound_or_indirect_prompt_injection_information_flow_policy_replay_refresh"
    )
