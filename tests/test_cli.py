...
def test_cli_authority_smoke(
    ...
    assert payload["surface_counts"]["copied_non_secret_macro_body_material_count"] == 8
...
def test_cli_intake_smoke(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert payload["projection_cell_count"] == 7
...
def test_cli_macro_projection_plan_smoke(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert payload["projection_intake_board"]["ready_cell_count"] == 7
