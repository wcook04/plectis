...
MANIFEST_PATH = (
    MICROCOSM_ROOT
    / "examples/macro_projection_import_protocol/exported_projection_import_bundle/compression_profiles_source_module_manifest.json"
)
PROTOCOL_PATH = (
    MICROCOSM_ROOT
    / "examples/macro_projection_import_protocol/exported_projection_import_bundle/self_comprehension_packet_compiler_source_projection_protocol.json"
)
...
def test_packet_compiler_source_modules_are_exact_macro_imports() -> None:
    manifest = _load_json(MANIFEST_PATH)

    assert manifest["schema_version"] == "public_microcosm_source_module_manifest_v1"
    assert manifest["manifest_id"] == "self_comprehension_packet_compiler_compression_profiles_import"
    assert manifest["classification"] == "copied_non_secret_macro_body"
    assert manifest["module_count"] == 3
    assert manifest["total_line_count"] > 1400
    assert manifest["body_storage_policy"].startswith("exact_non_secret_compression_profile_source_modules")
...
def test_packet_compiler_projection_protocol_preserves_import_membrane() -> None:
    manifest = _load_json(MANIFEST_PATH)
    protocol = _load_json(PROTOCOL_PATH)

    assert protocol["schema_version"] == "macro_projection_import_protocol_v1"
    assert protocol["protocol_id"] == "self_comprehension_packet_compiler_source_projection_import"
    ...
    assert protocol["material_selection"]["classification"] == "runnable_substrate_source_body_import"
    ...
    assert omission_ids == {
        ...
        "rendered_private_packets_excluded",
    }
    ...
    for material_id, copied in copied_rows.items():
        ...
        assert copied["public_safe_mode"] == "direct_verified_macro_packet_compiler_body"
