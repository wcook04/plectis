...
def test_observatory_legibility_validator_exposes_causal_chain(tmp_path: Path) -> None:
    ...
    assert receipt["projection_safety_audit_proof"]["projection_row_count"] == 35
    assert receipt["projection_safety_audit_proof"]["omission_receipt_count"] == 35
    ...
    assert receipt["agent_reliability_replay_gauntlet_proof"]["episode_count"] == 8
    assert receipt["agent_reliability_replay_gauntlet_proof"]["blocked_episode_count"] == 6
