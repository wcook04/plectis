...
def test_runtime_shell_status_is_product_centered() -> None:
    ...
    assert status["adapter_backed_organ_count"] == 39
...
def test_runtime_shell_spine_is_cold_reader_xray() -> None:
    ...
    assert spine["surface_counts"]["adapter_backed_organ_count"] == 39
    assert len(spine["accepted_runtime_spine"]) == 39
    ...
    assert spine["first_run_path"][39]["command"] == "microcosm benchmark-lab"
    assert spine["first_run_path"][40]["command"].startswith("microcosm agent-benchmark-integrity-anti-gaming-replay")
    assert spine["first_run_path"][41]["command"] == "microcosm legibility-scorecard"
    assert spine["first_run_path"][42]["command"] == "microcosm intake"
    assert spine["first_run_path"][44]["command"] == "microcosm cold-reader-route-map run-route-map-bundle"
...
def test_runtime_shell_authority_map_is_public_safe(tmp_path: Path) -> None:
    ...
    assert authority["surface_counts"]["organ_authority_count"] == 39
    assert authority["surface_counts"]["surface_authority_count"] == 39
...
def test_runtime_shell_projection_safety_lens_is_public_safe(tmp_path: Path) -> None:
    ...
    assert lens["projection_summary"]["projection_row_count"] == 39
    assert lens["projection_summary"]["omission_receipt_count"] == 39
    assert lens["projection_summary"]["reversible_drilldown_count"] == 39
...
def test_runtime_shell_replay_gauntlet_lens_is_public_safe(tmp_path: Path) -> None:
    ...
    assert lens["coverage_summary"]["episode_count"] == 10
    assert lens["coverage_summary"]["blocked_episode_count"] == 8
...
def test_runtime_shell_runs_demo_workflow_against_exported_bundles(tmp_path: Path) -> None:
    ...
    assert [event["status"] for event in result["events"]] == ["pass"] * 39
    ...
    assert trace["otel_shape"]["span_count"] == 39
    assert trace["otel_shape"]["metrics"]["runtime_steps_passed"] == 39
...
def test_runtime_shell_serves_observatory_and_status_endpoint(tmp_path: Path) -> None:
    ...
    assert payload["adapter_backed_organ_count"] == 39
    assert spine["schema_version"] == "microcosm_public_runtime_spine_v1"
    assert tour["schema_version"] == "microcosm_public_ten_minute_tour_v1"
    assert tour["status"] == "pass"
    assert authority["schema_version"] == "microcosm_public_authority_map_v1"
    ...
    assert replay_gauntlet["coverage_summary"]["episode_count"] == 10
