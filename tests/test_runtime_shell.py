...
def test_runtime_shell_status_is_product_centered() -> None:
    ...
    assert status["real_substrate_progress_count"] == 40
    assert status["non_progress_organ_count"] == 2
    assert status["truth_accounting"]["real_runtime_receipt_count"] == 3
    assert status["truth_accounting"]["source_faithful_refactor_count"] == 21
    assert status["truth_accounting"]["real_import_validation_count"] == 16
    assert status["truth_accounting"]["regression_negative_fixture_count"] == 2
...
def test_runtime_shell_spine_is_cold_reader_xray() -> None:
    ...
    assert spine["surface_counts"]["real_substrate_progress_count"] == 40
    assert spine["surface_counts"]["non_progress_organ_count"] == 2
    assert spine["surface_counts"]["real_runtime_receipt_count"] == 3
    assert spine["surface_counts"]["copied_non_secret_macro_body_count"] == 0
    assert spine["surface_counts"]["source_faithful_refactor_count"] == 21
    assert spine["surface_counts"]["real_import_validation_count"] == 16
    assert spine["surface_counts"]["regression_negative_fixture_count"] == 2
    ...
    assert len(spine["real_substrate_progress_spine"]) == 40
    assert len(spine["non_progress_runtime_spine"]) == 2
    ...
    assert spine["truth_accounting"]["real_substrate_progress_count"] == 40
    assert spine["truth_accounting"]["non_progress_organ_count"] == 2
...
def test_runtime_shell_authority_map_is_public_safe(tmp_path: Path) -> None:
    ...
    assert authority["surface_counts"]["real_substrate_progress_count"] == 40
    assert authority["surface_counts"]["non_progress_organ_count"] == 2
    assert authority["surface_counts"]["regression_negative_fixture_count"] == 2
    assert authority["surface_counts"]["hard_boundary_count"] == 6
    assert authority["surface_counts"]["safe_local_exception_count"] == 3
    assert authority["truth_accounting"]["real_substrate_progress_count"] == 40
    assert authority["truth_accounting"]["regression_negative_fixture_count"] == 2
