...
def test_launch_compression_validator_proves_one_command_aha(tmp_path: Path) -> None:
    ...
    assert receipt["projection_safety_summary"]["projection_row_count"] == 40
    assert receipt["projection_safety_summary"]["omission_receipt_count"] == 40
    ...
    assert receipt["agent_reliability_replay_gauntlet_summary"]["episode_count"] == 10
    assert receipt["agent_reliability_replay_gauntlet_summary"]["blocked_episode_count"] == 8
