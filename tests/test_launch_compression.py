...
def test_launch_compression_validator_proves_one_command_aha(tmp_path: Path) -> None:
    ...
    assert receipt["tour_summary"]["route_card_count"] == 10
    ...
    assert receipt["projection_safety_summary"]["projection_row_count"] == 42
    assert receipt["projection_safety_summary"]["omission_receipt_count"] == 42
