# batch8_audio_level_rms_port Audio Level RMS Port

_Generated from the governed organ JSON instance. Do not edit this markdown by hand._

- Source JSON: `organs/batch8_audio_level_rms_port.json`
- Atlas source of record: `core/organ_atlas.json::organs[62:batch8_audio_level_rms_port]`
- Registry source of record: `core/organ_registry.json::implemented_organs[62:batch8_audio_level_rms_port]`
- Authority boundary: JSON parity seed; organ atlas/registry source authority has not flipped.

## Role

An agent runs it to validate that the public Python port preserves the declared RMS normalization semantics over fixture arrays, rejects planted malformed cases, and keeps audio-session, device-capture, publication, and release claims fenced off.

## Lattice Neighbours

- `explained_by` -> `paper_module:paper_module.batch8_audio_level_rms_port` (resolved_paper_module_ref)
- `operates_through` -> `mechanism:mechanism.batch8_audio_level_rms_port.validates_public_audio_level_rms_port` (resolved_json_instance)
- `implemented_by` -> `code_locus:src/microcosm_core/organs/batch8_audio_level_rms_port.py` (resolved_code_locus)
