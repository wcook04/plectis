# batch7_macro_engines_capsule Trace, Code-Map & Scheduling Engines Capsule

_Generated from the governed organ JSON instance. Do not edit this markdown by hand._

- Source JSON: `organs/batch7_macro_engines_capsule.json`
- Atlas source of record: `core/organ_atlas.json::organs[57:batch7_macro_engines_capsule]`
- Registry source of record: `core/organ_registry.json::implemented_organs[57:batch7_macro_engines_capsule]`
- Authority boundary: JSON parity seed; organ atlas/registry source authority has not flipped.

## Role

An agent runs it to validate the Batch-7 source-open capsule: all 9 mechanisms must execute, all 15 copied source modules must preserve required anchors and digests, planted negative cases must be observed, receipts must omit copied body text and private state, and the capsule must stay inside its public authority ceiling.

## Lattice Neighbours

- `explained_by` -> `paper_module:paper_module.batch7_macro_engines_capsule` (resolved_paper_module_ref)
- `operates_through` -> `mechanism:mechanism.batch7_macro_engines_capsule.validates_public_macro_engines_capsule` (resolved_json_instance)
- `implemented_by` -> `code_locus:src/microcosm_core/organs/batch7_macro_engines_capsule.py` (resolved_code_locus)
