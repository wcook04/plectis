# batch9_macro_engines_capsule Backend & Governance Engines Capsule

_Generated from the governed organ JSON instance. Do not edit this markdown by hand._

- Source JSON: `organs/batch9_macro_engines_capsule.json`
- Atlas source of record: `core/organ_atlas.json::organs[65:batch9_macro_engines_capsule]`
- Registry source of record: `core/organ_registry.json::implemented_organs[65:batch9_macro_engines_capsule]`
- Authority boundary: JSON parity seed; organ atlas/registry source authority has not flipped.

## Role

An agent runs it to validate the Batch-9 source-open capsule: all 13 mechanisms must be present, exact copied source modules must preserve public anchors and digests, runtime exercises must pass, planted negative cases must be observed, and receipts must omit copied body text and private state.

## Lattice Neighbours

- `explained_by` -> `paper_module:paper_module.batch9_macro_engines_capsule` (resolved_paper_module_ref)
- `operates_through` -> `mechanism:mechanism.batch9_macro_engines_capsule.validates_public_macro_engines_capsule` (resolved_json_instance)
- `implemented_by` -> `code_locus:src/microcosm_core/organs/batch9_macro_engines_capsule.py` (resolved_code_locus)
