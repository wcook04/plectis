# batch7_demo_take_console_capsule Demo Take Console Source Capsule

_Generated from the governed organ JSON instance. Do not edit this markdown by hand._

- Source JSON: `organs/batch7_demo_take_console_capsule.json`
- Atlas source of record: `core/organ_atlas.json::organs[59:batch7_demo_take_console_capsule]`
- Registry source of record: `core/organ_registry.json::implemented_organs[59:batch7_demo_take_console_capsule]`
- Authority boundary: JSON parity seed; organ atlas/registry source authority has not flipped.

## Role

An agent runs it to validate the Batch 7 Demo Take Console source-open capsule: copied public Swift source bodies must preserve required anchors and digests, SwiftPM build witness posture must be present, deterministic recording-state/helper-bridge/recorder-store/hotkey-audio-meter/transcribe-payload exercises must pass, planted negative cases must be observed, and receipts must stay body-free inside the public authority ceiling.

## Lattice Neighbours

- `explained_by` -> `paper_module:paper_module.batch7_demo_take_console_capsule` (resolved_paper_module_ref)
- `operates_through` -> `mechanism:mechanism.batch7_demo_take_console_capsule.validates_public_demo_take_console_capsule` (resolved_json_instance)
- `implemented_by` -> `code_locus:src/microcosm_core/organs/batch7_demo_take_console_capsule.py` (resolved_code_locus)
