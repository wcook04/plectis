# batch7_secondary_runtime_capsule Secondary Runtime Source Capsule

_Generated from the governed organ JSON instance. Do not edit this markdown by hand._

- Source JSON: `organs/batch7_secondary_runtime_capsule.json`
- Atlas source of record: `core/organ_atlas.json::organs[77:batch7_secondary_runtime_capsule]`
- Registry source of record: `core/organ_registry.json::implemented_organs[77:batch7_secondary_runtime_capsule]`
- Authority boundary: JSON parity seed; organ atlas/registry source authority has not flipped.

## Role

An agent runs it to validate the Batch 7 secondary runtime capsule: all eight runtime, graph, cartography, stockgrid, and Polymarket fixture engines must execute, copied source modules must preserve digest anchors, planted negative cases must stay visible, and receipts must remain body-free inside the public authority ceiling.

## Lattice Neighbours

- `explained_by` -> `paper_module:paper_module.batch7_secondary_runtime_capsule` (resolved_paper_module_ref)
- `operates_through` -> `mechanism:mechanism.batch7_secondary_runtime_capsule.validates_public_secondary_runtime_capsule` (resolved_json_instance)
- `implemented_by` -> `code_locus:src/microcosm_core/organs/batch7_secondary_runtime_capsule.py` (resolved_code_locus)
