# batch11_saturation_engines_capsule Saturation Engines Capsule
...
An agent runs it to validate the Batch-11 saturation-engine capsule: all 12 imported modules must preserve source digests, public exercises must compute from fixture inputs, all 13 negative probes must be computed rather than fixture-verdict echoes, and receipts must omit copied body text and private state.

## Lattice Neighbours

- `explained_by` -> `paper_module:paper_module.batch11_saturation_engines_capsule` (resolved_paper_module_ref)
- `operates_through` -> `mechanism:mechanism.batch11_saturation_engines_capsule.validates_public_saturation_engines_capsule` (resolved_json_instance)
- `implemented_by` -> `code_locus:src/microcosm_core/organs/batch11_saturation_engines_capsule.py` (resolved_code_locus)
