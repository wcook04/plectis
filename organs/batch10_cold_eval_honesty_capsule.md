# batch10_cold_eval_honesty_capsule Cold-Eval Honesty Capsule

_Generated from the governed organ JSON instance. Do not edit this markdown by hand._

- Source JSON: `organs/batch10_cold_eval_honesty_capsule.json`
- Atlas source of record: `core/organ_atlas.json::organs[71:batch10_cold_eval_honesty_capsule]`
- Registry source of record: `core/organ_registry.json::implemented_organs[71:batch10_cold_eval_honesty_capsule]`
- Authority boundary: JSON parity seed; organ atlas/registry source authority has not flipped.

## Role

An agent runs it to validate the cold-eval honesty membrane: the copied cold_eval.py body must match its source digest and anchors, the public exercise must execute run_cold_eval, the scorecard asymmetry must stay fixture-scoped, all four negative cases must be observed, and receipts must omit copied body text, private refs, provider payloads, live-agent results, and release authority.

## Lattice Neighbours

- `explained_by` -> `paper_module:paper_module.batch10_cold_eval_honesty_capsule` (resolved_paper_module_ref)
- `operates_through` -> `mechanism:mechanism.batch10_cold_eval_honesty_capsule.validates_public_cold_eval_honesty_capsule` (resolved_json_instance)
- `implemented_by` -> `code_locus:src/microcosm_core/organs/batch10_cold_eval_honesty_capsule.py` (resolved_code_locus)
