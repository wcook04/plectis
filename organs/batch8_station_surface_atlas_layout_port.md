# batch8_station_surface_atlas_layout_port Surface Atlas Layout Port

_Generated from the governed organ JSON instance. Do not edit this markdown by hand._

- Source JSON: `organs/batch8_station_surface_atlas_layout_port.json`
- Atlas source of record: `core/organ_atlas.json::organs[63:batch8_station_surface_atlas_layout_port]`
- Registry source of record: `core/organ_registry.json::implemented_organs[63:batch8_station_surface_atlas_layout_port]`
- Authority boundary: JSON parity seed; organ atlas/registry source authority has not flipped.

## Role

An agent runs it to validate that the public Python port preserves declared band/lane layout semantics over fixture nodes, rejects planted malformed cases, and keeps UI runtime, browser-render, navigation-authority, publication, and release claims fenced off.

## Lattice Neighbours

- `explained_by` -> `paper_module:paper_module.batch8_station_surface_atlas_layout_port` (resolved_paper_module_ref)
- `operates_through` -> `mechanism:mechanism.batch8_station_surface_atlas_layout_port.validates_public_station_surface_atlas_layout_port` (resolved_json_instance)
- `implemented_by` -> `code_locus:src/microcosm_core/organs/batch8_station_surface_atlas_layout_port.py` (resolved_code_locus)
- `instantiates` -> `concept:concept.import_projection_and_drift_control_bundle` (resolved_json_instance)
- `governed_by` -> `principle:P-2` (resolved_json_instance)
- `governed_by` -> `principle:P-9` (resolved_json_instance)
- `governed_by` -> `principle:P-15` (resolved_json_instance)
- `governed_by` -> `principle:P-16` (resolved_json_instance)
- `constrained_by` -> `axiom:AX-5` (resolved_json_instance)
- `constrained_by` -> `axiom:AX-7` (resolved_json_instance)
- `constrained_by` -> `axiom:AX-8` (resolved_json_instance)
- `constrained_by` -> `axiom:AX-10` (resolved_json_instance)
- `wires_to` -> `organ:batch7_station_runtime_capsule` (resolved_registry_or_atlas_target)

## Anti-Claims

- This organ JSON seed does not flip source authority away from core/organ_atlas.json and core/organ_registry.json.
- Required paper/mechanism/code links prove declared structural references only, not runtime correctness or release readiness.
- Selective concept, principle, axiom, and sibling-organ edges are not inferred from prose, specialty, family, or evidence class.
- Absent selective organ edges are residual pressure, not evidence that no neighbours exist.
- Generated markdown, graph, health, and atlas projections cannot be read back as source evidence.
