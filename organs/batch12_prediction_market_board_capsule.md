# batch12_prediction_market_board_capsule Prediction Market Board Capsule
...
An agent runs it to validate copied quant_presentation_mart.py digests and anchors, execute prediction board, provider drift, missingness, delta, and macro lifecycle helpers over public fixtures, confirm seven negative cases are computed, and preserve claim ceilings around market truth, provider truth, advice, dispatch, publication, and release.

## Lattice Neighbours

- `explained_by` -> `paper_module:paper_module.batch12_prediction_market_board_capsule` (resolved_paper_module_ref)
- `operates_through` -> `mechanism:mechanism.batch12_prediction_market_board_capsule.validates_public_prediction_market_board_capsule` (resolved_json_instance)
- `implemented_by` -> `code_locus:src/microcosm_core/organs/batch12_prediction_market_board_capsule.py` (resolved_code_locus)
