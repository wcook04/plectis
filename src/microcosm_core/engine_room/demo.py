"""
Implements engine room demo for the public Plectis package.

Callers enter through `CapsuleExercise`, `default_root`, `run_capsule`,
`audit_controller_coverage`, `run_demo`, `build_parser`, and 1 more; constants such as
`SCHEMA_VERSION`, `ORGAN_ID`, `CLAIM_CEILING`, `EXPECTED_JEWEL_TARGETS`, and 1 more pin
local fixture names; dependencies include `argparse`, `importlib`, `json`, `dataclasses`,
and 2 more. The implementation is source-owned engine-room code, so receipts and tests
should name these callables directly.
"""
...
class CapsuleExercise:
    """
    Record object for Capsule Exercise.

    It keeps `capsule_id`, `module_name`, `input_dir`, `jewel_targets`, `evaluator_name`,
    and `cli_subcommand` together for the engine room demo flow. Methods such as
    `public_exercise` derive serialized or path-shaped views from that state.
    """
    capsule_id: str
    module_name: str
    input_dir: str
    ...
    def public_exercise(self) -> str:
        """
        Derive public exercise on `CapsuleExercise` without touching module import state.
...
def default_root() -> Path:
    """
    Produce the default root value used by `microcosm_core.engine_room.demo`.
...
def _compact_summary(receipt: Mapping[str, Any]) -> dict[str, Any]:
    """
    Compute compact summary from `receipt`.
...
def _relative_module_path(module_name: str) -> Path:
    """
    Produce the relative module path value used by `microcosm_core.engine_room.demo`.

    Inputs are `module_name`; notable helpers are `with_suffix`, `Path`, and `split`.
    """
...
def _load_json(path: Path) -> Mapping[str, Any]:
    """
    Serialize `microcosm_core.engine_room.demo._load_json` into the payload shape expected
    by engine room demo.
...
def _extract_ids(rows: Any) -> set[str]:
    """
    Compute extract IDs from `rows`.

    Inputs are `rows`; notable helpers are `add` and `get`.
    """
    ids: set[str] = set()
...
def _shared_surface_ids(*, root: Path) -> dict[str, list[str]]:
    """
    Serialize `microcosm_core.engine_room.demo._shared_surface_ids` into the payload shape
    expected by engine room demo.
...
def _capsule_surface_row(exercise: CapsuleExercise, *, root: Path) -> dict[str, Any]:
    """
    Serialize `microcosm_core.engine_room.demo._capsule_surface_row` into the payload shape
    expected by engine room demo.
...
def run_capsule(exercise: CapsuleExercise, *, root: Path) -> dict[str, Any]:
    """
    Serialize `microcosm_core.engine_room.demo.run_capsule` into the payload shape expected
    by engine room demo.
...
def audit_controller_coverage(*, root: Path | None = None, run_exercises: bool = True) -> dict[str, Any]:
    """
    Serialize `microcosm_core.engine_room.demo.audit_controller_coverage` into the payload
    shape expected by engine room demo.
...
def run_demo(*, root: Path | None = None, capsule_ids: Sequence[str] = ()) -> dict[str, Any]:
    """
    Serialize `microcosm_core.engine_room.demo.run_demo` into the payload shape expected by
    engine room demo.
...
def build_parser() -> argparse.ArgumentParser:
    """
    Register CLI syntax for `microcosm_core.engine_room.demo.build_parser`.
...
def main(argv: Sequence[str] | None = None) -> int:
    """
    Run the `microcosm_core.engine_room.demo` command-line entry point.
