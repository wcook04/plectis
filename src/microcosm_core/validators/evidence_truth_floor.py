"""
Implements validators evidence truth floor for the public Plectis package.

Callers enter through `audit_evidence_truth_floor` and `main`; constants such as
`CHECKER_ID`, `EVIDENCE_CLASS_REGISTRY_REL`, `ORGAN_REGISTRY_REL`,
`FIRST_WAVE_RECEIPTS_REL`, and 9 more pin local fixture names; dependencies include
`argparse`, `json`, `os`, `pathlib`, and 2 more. Validator outputs stay structured so
...
def _public_root_for_path(path: str | Path) -> Path:
    """
    Return public root for path for `microcosm_core.validators.evidence_truth_floor`.
...
def _rows(payload: dict[str, Any], key: str) -> list[dict[str, Any]]:
    """
    Return dictionary rows for `microcosm_core.validators.evidence_truth_floor._rows` from
    `payload[key]`.
...
def _accepted_registry_rows_by_organ(public_root: Path) -> dict[str, dict[str, Any]]:
    """
    Produce the accepted registry rows by organ value used by
    `microcosm_core.validators.evidence_truth_floor`.

    Inputs are `public_root`; notable helpers are `read_json_strict`, `ValueError`, `get`,
    and `_rows`; invalid cases raise from the explicit checks in the body.
    """
...
def _synthetic_disposition_value(row: dict[str, Any]) -> str:
    """
    Return synthetic disposition value for the validators evidence truth floor flow.
...
def _disposition_issue(
    ...
    """
    Serialize `microcosm_core.validators.evidence_truth_floor._disposition_issue` into the
    payload shape expected by validators evidence truth floor.
...
def _display(path: Path, *, public_root: Path) -> str:
    """
    Produce the display value used by `microcosm_core.validators.evidence_truth_floor`.

    Inputs are `path` and `public_root`; notable helpers are `as_posix`, `relative_to`, and
    `resolve`.
    """
    try:
...
def _iter_json_receipt_files(root: Path):
    """
    Temporarily apply iter JSON receipt files for callers using a `with` block.
...
def _receipt_paths(public_root: Path, organ_id: str) -> list[Path]:
    """
    Return receipt paths for the validators evidence truth floor flow.

    Inputs are `public_root` and `organ_id`; notable helpers are `is_dir`, `is_file`, and
    `_iter_json_receipt_files`.
    """
...
def _verification(payload: dict[str, Any]) -> dict[str, Any]:
    """
    Return verification for the validators evidence truth floor flow.
...
def _body_in_receipt(payload: dict[str, Any], verification: dict[str, Any]) -> bool | None:
    """
    Compute body in receipt from `payload` and `verification`.

    Inputs are `payload` and `verification`; notable helpers are `get`.
    """
...
def _list_count(value: Any) -> int:
    """
    Return list count for `microcosm_core.validators.evidence_truth_floor`.
...
def _ref_values(verification: dict[str, Any], plural_key: str, singular_key: str) -> list[str]:
    """
    Derive ref values without touching module import state.

    Inputs are `verification`, `plural_key`, and `singular_key`; notable helpers are `get`,
    `strip`, `extend`, `append`, and 1 more.
    """
    refs: list[str] = []
...
def _is_public_body_ref(ref: str) -> bool:
    """
    Return whether is public body ref holds for the validators evidence truth floor flow.

    The result is derived from `ref` with `removeprefix` and `startswith`; failing evidence
...
def _receipt_evidence(public_root: Path, path: Path) -> dict[str, Any] | None:
    """
    Serialize `microcosm_core.validators.evidence_truth_floor._receipt_evidence` into the
    payload shape expected by validators evidence truth floor.
...
def _candidate_from_evidence(
    ...
    """
    Serialize `microcosm_core.validators.evidence_truth_floor._candidate_from_evidence` into
    the payload shape expected by validators evidence truth floor.
...
def _proof_gap_from_evidence(
    ...
    """
    Serialize `microcosm_core.validators.evidence_truth_floor._proof_gap_from_evidence` into
    the payload shape expected by validators evidence truth floor.
...
def audit_evidence_truth_floor(public_root: str | Path) -> dict[str, Any]:
    """
    Serialize `microcosm_core.validators.evidence_truth_floor.audit_evidence_truth_floor`
    into the payload shape expected by validators evidence truth floor.
...
def main(argv: list[str] | None = None) -> int:
    """
    Run `microcosm_core.validators.evidence_truth_floor` as a command-line entry point.
