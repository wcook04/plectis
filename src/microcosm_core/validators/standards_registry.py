"""
Implements validators standards registry for the public Plectis package.

Callers enter through `validate_standards_registry` and `main`; constants such as
`VALIDATOR_ID`, `FIXTURE_ID`, `RECEIPT_REL`, `REQUIRED_STANDARD_FIELDS`, and 8 more pin
local fixture names; dependencies include `argparse`, `pathlib`, `typing`, and
`microcosm_core`. Validator outputs stay structured so release checks can consume findings
...
def _public_root_for_path(path: str | Path) -> Path:
    """
    Return public root for path for `microcosm_core.validators.standards_registry`.
...
def _display_path(path: Path, *, public_root: Path) -> str:
    """
    Produce the display path value used by `microcosm_core.validators.standards_registry`.
...
def _registry_rows(registry: dict[str, Any]) -> list[dict[str, Any]]:
    """
    Return registry rows for the validators standards registry flow.

    Inputs are `registry`; notable helpers are `get`.
    """
...
def _standard_file_for(row: dict[str, Any], standards_dir: Path) -> Path:
    """
    Return standard file for for the validators standards registry flow.

    Inputs are `row` and `standards_dir`; notable helpers are `strip`, `Path`,
    `is_absolute`, and `get`.
    """
...
def _load_standard(path: Path) -> dict[str, Any] | None:
    """
    Load load standard for `microcosm_core.validators.standards_registry`.
...
def _acceptance_status(acceptance: dict[str, Any]) -> dict[str, Any]:
    """
    Serialize `microcosm_core.validators.standards_registry._acceptance_status` into the
    payload shape expected by validators standards registry.
...
def _receipt_safe_scan(scan: dict[str, Any]) -> dict[str, Any]:
    """
    Derive receipt safe scan without touching module import state.
...
def _accepted_organ_standard_contract_alignment(
    ...
    """
    Serialize
    `microcosm_core.validators.standards_registry._accepted_organ_standard_contract_alignment`
    into the payload shape expected by validators standards registry.
    ...
    def add_error(
        ...
        *,
        ...
        """
        Add add error for the validators standards registry flow.
...
def _increment(counter: dict[str, int], key: object) -> None:
    """
    Run increment for `microcosm_core.validators.standards_registry`.
...
def _unique_strings(*values: Any) -> list[str]:
    """
    Produce the unique strings value used by `microcosm_core.validators.standards_registry`.

    Inputs are `values`; notable helpers are `strip`, `append`, and `add`.
    """
    strings: list[str] = []
    seen: set[str] = set()
    for value in values:
...
def _relationships(payload: Any) -> dict[str, Any]:
    """
    Serialize `microcosm_core.validators.standards_registry._relationships` into the payload
    shape expected by validators standards registry.
...
def _contract_projection_status(standard: dict[str, Any]) -> str:
    """
    Derive contract projection status without touching module import state.

    Inputs are `standard`; notable helpers are `get`.
    """
    if (
...
def _used_by_organ_admission_summary(
    ...
    """
    Serialize
    `microcosm_core.validators.standards_registry._used_by_organ_admission_summary` into the
    payload shape expected by validators standards registry.
...
def _activation_witness_gap_summary(
    ...
    """
    Serialize `microcosm_core.validators.standards_registry._activation_witness_gap_summary`
    into the payload shape expected by validators standards registry.
...
def validate_standards_registry(
    ...
    *,
    ...
    """
    Validate whether validate standards registry holds for the validators standards registry
    flow.

    The result is derived from `registry_path`, `standards_dir`, `acceptance_path`,
    `out_path`, and `command` with `Path`, `_public_root_for_path`, `read_json_strict`,
    `_registry_rows`, and 16 more; failing evidence is returned or raised exactly where the
    body says so.
    """
...
def _build_parser() -> argparse.ArgumentParser:
    """
    Register CLI syntax for `microcosm_core.validators.standards_registry._build_parser`.
...
def main(argv: list[str] | None = None) -> int:
    """
    Run the `microcosm_core.validators.standards_registry` command-line entry point.
