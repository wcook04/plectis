"""
Implements validators private state scan for the public Plectis package.
...
def _is_local_residue(path: Path, root: Path) -> bool:
    """
    Return whether is local residue holds for the validators private state scan flow.
...
def _iter_scan_paths(root: Path) -> Iterator[Path]:
    """
    Apply `microcosm_core.validators.private_state_scan._iter_scan_paths` for the lifetime
    of a context block.
...
def validate_scan(root: str | Path, policy: str | Path | None = None) -> dict[str, Any]:
    """
    Validate whether validate scan holds for the validators private state scan flow.
...
def main(argv: list[str] | None = None) -> int:
    """
    Run `microcosm_core.validators.private_state_scan` as a command-line entry point.
