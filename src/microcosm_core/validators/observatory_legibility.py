"""
Implements validators observatory legibility for the public Plectis package.

Callers enter through `validate_legibility` and `main`; constants such as `CHECKER_ID`,
`OBSERVABLE_FIRST_ARTIFACT_CONTRACT_REF`, and `OBSERVABLE_FIRST_ARTIFACT_SLOT_IDS` pin local
fixture names; dependencies include `argparse`, `pathlib`, `typing`, and `microcosm_core`.
...
def _public_relative(root: Path, path: Path) -> str:
    ...
    `microcosm_core.validators.observatory_legibility`.
...
def _rows(payload: dict[str, Any], key: str) -> list[dict[str, Any]]:
    """
    Return dictionary rows for `microcosm_core.validators.observatory_legibility._rows` from
    `payload[key]`.
...
def _html_private_hits(html: str) -> list[str]:
    """
    Produce the html private hits value used by
    `microcosm_core.validators.observatory_legibility`.
...
def _all_false(payload: dict[str, Any], keys: list[str]) -> bool:
    """
    Return whether all false holds for the validators observatory legibility flow.
...
def _source_open_body_floor_legible(floor: dict[str, Any]) -> bool:
    """
    Return whether source open body floor legible holds for the validators observatory
    legibility flow.

    The result is derived from `floor` with `get`; failing evidence is returned or raised
...
def _tour_legibility_visible(tour: dict[str, Any]) -> bool:
    """
    Return whether tour legibility visible holds for the validators observatory legibility
    flow.

    The result is derived from `tour` with `_rows`, `get`, `issubset`, and
    `_source_open_body_floor_legible`; failing evidence is returned or raised exactly where
    the body says so.
    """
...
def _observable_first_artifact_contract(
    *,
    ...
    """
    Serialize the local value into the validators observatory legibility payload shape.
...
def validate_legibility(
    ...
    *,
    ...
    """
    Validate whether validate legibility holds for the validators observatory legibility
    flow.

    The result is derived from `root`, `project`, `out_path`, and `command` with `resolve`,
    `Path`, `RuntimeShell`, `project_observatory`, and 18 more; failing evidence is returned
    or raised exactly where the body says so.
    """
...
def _parser() -> argparse.ArgumentParser:
    """
    Register CLI syntax for `microcosm_core.validators.observatory_legibility._parser`.
...
def main(argv: list[str] | None = None) -> int:
    """
    Run the `microcosm_core.validators.observatory_legibility` command-line entry point.
