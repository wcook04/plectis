"""
Implements validators research kernel density for the public Plectis package.

Callers enter through `validate_density` and `main`; constants such as `CHECKER_ID`,
`REQUIRED_PRIMITIVE_FIELDS`, `REQUIRED_PATTERN_SURFACE_FIELDS`, `REQUIRED_README_PHRASES`,
and 1 more pin local fixture names; dependencies include `argparse`, `os`, `pathlib`,
`typing`, and 1 more. Validator outputs stay structured so release checks can consume
findings without scraping prose.
"""
...
def _public_relative(root: Path, path: Path) -> str:
    """
    Return public relative for the validators research kernel density flow.
...
def _kernel_findings(root: Path) -> tuple[list[dict[str, Any]], list[str]]:
    """
    Build a structured finding row for kernel findings.
...
def _rows(payload: dict[str, Any], key: str) -> list[dict[str, Any]]:
    """
    Return dictionary rows for `microcosm_core.validators.research_kernel_density._rows`
    from `payload[key]`.
...
def _has_json_file(path: Path) -> bool:
    """
    Return whether has JSON file holds for the validators research kernel density flow.

    The result is derived from `path` with `is_dir` and `_iter_json_payload_files`; failing
...
def _iter_json_payload_files(root: Path):
    """
    Temporarily apply iter JSON payload files for callers using a `with` block.
...
def _iter_state_payload_files(state: Path):
    """
    Temporarily apply iter state payload files for callers using a `with` block.
...
def _explanation_ref(explanations: Path, path: Path) -> str:
    """
    Derive explanation ref without touching module import state.

    Inputs are `explanations` and `path`; notable helpers are `relative_to`, `resolve`,
    `Path`, and `as_posix`.
    """
    try:
...
def _file_contains_any(path: Path, markers: tuple[str, ...]) -> bool:
    """
    Return whether file contains any holds for the validators research kernel density flow.

    The result is derived from `path` and `markers` with `open`; failing evidence is
...
def _project_findings(project: Path) -> tuple[list[dict[str, Any]], list[str]]:
    """
    Build a structured finding row for project findings.
...
def validate_density(
    ...
    *,
    ...
    """
    Validate whether validate density holds for the validators research kernel density flow.

    The result is derived from `root`, `out_path`, `command`, and `project` with `resolve`,
    `Path`, `_kernel_findings`, `extend`, and 10 more; failing evidence is returned or
...
def _parser() -> argparse.ArgumentParser:
    """
    Register CLI syntax for `microcosm_core.validators.research_kernel_density._parser`.
...
def main(argv: list[str] | None = None) -> int:
    """
    Run the `microcosm_core.validators.research_kernel_density` command-line entry point.
