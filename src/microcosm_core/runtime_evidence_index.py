"""
Implements runtime evidence index for the public Plectis package.

Callers enter through `compact_receipt_summary` and `list_runtime_evidence`; constants such
as `PASS`, `PRIVATE_STATE_SCAN_RECEIPT_KEY`, `SCHEMA_VERSION`, and `INDEX_MODE` pin local
fixture names; dependencies include `os`, `collections`, `pathlib`, `typing`, and 1 more.
...
def _read_json_object(path: Path) -> dict[str, Any]:
    """
    Serialize `microcosm_core.runtime_evidence_index._read_json_object` into the payload
    shape expected by runtime evidence index.
...
def _public_relative(path: Path, root: Path) -> str:
    """
    Compute public relative from `path` and `root`.
...
def _has_nonempty_list(payload: dict[str, Any], *keys: str) -> bool:
    """
    Return whether has nonempty list holds for the runtime evidence index flow.
...
def _has_body_import_verification(payload: dict[str, Any]) -> bool:
    """
    Return whether has body import verification holds for the runtime evidence index flow.
...
def _receipt_evidence_contract_summary(payload: dict[str, Any]) -> dict[str, Any]:
    """
    Serialize `microcosm_core.runtime_evidence_index._receipt_evidence_contract_summary`
    into the payload shape expected by runtime evidence index.
...
def compact_receipt_summary(path: Path, root: Path) -> dict[str, Any]:
    """
    Serialize `microcosm_core.runtime_evidence_index.compact_receipt_summary` into the
    payload shape expected by runtime evidence index.
...
def _iter_json_files(root: Path) -> Iterator[Path]:
    """
    Apply `microcosm_core.runtime_evidence_index._iter_json_files` for the lifetime of a
    context block.
...
def list_runtime_evidence(
    ...
    """
    Serialize `microcosm_core.runtime_evidence_index.list_runtime_evidence` into the payload
    shape expected by runtime evidence index.
