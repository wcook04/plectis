"""
Implements organs batch7 macro engines capsule for the public Plectis package.

Callers enter through `evaluate_negative_case`, `run`, `run_batch7_bundle`, `result_card`,
and `main`; constants such as `ORGAN_ID`, `FIXTURE_ID`, `VALIDATOR_ID`, `RESULT_NAME`, and
13 more pin local fixture names; dependencies include `argparse`, `ast`, `json`, `socket`,
and 6 more. It builds public fixture, result, card, or verdict structures while keeping
...
def _repo_root(public_root: Path) -> Path:
    ...
    Inputs are `public_root`; notable helpers are `is_file` and `is_dir`.
    """
...
def _run_public_witness(
    ...
    *,
    ...
    """
    Serialize `microcosm_core.organs.batch7_macro_engines_capsule._run_public_witness` into
    the payload shape expected by organs batch7 macro engines capsule.
...
def _load_module(module_name: str, path: Path) -> Any:
    """
    Load load module for `microcosm_core.organs.batch7_macro_engines_capsule`.

    Input comes from `module_name` and `path`; malformed or missing data follows the
...
def _copied_source(public_root: Path, source_ref: str) -> Path:
    """
    Return copied source for the organs batch7 macro engines capsule flow.
...
def _agent_trace_exercise(public_root: Path) -> dict[str, Any]:
    """
    Serialize `microcosm_core.organs.batch7_macro_engines_capsule._agent_trace_exercise`
    into the payload shape expected by organs batch7 macro engines capsule.
...
def _dag_exercise(public_root: Path) -> dict[str, Any]:
    """
    Serialize `microcosm_core.organs.batch7_macro_engines_capsule._dag_exercise` into the
    payload shape expected by organs batch7 macro engines capsule.
...
def _release_root_exercise(public_root: Path) -> dict[str, Any]:
    """
    Serialize `microcosm_core.organs.batch7_macro_engines_capsule._release_root_exercise`
    into the payload shape expected by organs batch7 macro engines capsule.
...
def _source_surgeon_exercise(public_root: Path) -> dict[str, Any]:
    """
    Serialize `microcosm_core.organs.batch7_macro_engines_capsule._source_surgeon_exercise`
    into the payload shape expected by organs batch7 macro engines capsule.
...
def _clean_clone_exercise() -> dict[str, Any]:
    """
    Serialize `microcosm_core.organs.batch7_macro_engines_capsule._clean_clone_exercise`
    into the payload shape expected by organs batch7 macro engines capsule.
    ...
    class NetworkDisabled(RuntimeError):
        """
        Raised when network Disabled fails inside
        `microcosm_core.organs.batch7_macro_engines_capsule`.
    ...
    def blocked(*_args: Any, **_kwargs: Any) -> None:
        """
        Run blocked for `microcosm_core.organs.batch7_macro_engines_capsule`.
...
def _calculator_exercise(public_root: Path) -> dict[str, Any]:
    """
    Serialize `microcosm_core.organs.batch7_macro_engines_capsule._calculator_exercise` into
    the payload shape expected by organs batch7 macro engines capsule.
...
def _pagerank_exercise(public_root: Path) -> dict[str, Any]:
    """
    Serialize `microcosm_core.organs.batch7_macro_engines_capsule._pagerank_exercise` into
    the payload shape expected by organs batch7 macro engines capsule.
...
def _regression_selection_exercise(public_root: Path) -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.batch7_macro_engines_capsule._regression_selection_exercise` into
    the payload shape expected by organs batch7 macro engines capsule.
...
def _safe_engine(engine_id: str, runner: Any) -> dict[str, Any]:
    """
    Serialize `microcosm_core.organs.batch7_macro_engines_capsule._safe_engine` into the
    payload shape expected by organs batch7 macro engines capsule.
...
def _semantic_runtime_exercises(input_ref: str) -> dict[str, dict[str, Any]]:
    """
    Return semantic runtime exercises for the organs batch7 macro engines capsule flow.

    Inputs are `input_ref`; notable helpers are `lru_cache`, `Path`, `public_root_for_path`,
    `_safe_engine`, and 8 more.
    """
...
def _observed_negative_case(
    ...
    """
    Return whether observed negative case holds for the organs batch7 macro engines capsule
    flow.
...
def evaluate_negative_case(
    ...
    """
    Serialize `microcosm_core.organs.batch7_macro_engines_capsule.evaluate_negative_case`
    into the payload shape expected by organs batch7 macro engines capsule.
...
def _source_open_bundle_exercises(source_manifest: Mapping[str, Any]) -> list[dict[str, Any]]:
    """
    Produce the source open bundle exercises value used by
    `microcosm_core.organs.batch7_macro_engines_capsule`.

    Inputs are `source_manifest`; notable helpers are `get`.
    """
...
def _evaluate(
    ...
    """
    Serialize `microcosm_core.organs.batch7_macro_engines_capsule._evaluate` into the
    payload shape expected by organs batch7 macro engines capsule.
...
def run(
    ...
    *,
    ...
    """
    Return run for `microcosm_core.organs.batch7_macro_engines_capsule`.
...
def run_batch7_bundle(
    ...
    *,
    ...
    """
    Compute run batch7 bundle from `bundle_dir`, `out_dir`, `acceptance_out`, and `command`.
...
def result_card(result: Mapping[str, Any]) -> dict[str, Any]:
    """
    Return result card for the organs batch7 macro engines capsule flow.
...
def main(argv: list[str] | None = None) -> int:
    """
    Run `microcosm_core.organs.batch7_macro_engines_capsule` as a command-line entry point.
