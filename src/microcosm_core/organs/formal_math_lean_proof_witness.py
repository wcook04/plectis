"""
Implements organs formal math lean proof witness for the public Plectis package.

Callers enter through `validate_source_module_imports`, `result_card`, `write_receipts`,
`run`, `run_witness_bundle`, and `main`; constants such as `ORGAN_ID`, `FIXTURE_ID`,
`VALIDATOR_ID`, `RESULT_NAME`, and 32 more pin local fixture names; dependencies include
`argparse`, `copy`, `hashlib`, `json`, and 9 more. It builds public fixture, result, card,
...
def _public_root_for_path(path: str | Path) -> Path:
    """
    Return public root for path for the organs formal math lean proof witness flow.
...
def _strings(value: object) -> list[str]:
    ...
    `microcosm_core.organs.formal_math_lean_proof_witness._strings`.
...
def _rows(payload: object, key: str) -> list[dict[str, Any]]:
    """
    Return dictionary rows for `microcosm_core.organs.formal_math_lean_proof_witness._rows`
    from `payload[key]`.
...
def _input_paths(input_dir: Path, *, include_negative: bool) -> list[Path]:
    ...
    Inputs are `input_dir` and `include_negative`; notable helpers are `is_file`, `is_dir`,
    `append`, `extend`, and 1 more.
    """
    paths = [
...
def _iter_lean_project_files(path: Path) -> Iterator[Path]:
    """
    Apply `microcosm_core.organs.formal_math_lean_proof_witness._iter_lean_project_files`
...
def _receipt_is_current(receipt_path: Path, input_paths: list[Path]) -> bool:
    """
    Return whether receipt is current holds for the organs formal math lean proof witness
    flow.
...
def _fresh_bundle_receipt(
    *,
    ...
    """
    Serialize `microcosm_core.organs.formal_math_lean_proof_witness._fresh_bundle_receipt`
    into the payload shape expected by organs formal math lean proof witness.
...
def _sha256(path: Path) -> str:
    ...
    `microcosm_core.organs.formal_math_lean_proof_witness._sha256`.
...
def _sha256_file(path: Path) -> str:
    """
    Return the stable digest computed by
    `microcosm_core.organs.formal_math_lean_proof_witness._sha256_file`.
...
def _source_metadata(path: Path, *, public_root: Path) -> dict[str, Any]:
    """
    Serialize `microcosm_core.organs.formal_math_lean_proof_witness._source_metadata` into
    the payload shape expected by organs formal math lean proof witness.
...
def _normalize_sha256(value: Any) -> str:
    """
    Return the stable digest computed by
    `microcosm_core.organs.formal_math_lean_proof_witness._normalize_sha256`.
...
def _source_module_source_path(
    ...
    *,
    ...
    """
    Compute source module source path from `row` and `public_root`.

    Inputs are `row` and `public_root`; notable helpers are `_strip_microcosm_prefix`,
    `is_absolute`, `get`, and `Path`.
    """
...
def _source_module_target_path(
    ...
    *,
    ...
    """
    Return source module target path for the organs formal math lean proof witness flow.
...
def _dedupe_paths(paths: list[Path]) -> list[Path]:
    """
    Produce the dedupe paths value used by
    `microcosm_core.organs.formal_math_lean_proof_witness`.
...
def validate_source_module_imports(
    ...
    *,
    ...
    """
    Serialize
    `microcosm_core.organs.formal_math_lean_proof_witness.validate_source_module_imports`
    into the payload shape expected by organs formal math lean proof witness.
...
def _empty_source_module_imports(input_dir: str | Path, *, public_root: Path) -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.formal_math_lean_proof_witness._empty_source_module_imports` into
    the payload shape expected by organs formal math lean proof witness.
...
def _source_open_body_import_summary(source_imports: dict[str, Any]) -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.formal_math_lean_proof_witness._source_open_body_import_summary`
    into the payload shape expected by organs formal math lean proof witness.
...
def _import_names(text: str) -> list[str]:
    """
    Compute import names from `text`.
...
def _forbidden_imports(text: str) -> list[str]:
    """
    Derive forbidden imports without touching module import state.

    Inputs are `text`; notable helpers are `_import_names`, `append`, and `startswith`.
    """
    blocked: list[str] = []
...
def _iter_forbidden_manifest_keys(value: object, prefix: str = "") -> list[str]:
    """
    Derive iter forbidden manifest keys without touching module import state.

    Inputs are `value` and `prefix`; notable helpers are `items`, `extend`, `append`, and
    `_iter_forbidden_manifest_keys`.
    """
    found: list[str] = []
...
def _private_source_refs(payload: object) -> list[str]:
    """
    Compute private source refs from `payload`.

    Inputs are `payload`; notable helpers are `items`, `extend`, `endswith`,
...
def _finding(
    ...
    *,
    ...
    """
    Serialize `microcosm_core.organs.formal_math_lean_proof_witness._finding` into the
    payload shape expected by organs formal math lean proof witness.
...
def _record(
    ...
    *,
    ...
    """
    Record record for the organs formal math lean proof witness flow.
...
def _run_command(argv: list[str], *, cwd: Path, timeout_seconds: int = 30) -> dict[str, Any]:
    """
    Serialize `microcosm_core.organs.formal_math_lean_proof_witness._run_command` into the
    payload shape expected by organs formal math lean proof witness.
...
def _tool_versions() -> dict[str, Any]:
    """
    Produce the tool versions value used by
    `microcosm_core.organs.formal_math_lean_proof_witness`.

    Notable helpers are `which`, `_skipped_version_probe`, and `deepcopy`.
    """
...
def _standalone_exported_tool_versions() -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.formal_math_lean_proof_witness._standalone_exported_tool_versions`
    into the payload shape expected by organs formal math lean proof witness.
...
def _skipped_version_probe(tool_name: str, tool_path: str | None) -> dict[str, Any]:
    """
    Serialize `microcosm_core.organs.formal_math_lean_proof_witness._skipped_version_probe`
    into the payload shape expected by organs formal math lean proof witness.
...
def _lake_project_cache_key(input_dir: Path) -> str:
    """
    Derive lake project cache key without touching module import state.
...
def _copy_project_to_temp(input_dir: Path, temp_root: Path) -> Path:
    """
    Derive copy project to temp without touching module import state.
...
def _remember_built_lake_project(input_dir: Path, project_dir: Path) -> None:
    """
    Run remember built lake project for the organs formal math lean proof witness flow.
...
def _standalone_exported_lake_build(manifest_result: dict[str, Any]) -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.formal_math_lean_proof_witness._standalone_exported_lake_build`
    into the payload shape expected by organs formal math lean proof witness.
...
def _validate_manifest(payload: dict[str, Any], *, public_root: Path) -> dict[str, Any]:
    """
    Serialize `microcosm_core.organs.formal_math_lean_proof_witness._validate_manifest` into
    the payload shape expected by organs formal math lean proof witness.
...
def _validate_negative_manifest(
    ...
    *,
    ...
    """
    Serialize
    `microcosm_core.organs.formal_math_lean_proof_witness._validate_negative_manifest` into
    the payload shape expected by organs formal math lean proof witness.
...
def _merge_observed(*results: dict[str, Any]) -> dict[str, list[str]]:
    """
    Derive merge observed without touching module import state.

    Inputs are `results`; notable helpers are `items`, `update`, `get`, and `setdefault`.
    """
...
def _merge_findings(*results: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    `microcosm_core.organs.formal_math_lean_proof_witness._merge_findings`.
...
def _build_result(
    ...
    *,
    ...
    """
    Serialize `microcosm_core.organs.formal_math_lean_proof_witness._build_result` into the
    payload shape expected by organs formal math lean proof witness.
...
def _common_receipt(
    ...
    *,
    ...
    """
    Return common receipt for `microcosm_core.organs.formal_math_lean_proof_witness`.
...
def _list_count(value: object) -> int:
    """
    Return list count for the organs formal math lean proof witness flow.
...
def _dict_value(payload: dict[str, Any], key: str) -> dict[str, Any]:
    """
    Return dict value for `microcosm_core.organs.formal_math_lean_proof_witness`.
...
def result_card(result: dict[str, Any]) -> dict[str, Any]:
    """
    Serialize `microcosm_core.organs.formal_math_lean_proof_witness.result_card` into the
    payload shape expected by organs formal math lean proof witness.
...
def write_receipts(
    ...
    *,
    ...
    """
    Write write receipts for the organs formal math lean proof witness flow.
...
def run(
    ...
    *,
    ...
    """
    Produce the run value used by `microcosm_core.organs.formal_math_lean_proof_witness`.
...
def run_witness_bundle(
    ...
    """
    Produce the run witness bundle value used by
    `microcosm_core.organs.formal_math_lean_proof_witness`.

    Inputs are `input_dir`, `out_dir`, and `command`; notable helpers are `Path`,
    `_public_root_for_path`, `_fresh_bundle_receipt`, `_build_result`, and 7 more.
    """
    input_path = Path(input_dir)
    command_text = command or (
...
def main(argv: list[str] | None = None) -> int:
    """
    Run `microcosm_core.organs.formal_math_lean_proof_witness` as a command-line entry
    point.
