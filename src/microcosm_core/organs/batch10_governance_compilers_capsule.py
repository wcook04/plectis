"""
Implements organs batch10 governance compilers capsule for the public Plectis package.

Callers enter through `classify_latest_user_intent`, `latest_intent_gate`,
`resolve_horizon`, `clamp_root_number`, `root_object_lane_width_profile`,
`solve_root_object_lane_widths`, and 5 more; constants such as `ORGAN_ID`, `FIXTURE_ID`,
`VALIDATOR_ID`, `RESULT_NAME`, and 17 more pin local fixture names; dependencies include
`argparse`, `fnmatch`, `functools`, `hashlib`, and 8 more. It builds public fixture, result,
...
def _normalize_text(value: Any) -> str:
    """
    Derive normalize text without touching module import state.

    Inputs are `value`; notable helpers are `sub`, `casefold`, and `strip`.
    """
...
def classify_latest_user_intent(message: str | None) -> str:
    """
    Return classify latest user intent for the organs batch10 governance compilers capsule
    flow.

    Inputs are `message`; notable helpers are `_normalize_text` and `startswith`.
    """
...
def latest_intent_gate(message: str | None, requested_route: str | None = None) -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.batch10_governance_compilers_capsule.latest_intent_gate` into the
    payload shape expected by organs batch10 governance compilers capsule.
...
def _mutation_governance_matrix() -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.batch10_governance_compilers_capsule._mutation_governance_matrix`
    into the payload shape expected by organs batch10 governance compilers capsule.
...
def _extract_operations_from_artifact(text: str) -> list[dict[str, Any]]:
    """
    Produce the extract operations from artifact value used by
    `microcosm_core.organs.batch10_governance_compilers_capsule`.

    Inputs are `text`; notable helpers are `findall`, `append`, `loads`, and `get`.
    """
...
def _observe_apply_compiler_matrix() -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.batch10_governance_compilers_capsule._observe_apply_compiler_matrix`
    into the payload shape expected by organs batch10 governance compilers capsule.
...
def _status_from_requirements(requirements: Mapping[str, bool]) -> str:
    """
    Return status from requirements for the organs batch10 governance compilers capsule
    flow.

    Inputs are `requirements`; notable helpers are `values`.
    """
...
def _public_artifact_fixture_docs(*, boundary_doc_exists: bool = True) -> dict[str, str]:
    """
    Serialize
    `microcosm_core.organs.batch10_governance_compilers_capsule._public_artifact_fixture_docs`
    into the payload shape expected by organs batch10 governance compilers capsule.
...
def _artifact_doc(docs: Mapping[str, str], rel_path: str) -> str:
    """
    Derive artifact doc without touching module import state.

    Inputs are `docs` and `rel_path`; notable helpers are `get`.
    """
...
def _persona_results_from_public_artifact(
    ...
    *,
    ...
    """
    Compute persona results from public artifact from `docs` and `sequence_node_count`.

    Inputs are `docs` and `sequence_node_count`; notable helpers are `_artifact_doc`,
    `join`, `_status_from_requirements`, `append`, and 2 more.
    """
...
def _route_quality_from_public_artifact(
    ...
    *,
    ...
    """
    Derive route quality from public artifact without touching module import state.

    Inputs are `docs` and `manifest_exists`; notable helpers are
    `_status_from_requirements`, `items`, and `_artifact_doc`.
    """
    checks = {
        "tutorial": {
...
def _reviewer_decision(
    ...
    """
    Derive reviewer decision without touching module import state.

    Inputs are `command_receipts`, `persona_results`, and `route_quality`; notable helpers
    are `get` and `values`.
    """
...
def _reviewer_gauntlet_matrix() -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.batch10_governance_compilers_capsule._reviewer_gauntlet_matrix`
    into the payload shape expected by organs batch10 governance compilers capsule.
...
def _blocker_class(blocker_id: str, row: Mapping[str, Any]) -> str:
    """
    Compute blocker class from `blocker_id` and `row`.

    Inputs are `blocker_id` and `row`; notable helpers are `get`.
    """
...
def _release_toggle_triage_matrix() -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.batch10_governance_compilers_capsule._release_toggle_triage_matrix`
    into the payload shape expected by organs batch10 governance compilers capsule.
...
def _selector_list(spec: Mapping[str, Any]) -> list[str]:
    """
    Return selector list for the organs batch10 governance compilers capsule flow.

    Inputs are `spec`; notable helpers are `get`, `extend`, and `strip`.
    """
    selectors: list[str] = []
...
def _path_matches_selector(path_text: str, selector: str) -> bool:
    """
    Return whether path matches selector holds for the organs batch10 governance compilers
    capsule flow.

    The result is derived from `path_text` and `selector` with `strip`, `endswith`,
    `fnmatch`, `startswith`, and 1 more; failing evidence is returned or raised exactly
...
def _hard_exclude_selectors(exclude: Mapping[str, Any]) -> list[tuple[str, str]]:
    """
    Return hard exclude selectors for the organs batch10 governance compilers capsule flow.

    Inputs are `exclude`; notable helpers are `items`, `extend`, `get`, and
    `_selector_list`.
    """
...
def _publication_manifest_contract_matrix() -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.batch10_governance_compilers_capsule._publication_manifest_contract_matrix`
    into the payload shape expected by organs batch10 governance compilers capsule.
...
def _fingerprint_rows_by_path(rows: Any) -> dict[str, str | None]:
    """
    Derive fingerprint rows by path without touching module import state.
...
def _changed_stable_inputs(current_inputs: Any, prior_inputs: Any) -> list[dict[str, Any]]:
    """
    Return changed stable inputs for the organs batch10 governance compilers capsule flow.

    Inputs are `current_inputs` and `prior_inputs`; notable helpers are
    `_fingerprint_rows_by_path`, `get`, and `append`.
    """
...
def _receipt_reuse_matrix() -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.batch10_governance_compilers_capsule._receipt_reuse_matrix` into
    the payload shape expected by organs batch10 governance compilers capsule.
...
def resolve_horizon(policy: str | None = None, now: datetime | None = None) -> dict[str, str]:
    """
    Serialize `microcosm_core.organs.batch10_governance_compilers_capsule.resolve_horizon`
    into the payload shape expected by organs batch10 governance compilers capsule.
...
def _finance_temporal_matrix() -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.batch10_governance_compilers_capsule._finance_temporal_matrix`
    into the payload shape expected by organs batch10 governance compilers capsule.
...
def _session_wave_matrix() -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.batch10_governance_compilers_capsule._session_wave_matrix` into
    the payload shape expected by organs batch10 governance compilers capsule.
...
def _path_overlaps(left: str, right: str) -> bool:
    """
    Return whether path overlaps holds for the organs batch10 governance compilers capsule
    flow.

    The result is derived from `left` and `right` with `startswith`; failing evidence is
...
def _claim_conflict_wait_tax_matrix() -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.batch10_governance_compilers_capsule._claim_conflict_wait_tax_matrix`
    into the payload shape expected by organs batch10 governance compilers capsule.
...
def _resolve_group_state(group: Mapping[str, Any]) -> str:
    """
    Return resolve group state for the organs batch10 governance compilers capsule flow.

    Inputs are `group`; notable helpers are `strip` and `get`.
    """
...
def _resolve_blocked_groups(groups: list[dict[str, Any]]) -> int:
    """
    Produce the resolve blocked groups value used by
    `microcosm_core.organs.batch10_governance_compilers_capsule`.

    Inputs are `groups`; notable helpers are `strip`, `_resolve_group_state`, `lower`, and
    `get`.
    """
...
def _role_aware_dag_matrix() -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.batch10_governance_compilers_capsule._role_aware_dag_matrix` into
    the payload shape expected by organs batch10 governance compilers capsule.
...
def clamp_root_number(value: float, minimum: float, maximum: float) -> float:
    """
    Compute clamp root number from `value`, `minimum`, and `maximum`.

    Inputs are `value`, `minimum`, and `maximum`.
    """
...
def root_object_lane_width_profile(lane: str, selected: bool, lane_count: int) -> dict[str, float]:
    """
    Serialize
    `microcosm_core.organs.batch10_governance_compilers_capsule.root_object_lane_width_profile`
    into the payload shape expected by organs batch10 governance compilers capsule.
...
def solve_root_object_lane_widths(lanes: Sequence[str], selected: bool, margin_x: int, gap: int) -> dict[str, int]:
    """
    Return solve root object lane widths for the organs batch10 governance compilers capsule
    flow.

    Inputs are `lanes`, `selected`, `margin_x`, and `gap`; notable helpers are `floor`,
    `root_object_lane_width_profile`, and `clamp_root_number`.
    """
    canvas_width = 1060
...
def _lane_width_binding_matrix(public_root: Path) -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.batch10_governance_compilers_capsule._lane_width_binding_matrix`
    into the payload shape expected by organs batch10 governance compilers capsule.
...
def _value_at_path(payload: Mapping[str, Any], path: str) -> Any:
    """
    Produce the value at path value used by
    `microcosm_core.organs.batch10_governance_compilers_capsule`.
...
def _expected_matches(value: Any, binding: Mapping[str, Any]) -> bool:
    """
    Return whether expected matches holds for the organs batch10 governance compilers
    capsule flow.

    The result is derived from `value` and `binding` with `get`; failing evidence is
    returned or raised exactly where the body says so.
    """
...
def _canonical_digest(payload: Mapping[str, Any]) -> str:
    ...
    `microcosm_core.organs.batch10_governance_compilers_capsule._canonical_digest`.
...
def _negative_case_payloads(input_dir: Path) -> dict[str, dict[str, Any]]:
    ...
    Inputs are `input_dir`; notable helpers are `loads`, `is_file`, and `read_text`.
    """
...
def _case_probe_missing(case_id: str, reason: str) -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.batch10_governance_compilers_capsule._case_probe_missing` into
    the payload shape expected by organs batch10 governance compilers capsule.
...
def _publication_hard_exclude_rejected(included_path: str, hard_exclude: str) -> bool:
    """
    Return whether publication hard exclude rejected holds for the organs batch10 governance
    compilers capsule flow.

    The result is derived from `included_path` and `hard_exclude` with `items`, `get`,
    `_hard_exclude_selectors`, and `_path_matches_selector`; failing evidence is returned or
...
def _release_operator_review_ready(public_toggle_status: str, blocking_condition_count: int) -> bool:
    """
    Return whether release operator review ready holds for the organs batch10 governance
    compilers capsule flow.

    The result is derived from `public_toggle_status` and `blocking_condition_count` with
    `_blocker_class` and `get`; failing evidence is returned or raised exactly where the
    body says so.
    """
    gate = {
...
def _session_node_state(node: str, dependencies: Sequence[str], states: Mapping[str, Any]) -> str:
    """
    Compute session node state from `node`, `dependencies`, and `states`.

    Inputs are `node`, `dependencies`, and `states`; notable helpers are `setdefault`,
    `items`, and `get`.
    """
...
def _semantic_observed_value(
    ...
    """
    Return semantic observed value for the organs batch10 governance compilers capsule flow.

    Inputs are `case_id`, `probe_input`, and `public_root`; notable helpers are
    `_case_probe_missing`, `latest_intent_gate`, `_public_artifact_fixture_docs`,
    `_persona_results_from_public_artifact`, and 12 more.
    """
...
def _compute_negative_case_probe(
    ...
    *,
    ...
    """
    Serialize
    `microcosm_core.organs.batch10_governance_compilers_capsule._compute_negative_case_probe`
    into the payload shape expected by organs batch10 governance compilers capsule.
...
def _semantic_negative_result(case_id: str, error_codes: tuple[str, ...]) -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.batch10_governance_compilers_capsule._semantic_negative_result`
    into the payload shape expected by organs batch10 governance compilers capsule.
...
def _semantic_negative_not_rejected(case_id: str, observed: Any) -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.batch10_governance_compilers_capsule._semantic_negative_not_rejected`
    into the payload shape expected by organs batch10 governance compilers capsule.
...
def _semantic_negative_error(case_id: str, exc: Exception) -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.batch10_governance_compilers_capsule._semantic_negative_error`
    into the payload shape expected by organs batch10 governance compilers capsule.
...
def _source_manifest_for_input(input_dir: Path) -> dict[str, Any]:
    """
    Produce the source manifest for input value used by
    `microcosm_core.organs.batch10_governance_compilers_capsule`.

    Inputs are `input_dir`; notable helpers are `public_root_for_path` and
    `validate_source_manifest`.
    """
...
def evaluate_negative_case(
    ...
    """
    Compute evaluate negative case from `case_id`, `input_dir`, and `expected_codes`.

    Inputs are `case_id`, `input_dir`, and `expected_codes`; notable helpers are
    `_source_manifest_for_input`, `public_root_for_path`, `get`,
    `_compute_negative_case_probe`, and 4 more.
    """
    try:
...
def _raw_source_manifest(source_manifest: Mapping[str, Any]) -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.batch10_governance_compilers_capsule._raw_source_manifest` into
    the payload shape expected by organs batch10 governance compilers capsule.
...
def _source_evidence(
    ...
    """
    Return source evidence for the organs batch10 governance compilers capsule flow.

    Inputs are `mechanism_id` and `source_manifest`; notable helpers are
    `_raw_source_manifest`, `get`, and `append`.
    """
...
def _build_integrity_matrix(
    ...
    """
    Serialize
    `microcosm_core.organs.batch10_governance_compilers_capsule._build_integrity_matrix`
    into the payload shape expected by organs batch10 governance compilers capsule.
...
def _evaluate(
    ...
    """
    Serialize `microcosm_core.organs.batch10_governance_compilers_capsule._evaluate` into
    the payload shape expected by organs batch10 governance compilers capsule.
...
def run_batch10_governance_compilers_bundle(
    ...
    *,
    ...
    """
    Produce the run batch10 governance compilers bundle value used by
    `microcosm_core.organs.batch10_governance_compilers_capsule`.
...
def result_card(result: Mapping[str, Any]) -> dict[str, Any]:
    """
    Return result card for `microcosm_core.organs.batch10_governance_compilers_capsule`.
...
def main(argv: list[str] | None = None) -> int:
    """
    Run `microcosm_core.organs.batch10_governance_compilers_capsule` as a command-line entry
    point.
