"""
Implements organs durable agent work landing replay for the public Plectis package.

Callers enter through `validate_source_module_imports`, `validate_projection_protocol`,
`validate_landing_policy`, `validate_work_landing_runs`, `run`, `run_work_landing_bundle`,
and 2 more; constants such as `ORGAN_ID`, `FIXTURE_ID`, `VALIDATOR_ID`, `RESULT_NAME`, and
21 more pin local fixture names; dependencies include `argparse`, `hashlib`, `json`,
...
def _public_root_for_path(path: str | Path) -> Path:
    """
    Return public root for path for the organs durable agent work landing replay flow.
...
def _display(path: Path, *, public_root: Path) -> str:
    """
    Return display for `microcosm_core.organs.durable_agent_work_landing_replay`.
...
def _rows(payload: object, key: str) -> list[dict[str, Any]]:
    ...
    `microcosm_core.organs.durable_agent_work_landing_replay._rows` from `payload[key]`.
...
def _strings(value: object) -> list[str]:
    ...
    `microcosm_core.organs.durable_agent_work_landing_replay._strings`.
...
def _input_paths(input_dir: Path, *, include_negative: bool) -> list[Path]:
    """
    Return input paths for the organs durable agent work landing replay flow.
...
def _sha256(path: Path) -> str:
    ...
    `microcosm_core.organs.durable_agent_work_landing_replay._sha256`.
...
def validate_source_module_imports(
    ...
    *,
    ...
    """
    Serialize
    `microcosm_core.organs.durable_agent_work_landing_replay.validate_source_module_imports`
    into the payload shape expected by organs durable agent work landing replay.
...
def _empty_source_module_imports(input_dir: str | Path, *, public_root: Path) -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.durable_agent_work_landing_replay._empty_source_module_imports`
    into the payload shape expected by organs durable agent work landing replay.
...
def _source_open_body_import_summary(source_imports: dict[str, Any]) -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.durable_agent_work_landing_replay._source_open_body_import_summary`
    into the payload shape expected by organs durable agent work landing replay.
...
def _freshness_basis(input_dir: Path, *, include_negative: bool) -> dict[str, Any]:
    """
    Serialize `microcosm_core.organs.durable_agent_work_landing_replay._freshness_basis`
    into the payload shape expected by organs durable agent work landing replay.
...
def _fresh_bundle_receipt(input_dir: Path, out_dir: Path) -> dict[str, Any] | None:
    """
    Compute fresh bundle receipt from `input_dir` and `out_dir`.
...
def _load_payloads(input_dir: Path, *, include_negative: bool) -> dict[str, Any]:
    """
    Load load payloads for `microcosm_core.organs.durable_agent_work_landing_replay`.
...
def _finding(
    ...
    *,
    ...
    """
    Serialize `microcosm_core.organs.durable_agent_work_landing_replay._finding` into the
    payload shape expected by organs durable agent work landing replay.
...
def _record(
    ...
    *,
    ...
    """
    Record record for the organs durable agent work landing replay flow.
...
def _merge_observed(*results: dict[str, Any]) -> dict[str, list[str]]:
    """
    Return merge observed for the organs durable agent work landing replay flow.
...
def _merge_findings(*results: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    `microcosm_core.organs.durable_agent_work_landing_replay._merge_findings`.
...
def _walk_keys(payload: object) -> list[str]:
    """
    Return walk keys for `microcosm_core.organs.durable_agent_work_landing_replay`.
...
def _walk_strings(payload: object) -> list[str]:
    """
    Return walk strings for the organs durable agent work landing replay flow.
...
def validate_projection_protocol(payload: object) -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.durable_agent_work_landing_replay.validate_projection_protocol`
    into the payload shape expected by organs durable agent work landing replay.
...
def validate_landing_policy(payload: object) -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.durable_agent_work_landing_replay.validate_landing_policy` into
    the payload shape expected by organs durable agent work landing replay.
...
def _validate_run_row(
    ...
    *,
    ...
    """
    Validate whether validate run row holds for the organs durable agent work landing replay
    flow.

    The result is derived from `row`, `case_id`, `observed`, and `findings` with `_strings`,
    `get`, `_record`, `_walk_keys`, and 1 more; failing evidence is returned or raised
...
def validate_work_landing_runs(
    ...
    """
    Serialize
    `microcosm_core.organs.durable_agent_work_landing_replay.validate_work_landing_runs`
    into the payload shape expected by organs durable agent work landing replay.
...
def _build_result(
    ...
    *,
    ...
    """
    Serialize `microcosm_core.organs.durable_agent_work_landing_replay._build_result` into
    the payload shape expected by organs durable agent work landing replay.
...
def _board_from_result(result: dict[str, Any]) -> dict[str, Any]:
    """
    Serialize `microcosm_core.organs.durable_agent_work_landing_replay._board_from_result`
    into the payload shape expected by organs durable agent work landing replay.
...
def _write_receipts(
    ...
    *,
    ...
    """
    Serialize `microcosm_core.organs.durable_agent_work_landing_replay._write_receipts` into
    the payload shape expected by organs durable agent work landing replay.
...
def run(
    ...
    *,
    ...
    """
    Return run for the organs durable agent work landing replay flow.
...
def run_work_landing_bundle(
    ...
    *,
    ...
    """
    Produce the run work landing bundle value used by
    `microcosm_core.organs.durable_agent_work_landing_replay`.
...
def _card_receipt_paths(paths: object) -> list[str]:
    """
    Return card receipt paths for `microcosm_core.organs.durable_agent_work_landing_replay`.
...
def result_card(result: dict[str, Any]) -> dict[str, Any]:
    """
    Serialize `microcosm_core.organs.durable_agent_work_landing_replay.result_card` into the
    payload shape expected by organs durable agent work landing replay.
...
def _parser() -> argparse.ArgumentParser:
    ...
    `microcosm_core.organs.durable_agent_work_landing_replay._parser`.
...
def main(argv: list[str] | None = None) -> int:
    """
    Run the `microcosm_core.organs.durable_agent_work_landing_replay` command-line entry
    point.
