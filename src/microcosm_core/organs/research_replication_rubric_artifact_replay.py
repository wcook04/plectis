"""
Implements organs research replication rubric artifact replay for the public Plectis
package.

Callers enter through `validate_source_module_imports`,
`validate_execution_artifact_replay`, `validate_projection_protocol`,
`validate_replication_policy`, `validate_research_replays`, `run`, and 3 more; constants
such as `ORGAN_ID`, `FIXTURE_ID`, `VALIDATOR_ID`, `RESULT_NAME`, and 24 more pin local
...
EXECUTION_ARTIFACT_MANIFEST_REF = (
    "execution_artifacts/execution_artifact_manifest.json"
)
...
EXPECTED_NEGATIVE_CASE_BY_CODE = {
    code: case_id
...
def _display(path: Path, *, public_root: Path) -> str:
    """
    Return display for the organs research replication rubric artifact replay flow.
...
def _rows(payload: object, key: str) -> list[dict[str, Any]]:
    ...
    `microcosm_core.organs.research_replication_rubric_artifact_replay._rows` from
    `payload[key]`.
...
def _strings(value: object) -> list[str]:
    ...
    `microcosm_core.organs.research_replication_rubric_artifact_replay._strings`.
...
def _sha256(path: Path) -> str:
    ...
    `microcosm_core.organs.research_replication_rubric_artifact_replay._sha256`.
...
def _sha256_prefixed(value: object) -> str:
    """
    Return the stable digest computed by
    `microcosm_core.organs.research_replication_rubric_artifact_replay._sha256_prefixed`.
...
def _canonical_payload_sha256(payload: object) -> str:
    """
    Return the stable digest computed by
    `microcosm_core.organs.research_replication_rubric_artifact_replay._canonical_payload_sha256`.
...
def _json_file_sha256_for_payload(payload: object) -> str:
    """
    Return the stable digest computed by
    `microcosm_core.organs.research_replication_rubric_artifact_replay._json_file_sha256_for_payload`.
...
def _is_safe_public_ref(ref: str) -> bool:
    """
    Return whether is safe public ref holds for the organs research replication rubric
    artifact replay flow.

    The result is derived from `ref` with `Path` and `is_absolute`; failing evidence is
...
def _public_ref_path(input_dir: Path, ref: str) -> Path:
    """
    Return public ref path for the organs research replication rubric artifact replay flow.

    Inputs are `input_dir` and `ref`; notable helpers are `Path`.
    """
...
def _body_import_verification(
    ...
    *,
    ...
    """
    Serialize
    `microcosm_core.organs.research_replication_rubric_artifact_replay._body_import_verification`
    into the payload shape expected by organs research replication rubric artifact replay.
    ...
    return {
        **base_verification,
        ...
        "source_module_digest_count": source_module_count,
...
def _walk_keys(payload: object) -> list[str]:
    """
    Return walk keys for
    `microcosm_core.organs.research_replication_rubric_artifact_replay`.

    Inputs are `payload`; notable helpers are `values`, `extend`, and `_walk_keys`.
    """
...
def _source_module_target_path(
    ...
    *,
    ...
    """
    Derive source module target path without touching module import state.

    Inputs are `row`, `manifest_path`, and `public_root`; notable helpers are
    `_strip_microcosm_prefix`, `get`, `_is_safe_public_ref`, and `_display`.
    """
    target_ref = _strip_microcosm_prefix(str(row.get("target_ref") or ""))
    row_path = str(row.get("path") or "")
    if row_path:
        if not _is_safe_public_ref(row_path):
            return public_root / "__invalid_source_module_path__", row_path
...
def _source_module_source_path(
    ...
    *,
    ...
    """
    Derive source module source path without touching module import state.

    Inputs are `row` and `public_root`; notable helpers are `_strip_microcosm_prefix`,
    `is_file`, `_is_safe_public_ref`, `resolve`, and 2 more.
    """
    source_ref = _strip_microcosm_prefix(str(row.get("source_ref") or ""))
    if not source_ref:
        return None, ""
    if not _is_safe_public_ref(source_ref):
        return public_root / "__invalid_source_module_source__", source_ref

    public_source = public_root / source_ref
    if public_source.is_file():
        return public_source, source_ref

    installed_public_root = Path(__file__).resolve().parents[3]
    installed_source = installed_public_root / source_ref
    return installed_source, source_ref


def _source_artifact_paths(input_dir: Path, *, public_root: Path) -> list[Path]:
    """
    Compute source artifact paths from `input_dir` and `public_root`.

    Inputs are `input_dir` and `public_root`; notable helpers are
    `_source_module_manifest_path`, `read_json_strict`, `_rows`, `is_file`, and 4 more.
    """
    ...
    for row in _rows(manifest, "modules"):
        ...
        if str(row.get("source_to_target_relation") or "") == "exact_copy":
            source_path, _source_ref = _source_module_source_path(
                row,
...
def _execution_artifact_paths(input_dir: Path) -> list[Path]:
    """
    Compute execution artifact paths from `input_dir`.

    Inputs are `input_dir`; notable helpers are `is_file`, `read_json_strict`, `extend`,
    `_rows`, and 5 more.
    """
    ...
    if isinstance(manifest, dict):
        refs.extend(_strings(manifest.get("allowed_public_input_refs")))
        refs.extend(_strings(manifest.get("declared_artifact_hash_refs")))
        refs.extend(_strings(manifest.get("declared_public_input_hash_refs")))
        refs.extend(_strings(manifest.get("declared_metric_script_hash_refs")))
        for row in _rows(manifest, "metric_specs"):
            for key in (
                "metric_ref",
                "metric_hash_ref",
                "input_ref",
                ...
                if isinstance(value, str):
                    refs.append(value)
    for ref in refs:
        if _is_safe_public_ref(ref):
            paths.append(_public_ref_path(input_dir, ref))
    return paths


def _freshness_paths(input_dir: Path, *, include_negative: bool) -> list[Path]:
    """
    Return freshness paths for the organs research replication rubric artifact replay flow.
    ...
    return [
        ...
        *_execution_artifact_paths(source),
...
def _freshness_basis(input_dir: Path, *, include_negative: bool) -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.research_replication_rubric_artifact_replay._freshness_basis`
    into the payload shape expected by organs research replication rubric artifact replay.
...
def _fresh_replication_bundle_receipt(
    ...
    *,
    ...
    """
    Derive fresh replication bundle receipt without touching module import state.
    ...
    if payload.get("input_mode") != "exported_research_replication_bundle":
        return None
    stored_command = payload.get("command")
    if stored_command not in {command, _stored_receipt_command(command)}:
        return None
...
def validate_source_module_imports(
    ...
    *,
    ...
    """
    Serialize
    `microcosm_core.organs.research_replication_rubric_artifact_replay.validate_source_module_imports`
    into the payload shape expected by organs research replication rubric artifact replay.
    ...
    for row in _rows(manifest, "modules"):
        ...
        source_path, source_ref = _source_module_source_path(row, public_root=public_root)
        material_class = str(row.get("material_class") or "")
        expected_digest = _sha256_prefixed(row.get("sha256"))
        expected_source_digest = _sha256_prefixed(row.get("source_sha256"))
        expected_target_digest = _sha256_prefixed(row.get("target_sha256"))
        ...
        if material_class not in PUBLIC_SAFE_SOURCE_MODULE_CLASSES:
            findings.append(
                _finding(
                    ...
                    "Research replication may import public macro pattern provenance bodies and public Python control-plane bodies only.",
                    case_id="source_module_manifest_floor",
                    subject_id=module_id or target_ref or "source_module",
                    subject_kind="source_module",
                )
        ...
        if relation == "exact_copy":
            if not source_ref:
                findings.append(
                    _finding(
                        "REPLICATION_SOURCE_MODULE_SOURCE_REF_MISSING",
                        "Exact-copy source module rows must retain a source_ref for currentness verification.",
                        case_id="source_module_manifest_floor",
                        subject_id=module_id or target_ref or "source_module",
                        subject_kind="source_module",
                    )
                ...
                findings.append(
                    _finding(
                        "REPLICATION_SOURCE_MODULE_SOURCE_MISSING",
                        "Exact-copy source module source_ref must resolve to the live public source body.",
                        case_id="source_module_manifest_floor",
                        subject_id=source_ref or module_id or "source_module",
                        subject_kind="source_module",
                    )
                )
            if not expected_source_digest or not expected_target_digest:
                findings.append(
                    _finding(
                        "REPLICATION_SOURCE_MODULE_SOURCE_TARGET_DIGEST_MISSING",
                        "Exact-copy source module rows must declare source_sha256 and target_sha256.",
                        case_id="source_module_manifest_floor",
                        subject_id=module_id or target_ref or "source_module",
                        subject_kind="source_module",
                    )
        ...
        if expected_digest != actual_digest or (
            expected_target_digest and expected_target_digest != actual_digest
        ):
            findings.append(
                _finding(
        ...
        source_current = True
        source_target_exact_copy = True
        if relation == "exact_copy":
            source_current = bool(actual_source_digest) and (
                expected_source_digest == actual_source_digest
            )
            source_target_exact_copy = (
            ...
            if not source_current:
                findings.append(
                    _finding(
                        "REPLICATION_SOURCE_MODULE_SOURCE_DIGEST_MISMATCH",
                        "Exact-copy source module source_sha256 must match the live public source body.",
                        case_id="source_module_manifest_floor",
                        subject_id=source_ref or module_id or "source_module",
                        subject_kind="source_module",
                    )
            ...
            if not source_target_exact_copy:
                findings.append(
                    _finding(
                        ...
                        "Exact-copy source module target must be byte-identical to the live public source body.",
                        case_id="source_module_manifest_floor",
                        subject_id=target_ref or module_id or "source_module",
                        subject_kind="source_module",
                    )
...
def _finding(
    ...
    *,
    ...
    """
    Serialize `microcosm_core.organs.research_replication_rubric_artifact_replay._finding`
    into the payload shape expected by organs research replication rubric artifact replay.
...
def _record(
    ...
    *,
    ...
    """
    Record record for the organs research replication rubric artifact replay flow.
    ...
    observed[observed_case_id].add(code)


def _execute_metric_script(
    metric_payload: dict[str, Any],
    input_payload: dict[str, Any],
) -> tuple[dict[str, Any] | None, str | None]:
    """
    Produce the execute metric script value used by
    `microcosm_core.organs.research_replication_rubric_artifact_replay`.

    Inputs are `metric_payload` and `input_payload`; notable helpers are `get` and
    `is_integer`.
    """
    operation = str(metric_payload.get("operation") or "")
    input_field = str(metric_payload.get("input_field") or "")
    output_key = str(metric_payload.get("output_key") or "")
    rows = input_payload.get("rows")
    if operation != "sum_field" or not input_field or not output_key:
        return None, "REPLICATION_EXECUTION_METRIC_SPEC_INVALID"
    if not isinstance(rows, list):
        return None, "REPLICATION_EXECUTION_INPUT_INVALID"

    total: int | float = 0
    for item in rows:
        value = item.get(input_field) if isinstance(item, dict) else None
        if not isinstance(value, (int, float)):
            return None, "REPLICATION_EXECUTION_INPUT_INVALID"
        total += value
    if isinstance(total, float) and total.is_integer():
        total = int(total)
    return {output_key: total, "row_count": len(rows)}, None


def _declared_verdict_label_keys(row: dict[str, Any]) -> list[str]:
    """
    Produce the declared verdict label keys value used by
    `microcosm_core.organs.research_replication_rubric_artifact_replay`.
...
def _replay_cited_refs(
    ...
    """
    Serialize
    `microcosm_core.organs.research_replication_rubric_artifact_replay._replay_cited_refs`
    into the payload shape expected by organs research replication rubric artifact replay.
    ...
    return {
        "allowed_public_input_refs": {
            ...
            for ref in _strings(row.get("allowed_public_input_refs"))
        },
        "metric_script_refs": {
            ref
            for row in replay_rows
            for ref in _strings(row.get("metric_script_refs"))
        },
        "declared_artifact_hash_refs": {
            ref
            for row in replay_rows
            for ref in _strings(row.get("declared_artifact_hash_refs"))
        },
    }


def validate_execution_artifact_replay(
    input_dir: Path,
    *,
    replay_rows: list[dict[str, Any]],
) -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.research_replication_rubric_artifact_replay.validate_execution_artifact_replay`
    into the payload shape expected by organs research replication rubric artifact replay.
    ...
    source = Path(input_dir)
    public_root = _public_root_for_path(source)
    manifest_path = source / EXECUTION_ARTIFACT_MANIFEST_REF
    manifest_ref = _display(manifest_path, public_root=public_root)
    findings: list[dict[str, Any]] = []
    metric_results: list[dict[str, Any]] = []
    replay_refs = _replay_cited_refs(replay_rows)

    if not manifest_path.is_file():
        findings.append(
            _finding(
                "REPLICATION_EXECUTION_ARTIFACT_MANIFEST_MISSING",
                "Execution-backed research replication requires a public execution artifact manifest.",
                case_id="execution_artifact_replay",
                subject_id=manifest_ref,
                subject_kind="execution_artifact_manifest",
            )
        )
        return {
            "schema_version": "research_replication_execution_artifact_replay_v1",
            "status": "blocked",
            "verdict_source": "local_metric_script_execution_over_allowed_public_inputs",
            "declared_fixture_labels_used": False,
            ...
            "allowed_public_input_refs": [],
            "declared_artifact_hash_refs": [],
            "declared_public_input_hash_refs": [],
            "declared_metric_script_hash_refs": [],
            "cited_metric_script_refs": [],
            "cited_allowed_public_input_refs": [],
            "cited_declared_artifact_hash_refs": [],
            "metric_count": 0,
            "computed_output_refs": [],
            "computed_output_count": 0,
            "evidence_rung": "R2",
            "evidence_state": "blocked_replay",
            "rung_evidence": {
                "rank_rederived_from": [],
                "metric_script_executed_count": 0,
                "input_body_hash_verified_count": 0,
                "metric_body_hash_verified_count": 0,
                "computed_output_count": 0,
                "declared_fixture_labels_used": False,
            },
            "metric_results": [],
    ...
    if not isinstance(manifest, dict):
        manifest = {}
    allowed_input_refs = _strings(manifest.get("allowed_public_input_refs"))
    declared_artifact_hash_refs = _strings(manifest.get("declared_artifact_hash_refs"))
    declared_input_hash_refs = _strings(
        manifest.get("declared_public_input_hash_refs")
    )
    declared_metric_hash_refs = _strings(
        manifest.get("declared_metric_script_hash_refs")
    )
    metric_specs = _rows(manifest, "metric_specs")
    manifest_metric_refs = {
        str(row.get("metric_ref") or "") for row in metric_specs if row.get("metric_ref")
    }
    manifest_input_refs = {
        str(row.get("input_ref") or "") for row in metric_specs if row.get("input_ref")
    }
    manifest_hash_refs = {
        str(row.get("declared_hash_ref") or "")
        for row in metric_specs
        if row.get("declared_hash_ref")
    }
    manifest_input_hash_refs = {
        str(row.get("input_hash_ref") or "")
        for row in metric_specs
        if row.get("input_hash_ref")
    }
    manifest_metric_hash_refs = {
        str(row.get("metric_hash_ref") or "")
        for row in metric_specs
        if row.get("metric_hash_ref")
    }

    if manifest.get("body_in_receipt") is not False:
        findings.append(
            _finding(
                "REPLICATION_EXECUTION_BODY_IN_RECEIPT_FORBIDDEN",
                "Execution artifact bodies must stay in public artifact files, not receipts.",
                case_id="execution_artifact_replay",
                subject_id=manifest_ref,
                subject_kind="execution_artifact_manifest",
            )
    ...
    if manifest.get("provider_calls_authorized") is not False:
        findings.append(
            _finding(
                "REPLICATION_EXECUTION_PROVIDER_CALL_FORBIDDEN",
                "Execution artifact replay must be local and public-safe; provider calls are not authorized.",
                case_id="execution_artifact_replay",
                subject_id=manifest_ref,
                subject_kind="execution_artifact_manifest",
            )
        )
    if manifest.get("private_body_refs") not in ([], None):
        findings.append(
            _finding(
                "REPLICATION_EXECUTION_PRIVATE_BODY_REF_FORBIDDEN",
                "Execution artifact replay cannot reference private paper, data, provider, or hidden rubric bodies.",
                case_id="execution_artifact_replay",
                subject_id=manifest_ref,
                subject_kind="execution_artifact_manifest",
            )
    ...
    if not metric_specs:
        findings.append(
            _finding(
                "REPLICATION_EXECUTION_METRIC_SPEC_MISSING",
                "Execution artifact manifest must declare at least one metric spec.",
                case_id="execution_artifact_replay",
                subject_id=manifest_ref,
                subject_kind="execution_artifact_manifest",
            )
        )
    for metric_ref in sorted(manifest_metric_refs - replay_refs["metric_script_refs"]):
        findings.append(
            _finding(
                "REPLICATION_EXECUTION_METRIC_REF_NOT_CITED",
                "Executed metric script refs must be cited by research_replays.metric_script_refs.",
                case_id="execution_artifact_replay",
                subject_id=metric_ref,
                subject_kind="metric_spec",
            )
        )
    for metric_ref in sorted(replay_refs["metric_script_refs"] - manifest_metric_refs):
        findings.append(
            _finding(
                "REPLICATION_EXECUTION_METRIC_REF_NOT_REPLAYED",
                "Research replay metric_script_refs must be executed by the public execution artifact manifest.",
                case_id="execution_artifact_replay",
                subject_id=metric_ref,
                subject_kind="metric_spec",
            )
        )
    for input_ref in sorted(manifest_input_refs - replay_refs["allowed_public_input_refs"]):
        findings.append(
            _finding(
                "REPLICATION_EXECUTION_INPUT_REF_NOT_CITED",
                "Executed metric inputs must be cited by research_replays.allowed_public_input_refs.",
                case_id="execution_artifact_replay",
                subject_id=input_ref,
                subject_kind="metric_input",
            )
        )
    for input_ref in sorted(replay_refs["allowed_public_input_refs"] - manifest_input_refs):
        findings.append(
            _finding(
                "REPLICATION_EXECUTION_INPUT_REF_NOT_REPLAYED",
                "Research replay allowed_public_input_refs must be consumed by the public execution artifact manifest.",
                case_id="execution_artifact_replay",
                subject_id=input_ref,
                subject_kind="metric_input",
            )
        )
    for hash_ref in sorted(manifest_hash_refs - replay_refs["declared_artifact_hash_refs"]):
        findings.append(
            _finding(
                "REPLICATION_EXECUTION_HASH_REF_NOT_CITED",
                "Verified artifact hashes must be cited by research_replays.declared_artifact_hash_refs.",
                case_id="execution_artifact_replay",
                subject_id=hash_ref,
                subject_kind="artifact_hash",
            )
        )
    for hash_ref in sorted(
        replay_refs["declared_artifact_hash_refs"] - manifest_hash_refs
    ):
        findings.append(
            _finding(
                "REPLICATION_EXECUTION_HASH_REF_NOT_REPLAYED",
                "Research replay declared_artifact_hash_refs must be verified by the public execution artifact manifest.",
                case_id="execution_artifact_replay",
                subject_id=hash_ref,
                subject_kind="artifact_hash",
            )
        )
    for hash_ref in sorted(manifest_input_hash_refs - set(declared_input_hash_refs)):
        findings.append(
            _finding(
                "REPLICATION_EXECUTION_INPUT_HASH_REF_UNDECLARED",
                "Metric input body hash refs must belong to the declared public input-hash roster.",
                case_id="execution_artifact_replay",
                subject_id=hash_ref,
                subject_kind="metric_input_hash",
            )
        )
    for hash_ref in sorted(manifest_metric_hash_refs - set(declared_metric_hash_refs)):
        findings.append(
            _finding(
                "REPLICATION_EXECUTION_METRIC_HASH_REF_UNDECLARED",
                "Metric script body hash refs must belong to the declared metric-script hash roster.",
                case_id="execution_artifact_replay",
                subject_id=hash_ref,
                subject_kind="metric_script_hash",
            )
        )

    def path_for(ref: str, *, subject_kind: str, code: str) -> Path | None:
        """
        Compute path for from `ref`, `subject_kind`, and `code`.

        Inputs are `ref`, `subject_kind`, and `code`; notable helpers are
        `_public_ref_path`, `_is_safe_public_ref`, `append`, `is_file`, and 1 more.
        """
        if not _is_safe_public_ref(ref):
            findings.append(
                _finding(
                    code,
                    "Execution artifact refs must be non-empty public relative paths inside the bundle.",
                    case_id="execution_artifact_replay",
                    subject_id=ref or subject_kind,
                    subject_kind=subject_kind,
                )
            )
            return None
        path = _public_ref_path(source, ref)
        if not path.is_file():
            findings.append(
                _finding(
                    code,
                    "Execution artifact ref must resolve to a public file inside the bundle.",
                    case_id="execution_artifact_replay",
                    subject_id=ref,
                    subject_kind=subject_kind,
                )
    ...
    for row in metric_specs:
        metric_id = str(row.get("metric_id") or "execution_metric")
        metric_ref = str(row.get("metric_ref") or "")
        metric_hash_ref = str(row.get("metric_hash_ref") or "")
        input_ref = str(row.get("input_ref") or "")
        input_hash_ref = str(row.get("input_hash_ref") or "")
        output_ref = str(row.get("output_ref") or "")
        declared_hash_ref = str(row.get("declared_hash_ref") or "")
        if input_ref not in allowed_input_refs:
            findings.append(
                _finding(
                    "REPLICATION_EXECUTION_INPUT_NOT_ALLOWED",
                    "Metric input ref must be listed in allowed_public_input_refs.",
                    case_id="execution_artifact_replay",
                    subject_id=input_ref or metric_id,
                    subject_kind="execution_metric",
                )
            )
        if metric_ref not in replay_refs["metric_script_refs"]:
            findings.append(
                _finding(
                    "REPLICATION_EXECUTION_METRIC_REF_NOT_CITED",
                    "Metric script ref must be cited by at least one research replay row.",
                    case_id="execution_artifact_replay",
                    subject_id=metric_ref or metric_id,
                    subject_kind="metric_spec",
                )
            )
        if input_ref not in replay_refs["allowed_public_input_refs"]:
            findings.append(
                _finding(
                    "REPLICATION_EXECUTION_INPUT_REF_NOT_CITED",
                    "Metric input ref must be cited by at least one research replay row.",
                    case_id="execution_artifact_replay",
                    subject_id=input_ref or metric_id,
                    subject_kind="metric_input",
                )
            )
        if declared_hash_ref not in replay_refs["declared_artifact_hash_refs"]:
            findings.append(
                _finding(
                    "REPLICATION_EXECUTION_HASH_REF_NOT_CITED",
                    "Declared hash ref must be cited by at least one research replay row.",
                    case_id="execution_artifact_replay",
                    subject_id=declared_hash_ref or metric_id,
                    subject_kind="artifact_hash",
                )
            )
        if declared_hash_ref not in declared_artifact_hash_refs:
            findings.append(
                _finding(
                    "REPLICATION_UNDECLARED_ARTIFACT_HASH_REF",
                    "Computed output hash ref must belong to the declared public artifact-hash roster.",
                    case_id="execution_artifact_replay",
                    subject_id=declared_hash_ref or metric_id,
                    subject_kind="execution_metric",
                )
            )
        if input_hash_ref not in declared_input_hash_refs:
            findings.append(
                _finding(
                    "REPLICATION_EXECUTION_INPUT_HASH_REF_UNDECLARED",
                    "Metric input body hash ref must belong to declared_public_input_hash_refs.",
                    case_id="execution_artifact_replay",
                    subject_id=input_hash_ref or metric_id,
                    subject_kind="metric_input_hash",
                )
            )
        if metric_hash_ref not in declared_metric_hash_refs:
            findings.append(
                _finding(
                    "REPLICATION_EXECUTION_METRIC_HASH_REF_UNDECLARED",
                    "Metric script body hash ref must belong to declared_metric_script_hash_refs.",
                    case_id="execution_artifact_replay",
                    subject_id=metric_hash_ref or metric_id,
                    subject_kind="metric_script_hash",
                )
            )

        metric_path = path_for(
            metric_ref,
            subject_kind="metric_spec",
            code="REPLICATION_EXECUTION_METRIC_SPEC_MISSING",
        )
        metric_hash_path = path_for(
            metric_hash_ref,
            subject_kind="metric_script_hash",
            code="REPLICATION_EXECUTION_METRIC_HASH_MISSING",
        )
        input_path = path_for(
            input_ref,
            subject_kind="metric_input",
            code="REPLICATION_EXECUTION_INPUT_MISSING",
        )
        input_hash_path = path_for(
            input_hash_ref,
            subject_kind="metric_input_hash",
            code="REPLICATION_EXECUTION_INPUT_HASH_MISSING",
        )
        output_path = path_for(
            output_ref,
            subject_kind="metric_output",
            code="REPLICATION_EXECUTION_OUTPUT_MISSING",
        )
        hash_path = path_for(
            declared_hash_ref,
            subject_kind="artifact_hash",
            code="REPLICATION_EXECUTION_ARTIFACT_HASH_MISSING",
        )
        if None in (
            metric_path,
            metric_hash_path,
        ...
        metric_payload = read_json_strict(metric_path)
        metric_hash_payload = read_json_strict(metric_hash_path)
        input_payload = read_json_strict(input_path)
        input_hash_payload = read_json_strict(input_hash_path)
        output_payload = read_json_strict(output_path)
        hash_payload = read_json_strict(hash_path)
        if not isinstance(metric_payload, dict):
            metric_payload = {}
        if not isinstance(metric_hash_payload, dict):
            metric_hash_payload = {}
        if not isinstance(input_payload, dict):
            input_payload = {}
        if not isinstance(input_hash_payload, dict):
            input_hash_payload = {}
        if not isinstance(output_payload, dict):
            output_payload = {}
        if not isinstance(hash_payload, dict):
            hash_payload = {}
        metric_file_hash = _sha256(metric_path)
        metric_hash_file_hash = _sha256(metric_hash_path)
        expected_metric_file_hash = str(metric_hash_payload.get("sha256") or "")
        metric_body_hash_matches_declared = (
            metric_hash_payload.get("artifact_ref") == metric_ref
            and expected_metric_file_hash == metric_file_hash
        )
        if not metric_body_hash_matches_declared:
            findings.append(
                _finding(
                    "REPLICATION_EXECUTION_METRIC_BODY_HASH_MISMATCH",
                    "Metric script body hash must name the metric ref and match the local public metric body.",
                    case_id="execution_artifact_replay",
                    subject_id=metric_hash_ref or metric_ref,
                    subject_kind="metric_script_hash",
                )
            )
        input_file_hash = _sha256(input_path)
        input_hash_file_hash = _sha256(input_hash_path)
        expected_input_file_hash = str(input_hash_payload.get("sha256") or "")
        input_body_hash_matches_declared = (
            input_hash_payload.get("artifact_ref") == input_ref
            and expected_input_file_hash == input_file_hash
        )
        if not input_body_hash_matches_declared:
            findings.append(
                _finding(
                    "REPLICATION_EXECUTION_INPUT_BODY_HASH_MISMATCH",
                    "Allowed public input body hash must name the input ref and match the local public input body.",
                    case_id="execution_artifact_replay",
                    subject_id=input_hash_ref or input_ref,
                    subject_kind="metric_input_hash",
                )
            )
        input_metric_hashes_match_declared = (
            metric_body_hash_matches_declared and input_body_hash_matches_declared
        )
        computed_output, execution_error = _execute_metric_script(
            metric_payload,
            input_payload,
        )
        if execution_error == "REPLICATION_EXECUTION_METRIC_SPEC_INVALID":
            findings.append(
                _finding(
                    "REPLICATION_EXECUTION_METRIC_SPEC_INVALID",
                    "Only executable public sum_field metric scripts with input_field and output_key are supported.",
                    case_id="execution_artifact_replay",
                    subject_id=metric_id,
                    subject_kind="metric_spec",
                )
            )
        elif execution_error == "REPLICATION_EXECUTION_INPUT_INVALID":
            findings.append(
                _finding(
                    "REPLICATION_EXECUTION_INPUT_INVALID",
                    "Metric input must contain public rows exposing the metric script input_field.",
                    case_id="execution_artifact_replay",
                    subject_id=input_ref,
                    subject_kind="metric_input",
                )
            )
        if computed_output is not None and output_payload != computed_output:
            findings.append(
                _finding(
                    "REPLICATION_EXECUTION_METRIC_OUTPUT_MISMATCH",
                    "Declared output artifact must equal the metric script output produced from the allowed public input.",
                    case_id="execution_artifact_replay",
                    subject_id=output_ref,
                    subject_kind="metric_output",
                )
            )

        produced_output_hash = (
            _json_file_sha256_for_payload(computed_output)
            if computed_output is not None
            else None
        )
        expected_hash = str(hash_payload.get("sha256") or "")
        actual_hash = _sha256(output_path)
        if (
            hash_payload.get("artifact_ref") != output_ref
            or expected_hash != actual_hash
            or (
                produced_output_hash is not None
                and expected_hash != produced_output_hash
            )
        ):
            findings.append(
                _finding(
                    "REPLICATION_EXECUTION_ARTIFACT_HASH_MISMATCH",
                    "Declared artifact hash must name the output artifact and match the metric script output produced from the allowed public input.",
                    case_id="execution_artifact_replay",
                    subject_id=declared_hash_ref or output_ref,
                    subject_kind="artifact_hash",
                )
            )

        declared_hash_matches_produced_output = (
            produced_output_hash is not None and expected_hash == produced_output_hash
        )
        declared_output_matches_produced_output = (
            computed_output is not None and output_payload == computed_output
        )
        if input_ref not in allowed_input_refs:
            declared_input_refs: list[str] = []
        else:
            declared_input_refs = [input_ref]

        metric_results.append(
            {
                "metric_id": metric_id,
                "metric_ref": metric_ref,
                "metric_file_sha256": metric_file_hash,
                "metric_hash_ref": metric_hash_ref,
                "metric_hash_file_sha256": metric_hash_file_hash,
                "declared_metric_file_sha256": expected_metric_file_hash,
                "metric_body_hash_matches_declared": (
                    metric_body_hash_matches_declared
                ),
                "metric_payload_canonical_sha256": _canonical_payload_sha256(
                    metric_payload
                ),
                "metric_script_refs": [metric_ref],
                "input_ref": input_ref,
                "input_file_sha256": input_file_hash,
                "input_hash_ref": input_hash_ref,
                "input_hash_file_sha256": input_hash_file_hash,
                "declared_input_file_sha256": expected_input_file_hash,
                "input_body_hash_matches_declared": (
                    input_body_hash_matches_declared
                ),
                "input_payload_canonical_sha256": _canonical_payload_sha256(
                    input_payload
                ),
                "allowed_public_input_refs": declared_input_refs,
                "output_ref": output_ref,
                "declared_hash_ref": declared_hash_ref,
                "declared_hash_file_sha256": _sha256(hash_path),
                "execution_mode": "local_metric_script_over_allowed_public_input",
                "metric_script_executed": computed_output is not None,
                "computed_output_ref": output_ref,
                "computed_output_canonical_sha256": _canonical_payload_sha256(
                    computed_output
                )
                if computed_output is not None
                else None,
                "produced_output_payload": computed_output,
                "produced_output_file_sha256": produced_output_hash,
                "output_payload_canonical_sha256": _canonical_payload_sha256(
                    output_payload
                ),
                "declared_output_matches_produced_output": (
                    declared_output_matches_produced_output
                ...
                "declared_output_file_sha256": expected_hash,
                "actual_output_file_sha256": actual_hash,
                "input_metric_hashes_match_declared": (
                    input_metric_hashes_match_declared
                ),
                "body_in_receipt": False,
    ...
    computed_output_refs = sorted(
        {
            str(row.get("computed_output_ref") or "")
            for row in metric_results
            if row.get("computed_output_ref")
        }
    )
    evidence_ready = bool(metric_results) and not findings
    input_hash_match_count = sum(
        1 for row in metric_results if row.get("input_body_hash_matches_declared")
    )
    metric_hash_match_count = sum(
        1 for row in metric_results if row.get("metric_body_hash_matches_declared")
    )
    return {
        "schema_version": "research_replication_execution_artifact_replay_v1",
        "status": PASS if metric_results and not findings else "blocked",
        "verdict_source": "local_metric_script_execution_over_allowed_public_inputs",
        "declared_fixture_labels_used": False,
        "hash_verification_basis": (
            "input_metric_body_hash_refs_plus_output_artifact_ref_and_sha256"
        ...
        "manifest_ref": manifest_ref,
        "allowed_public_input_refs": allowed_input_refs,
        "declared_artifact_hash_refs": declared_artifact_hash_refs,
        "declared_public_input_hash_refs": declared_input_hash_refs,
        "declared_metric_script_hash_refs": declared_metric_hash_refs,
        "cited_metric_script_refs": sorted(replay_refs["metric_script_refs"]),
        "cited_allowed_public_input_refs": sorted(
            replay_refs["allowed_public_input_refs"]
        ),
        "cited_declared_artifact_hash_refs": sorted(
            replay_refs["declared_artifact_hash_refs"]
        ),
        "metric_count": len(metric_results),
        "computed_output_refs": computed_output_refs,
        "computed_output_count": len(computed_output_refs),
        "evidence_rung": "R3" if evidence_ready else "R2",
        "evidence_state": (
            "real_metric_hash_replay" if evidence_ready else "blocked_replay"
        ),
        "rung_evidence": {
            "rank_rederived_from": [
            ...
            "metric_script_executed_count": sum(
                1 for row in metric_results if row.get("metric_script_executed")
            ),
            "input_body_hash_verified_count": input_hash_match_count,
            "metric_body_hash_verified_count": metric_hash_match_count,
            "computed_output_count": len(computed_output_refs),
            "declared_fixture_labels_used": False,
        },
        "metric_results": sorted(metric_results, key=lambda row: row["metric_id"]),
        "findings": findings,
        "observed_negative_cases": {},
        "body_in_receipt": False,
    }
...
def _merge_findings(*results: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    `microcosm_core.organs.research_replication_rubric_artifact_replay._merge_findings`.
...
def validate_projection_protocol(
    ...
    *,
    ...
    """
    Serialize
    `microcosm_core.organs.research_replication_rubric_artifact_replay.validate_projection_protocol`
    into the payload shape expected by organs research replication rubric artifact replay.
    ...
    base_body_import_verification = protocol.get("body_import_verification", {})
    if not isinstance(base_body_import_verification, dict):
        base_body_import_verification = {}
    findings: list[dict[str, Any]] = []
    if (
        ...
        or base_body_import_verification.get("verification_mode")
...
def validate_replication_policy(payload: object) -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.research_replication_rubric_artifact_replay.validate_replication_policy`
    into the payload shape expected by organs research replication rubric artifact replay.
...
def _has_required_replay_fields(row: dict[str, Any]) -> bool:
    """
    Return whether has required replay fields holds for the organs research replication
    rubric artifact replay flow.
...
def _validate_replay_row(
    ...
    *,
    ...
    """
    Validate whether validate replay row holds for the organs research replication rubric
    artifact replay flow.

    The result is derived from `row`, `case_id`, `observed`, `findings`, and
    `derive_observed_case_id` with `_has_required_replay_fields`, `_record`, `_strings`,
    `get`, and 1 more; failing evidence is returned or raised exactly where the body says
    so.
    """
    ...
    if not _has_required_replay_fields(row):
        _record(
            ...
            derive_observed_case_id=derive_observed_case_id,
        )
...
def validate_research_replays(
    ...
    """
    Serialize
    `microcosm_core.organs.research_replication_rubric_artifact_replay.validate_research_replays`
    into the payload shape expected by organs research replication rubric artifact replay.
    ...
    ignored_declared_label_cases: list[dict[str, Any]] = []
    for case_id, payload in negative_payloads.items():
        row = payload if isinstance(payload, dict) else {}
        ignored_keys = _declared_verdict_label_keys(row)
        if ignored_keys:
            ignored_declared_label_cases.append(
                {"case_id": case_id, "ignored_keys": ignored_keys}
            )
        _validate_replay_row(
            row,
            case_id=case_id,
    ...
    return {
        ...
        "negative_case_verdict_basis": {
            "verdict_source": "semantic_replay_row_fields",
            "declared_fixture_labels_used": False,
            "coverage_case_id_source": "derived_error_code_semantics_not_fixture_filename",
            "ignored_declared_label_cases": ignored_declared_label_cases,
        },
    }
...
def _build_result(
    ...
    *,
    ...
    """
    Serialize
    `microcosm_core.organs.research_replication_rubric_artifact_replay._build_result` into
    the payload shape expected by organs research replication rubric artifact replay.
    ...
    secret_scan = scan_paths(
        [
            ...
            *_execution_artifact_paths(input_dir),
        ],
    ...
    execution_artifact_replay = validate_execution_artifact_replay(
        input_dir,
        replay_rows=research_replays["research_replays"],
    )
    projection = validate_projection_protocol(
        payloads["projection_protocol"],
        public_root=public_root,
        public_trace=public_agent_execution_trace,
        source_imports=source_imports,
    )
    ...
    status = (
        ...
        and execution_artifact_replay["status"] == PASS
        else "blocked"
    )
    return {
        ...
        "semantic_negative_case_evaluator_used": include_negative,
        "negative_case_verdict_basis": research_replays[
            "negative_case_verdict_basis"
        ],
        ...
        "execution_artifact_replay": execution_artifact_replay,
        "execution_artifact_replay_status": execution_artifact_replay["status"],
        "replay_evidence_rung": execution_artifact_replay["evidence_rung"],
        "replay_evidence_state": execution_artifact_replay["evidence_state"],
        "execution_artifact_metric_count": execution_artifact_replay["metric_count"],
        "execution_artifact_computed_output_count": execution_artifact_replay[
            "computed_output_count"
        ],
        "execution_artifact_declared_hash_ref_count": len(
            execution_artifact_replay["declared_artifact_hash_refs"]
        ),
...
def _board_from_result(result: dict[str, Any]) -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.research_replication_rubric_artifact_replay._board_from_result`
    into the payload shape expected by organs research replication rubric artifact replay.
    ...
    return {
        ...
        "execution_artifact_replay": result["execution_artifact_replay"],
        "execution_artifact_replay_status": result["execution_artifact_replay_status"],
        "execution_artifact_metric_count": result["execution_artifact_metric_count"],
        "execution_artifact_computed_output_count": result[
            "execution_artifact_computed_output_count"
        ],
        "execution_artifact_declared_hash_ref_count": result[
            "execution_artifact_declared_hash_ref_count"
        ],
...
def _write_receipts(
    ...
    *,
    ...
    """
    Serialize
    `microcosm_core.organs.research_replication_rubric_artifact_replay._write_receipts` into
    the payload shape expected by organs research replication rubric artifact replay.
...
def run(
    ...
    *,
    ...
    """
    Return run for the organs research replication rubric artifact replay flow.
...
def run_replication_bundle(
    ...
    *,
    ...
    """
    Derive run replication bundle without touching module import state.
...
def result_card(result: dict[str, Any]) -> dict[str, Any]:
    """
    Return result card for the organs research replication rubric artifact replay flow.
    ...
    execution_replay = result.get("execution_artifact_replay")
    replay = execution_replay if isinstance(execution_replay, dict) else {}
    card = {
        ...
        "execution_artifact_replay": {
            "status": result.get("execution_artifact_replay_status"),
            "verdict_source": replay.get("verdict_source"),
            "evidence_rung": replay.get("evidence_rung"),
            "evidence_state": replay.get("evidence_state"),
            "hash_verification_basis": replay.get("hash_verification_basis"),
            "declared_fixture_labels_used": replay.get("declared_fixture_labels_used")
            is True,
            "metric_count": result.get("execution_artifact_metric_count"),
            "computed_output_count": result.get(
                "execution_artifact_computed_output_count"
            ),
            "declared_artifact_hash_ref_count": result.get(
                "execution_artifact_declared_hash_ref_count"
    ...
    normalized = normalize_public_receipt_paths(card)
    return normalized if isinstance(normalized, dict) else card


def _parser() -> argparse.ArgumentParser:
    """
    Register CLI syntax for
    `microcosm_core.organs.research_replication_rubric_artifact_replay._parser`.
...
def main(argv: list[str] | None = None) -> int:
    """
    Run the `microcosm_core.organs.research_replication_rubric_artifact_replay` command-line
    entry point.
