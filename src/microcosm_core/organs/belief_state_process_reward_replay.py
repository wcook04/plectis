"""
Implements organs belief state process reward replay for the public Plectis package.

Callers enter through `validate_projection_protocol`, `validate_reward_policy`,
`validate_task_episodes`, `validate_belief_states`, `validate_verifier_feedback`,
`validate_reward_events`, and 8 more; constants such as `ORGAN_ID`, `FIXTURE_ID`,
`VALIDATOR_ID`, `RESULT_NAME`, and 22 more pin local fixture names; dependencies include
...
def _display(path: Path, *, public_root: Path) -> str:
    """
    Return display for the organs belief state process reward replay flow.
...
def _rows(payload: object, key: str) -> list[dict[str, Any]]:
    ...
    `microcosm_core.organs.belief_state_process_reward_replay._rows` from `payload[key]`.
...
def _strings(value: object) -> list[str]:
    ...
    `microcosm_core.organs.belief_state_process_reward_replay._strings`.
...
def _is_numeric_value(value: object) -> bool:
    """
    Return whether is numeric value holds for the organs belief state process reward replay
    flow.
...
def _input_paths(input_dir: Path, *, include_negative: bool) -> list[Path]:
    """
    Return input paths for the organs belief state process reward replay flow.
...
def _sha256(path: Path) -> str:
    ...
    `microcosm_core.organs.belief_state_process_reward_replay._sha256`.
...
def _repo_root_for_public_refactor(public_root: Path) -> Path | None:
    """
    Produce the repo root for public refactor value used by
    `microcosm_core.organs.belief_state_process_reward_replay`.
...
def _body_import_verification(
    *,
    ...
    """
    Serialize
    `microcosm_core.organs.belief_state_process_reward_replay._body_import_verification`
    into the payload shape expected by organs belief state process reward replay.
...
def _source_module_authority_candidates(public_root: Path) -> list[Path]:
    """
    Produce the source module authority candidates value used by
    `microcosm_core.organs.belief_state_process_reward_replay`.

    Inputs are `public_root`; notable helpers are `resolve`, `extend`, `Path`, `append`, and
    2 more.
    """
    candidates = [
...
def _source_module_authority_path(source_ref: str, *, public_root: Path) -> Path | None:
    """
    Derive source module authority path without touching module import state.

    Inputs are `source_ref` and `public_root`; notable helpers are `removeprefix`,
    `_source_module_authority_candidates`, `is_absolute`, `is_file`, and 1 more.
    """
...
def _source_module_authority_paths(input_dir: Path, *, public_root: Path) -> list[Path]:
    """
    Produce the source module authority paths value used by
    `microcosm_core.organs.belief_state_process_reward_replay`.

    Inputs are `input_dir` and `public_root`; notable helpers are
    `_source_module_manifest_path`, `_rows`, `is_file`, `read_json_strict`, and 7 more.
    """
    manifest_path = _source_module_manifest_path(input_dir)
    if not manifest_path.is_file():
        return []
...
def _scan_paths_for_input(input_dir: Path, *, include_negative: bool) -> list[Path]:
    """
    Scan whether scan paths for input holds for the organs belief state process reward
    replay flow.
...
def _freshness_paths(input_dir: Path, *, include_negative: bool) -> list[Path]:
    ...
    `_public_root_for_path`, `resolve`, `_input_paths`, and 2 more.
    """
...
def _freshness_basis(input_dir: Path, *, include_negative: bool) -> dict[str, Any]:
    """
    Serialize `microcosm_core.organs.belief_state_process_reward_replay._freshness_basis`
    into the payload shape expected by organs belief state process reward replay.
...
def _fresh_bundle_receipt(input_dir: Path, out_dir: Path) -> dict[str, Any] | None:
    """
    Derive fresh bundle receipt without touching module import state.
...
def _load_payloads(input_dir: Path, *, include_negative: bool) -> dict[str, Any]:
    """
    Load load payloads for `microcosm_core.organs.belief_state_process_reward_replay`.
...
def _finding(
    ...
    *,
    ...
    """
    Serialize `microcosm_core.organs.belief_state_process_reward_replay._finding` into the
    payload shape expected by organs belief state process reward replay.
...
def _record(
    ...
    *,
    ...
    """
    Record record for the organs belief state process reward replay flow.
...
def _merge_observed(*results: dict[str, Any]) -> dict[str, list[str]]:
    """
    Return merge observed for `microcosm_core.organs.belief_state_process_reward_replay`.
...
def _merge_findings(*results: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    `microcosm_core.organs.belief_state_process_reward_replay._merge_findings`.
...
def _source_module_manifest_result(
    ...
    *,
    ...
    """
    Serialize
    `microcosm_core.organs.belief_state_process_reward_replay._source_module_manifest_result`
    into the payload shape expected by organs belief state process reward replay.
...
def _source_open_body_import_summary(
    ...
    """
    Serialize
    `microcosm_core.organs.belief_state_process_reward_replay._source_open_body_import_summary`
    into the payload shape expected by organs belief state process reward replay.
...
def _has_forbidden_key(row: dict[str, Any]) -> bool:
    """
    Return whether has forbidden key holds for the organs belief state process reward replay
    flow.
...
def _negative_rows(payloads: dict[str, object]) -> list[dict[str, Any]]:
    """
    Compute negative rows from `payloads`.

    Inputs are `payloads`; notable helpers are `values`, `_rows`, `extend`, and `append`.
    """
    rows: list[dict[str, Any]] = []
...
def validate_projection_protocol(payload: object) -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.belief_state_process_reward_replay.validate_projection_protocol`
    into the payload shape expected by organs belief state process reward replay.
...
def validate_reward_policy(payload: object) -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.belief_state_process_reward_replay.validate_reward_policy` into
    the payload shape expected by organs belief state process reward replay.
...
def validate_task_episodes(payload: object) -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.belief_state_process_reward_replay.validate_task_episodes` into
    the payload shape expected by organs belief state process reward replay.
...
def validate_belief_states(payload: object) -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.belief_state_process_reward_replay.validate_belief_states` into
    the payload shape expected by organs belief state process reward replay.
...
def validate_verifier_feedback(payload: object) -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.belief_state_process_reward_replay.validate_verifier_feedback`
    into the payload shape expected by organs belief state process reward replay.
...
def validate_reward_events(payload: object) -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.belief_state_process_reward_replay.validate_reward_events` into
    the payload shape expected by organs belief state process reward replay.
...
def validate_trajectory_groups(payload: object) -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.belief_state_process_reward_replay.validate_trajectory_groups`
    into the payload shape expected by organs belief state process reward replay.
...
def validate_cold_replay(payload: object) -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.belief_state_process_reward_replay.validate_cold_replay` into the
    payload shape expected by organs belief state process reward replay.
...
def _index_by_id(rows: list[dict[str, Any]], key: str) -> dict[str, dict[str, Any]]:
    """
    Return index by ID for the organs belief state process reward replay flow.
...
def validate_semantic_recompute(payloads: dict[str, object]) -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.belief_state_process_reward_replay.validate_semantic_recompute`
    into the payload shape expected by organs belief state process reward replay.
    ...
    def add_reason(reasons: list[str], reason: str) -> None:
        """
        Add add reason for the organs belief state process reward replay flow.
...
def validate_negative_cases(payloads: dict[str, object]) -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.belief_state_process_reward_replay.validate_negative_cases` into
    the payload shape expected by organs belief state process reward replay.
...
def _build_result(
    ...
    *,
    ...
    """
    Serialize `microcosm_core.organs.belief_state_process_reward_replay._build_result` into
    the payload shape expected by organs belief state process reward replay.
...
def _board_from_result(result: dict[str, Any]) -> dict[str, Any]:
    """
    Serialize `microcosm_core.organs.belief_state_process_reward_replay._board_from_result`
    into the payload shape expected by organs belief state process reward replay.
...
def _write_receipts(
    ...
    *,
    ...
    """
    Serialize `microcosm_core.organs.belief_state_process_reward_replay._write_receipts`
    into the payload shape expected by organs belief state process reward replay.
...
def run(
    ...
    *,
    ...
    """
    Return run for `microcosm_core.organs.belief_state_process_reward_replay`.
...
def run_reward_bundle(
    ...
    *,
    ...
    """
    Derive run reward bundle without touching module import state.
...
def result_card(result: dict[str, Any]) -> dict[str, Any]:
    """
    Serialize `microcosm_core.organs.belief_state_process_reward_replay.result_card` into
    the payload shape expected by organs belief state process reward replay.
...
def _parser() -> argparse.ArgumentParser:
    ...
    `microcosm_core.organs.belief_state_process_reward_replay._parser`.
...
def main(argv: list[str] | None = None) -> int:
    """
    Run the `microcosm_core.organs.belief_state_process_reward_replay` command-line entry
    point.
