"""
Implements projections route candidate read model for the public Plectis package.

Callers enter through `build_route_candidate_read_model`,
`validate_route_candidate_read_model`, `compile_paths`, and `main`; constants such as
`SCHEMA`, `VALIDATION_SCHEMA`, `DEFAULT_OUT_NAME`, `DEFAULT_RECEIPT_NAME`, and 7 more pin
local fixture names; dependencies include `argparse`, `json`, `pathlib`, `typing`, and 1
more. Projection code here turns source-side records into JSON-safe views for public
inspection.
"""
...
def _as_dict(value: Any) -> dict[str, Any]:
    """
    Return as dict for `microcosm_core.projections.route_candidate_read_model`.
...
def _as_list(value: Any) -> list[Any]:
    """
    Return as list for `microcosm_core.projections.route_candidate_read_model`.
...
def _strings(value: Any) -> list[str]:
    ...
    `microcosm_core.projections.route_candidate_read_model._strings`.
...
def _rows(payload: dict[str, Any], key: str) -> list[dict[str, Any]]:
    """
    Return dictionary rows for `microcosm_core.projections.route_candidate_read_model._rows`
    from `payload[key]`.
...
def _load_optional_json(path: Path) -> dict[str, Any]:
    """
    Serialize `microcosm_core.projections.route_candidate_read_model._load_optional_json`
    into the payload shape expected by projections route candidate read model.
...
def _public_microcosm_command(command: str) -> str:
    """
    Produce the public microcosm command value used by
    `microcosm_core.projections.route_candidate_read_model`.

    Inputs are `command`; notable helpers are `split`, `startswith`, `replace`, `join`, and
    1 more.
    """
    parts = command.split()
...
def _by_organ_id(rows: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
    """
    Return by organ ID for the projections route candidate read model flow.

    Inputs are `rows`; notable helpers are `strip` and `get`.
    """
...
def _route_organ_ids(row: dict[str, Any]) -> list[str]:
    """
    Derive route organ IDs without touching module import state.

    Inputs are `row`; notable helpers are `_as_list`, `strip`, `get`, and `append`.
    """
    organ_ids: list[str] = []
...
def _route_index(routes_payload: dict[str, Any]) -> dict[str, dict[str, Any]]:
    """
    Serialize `microcosm_core.projections.route_candidate_read_model._route_index` into the
    payload shape expected by projections route candidate read model.
...
def _task_route_ref(route_row: dict[str, Any]) -> str:
    """
    Derive task route ref without touching module import state.

    Inputs are `route_row`; notable helpers are `strip` and `get`.
    """
...
def _evidence_profiles(payload: dict[str, Any]) -> dict[str, dict[str, Any]]:
    """
    Serialize `microcosm_core.projections.route_candidate_read_model._evidence_profiles`
    into the payload shape expected by projections route candidate read model.
...
def _source_module_summary(receipt: dict[str, Any]) -> dict[str, Any]:
    """
    Serialize `microcosm_core.projections.route_candidate_read_model._source_module_summary`
    into the payload shape expected by projections route candidate read model.
...
def _trace_repair_summary(receipt: dict[str, Any]) -> dict[str, Any]:
    """
    Serialize `microcosm_core.projections.route_candidate_read_model._trace_repair_summary`
    into the payload shape expected by projections route candidate read model.
...
def _observability_summary(receipt: dict[str, Any]) -> dict[str, Any]:
    """
    Serialize `microcosm_core.projections.route_candidate_read_model._observability_summary`
    into the payload shape expected by projections route candidate read model.
...
def _score_candidate(
    ...
    *,
    ...
    """
    Compute score candidate from `registry_row`, `route_row`, `receipt_count`,
    `source_module_count`, and `trace_repair_count`.

    Inputs are `registry_row`, `route_row`, `receipt_count`, `source_module_count`, and
    `trace_repair_count`; notable helpers are `get`.
    """
...
def build_route_candidate_read_model(root: str | Path | None = None) -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.projections.route_candidate_read_model.build_route_candidate_read_model`
    into the payload shape expected by projections route candidate read model.
...
def validate_route_candidate_read_model(payload: dict[str, Any]) -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.projections.route_candidate_read_model.validate_route_candidate_read_model`
    into the payload shape expected by projections route candidate read model.
...
def compile_paths(root: str | Path | None = None, out: str | Path | None = None) -> dict[str, Any]:
    ...
    Inputs are `root` and `out`; notable helpers are `build_route_candidate_read_model`,
    `validate_route_candidate_read_model`, `Path`, `mkdir`, and 2 more.
    """
...
def main(argv: list[str] | None = None) -> int:
    """
    Run `microcosm_core.projections.route_candidate_read_model` as a command-line entry
    point.
