"""
Implements projections organ surface contract for the public Plectis package.

Callers enter through `build_organ_surface_contract`, `build_card`,
`build_organ_relationship_topology_card`, and `main`; constants such as `SCHEMA_VERSION`,
`CARD_PREVIEW_LIMIT`, `AUTHORITY_POSTURE`, `ENTRY_ROUTE_REF`, and 14 more pin local fixture
names; dependencies include `argparse`, `ast`, `hashlib`, `json`, and 5 more. Projection
...
def _repo_root(root: Path) -> Path:
    """
    Return repo root for the projections organ surface contract flow.
...
def _as_list(value: Any) -> list[Any]:
    """
    Return as list for the projections organ surface contract flow.
...
def _accepted_registry_rows(registry: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    Inputs are `registry`; notable helpers are `_as_list` and `get`.
    """
    return [
        row
...
def _is_synthetic_acceptance_row(row: dict[str, Any]) -> bool:
    """
    Return whether is synthetic acceptance row holds for the projections organ surface
    contract flow.
...
def _synthetic_disposition_value(row: dict[str, Any]) -> str:
    """
    Return synthetic disposition value for
...
def _disposition_audit_for_row(
    ...
    """
    Serialize `microcosm_core.projections.organ_surface_contract._disposition_audit_for_row`
    into the payload shape expected by projections organ surface contract.
...
def _acceptance_metadata_audit_for_row(
    ...
    """
    Serialize
    `microcosm_core.projections.organ_surface_contract._acceptance_metadata_audit_for_row`
    into the payload shape expected by projections organ surface contract.
...
def _runner_source_ref(runner: str) -> str:
    """
    Compute runner source ref from `runner`.
...
def _resolve_ref(root: Path, ref: str) -> Path:
    ...
    Inputs are `root` and `ref`; notable helpers are `Path`, `is_absolute`, `split`, and
    `_repo_root`.
    """
...
def _paper_module_ref(root: Path, organ_id: str, atlas_row: dict[str, Any]) -> str:
    """
    Return paper module ref for the projections organ surface contract flow.

    Inputs are `root`, `organ_id`, and `atlas_row`; notable helpers are `strip`, `is_file`,
    `Path`, `as_posix`, and 1 more.
    """
...
def _atlas_rows_by_organ(root: Path) -> dict[str, dict[str, Any]]:
    """
    Compute atlas rows by organ from `root`.

    Inputs are `root`; notable helpers are `read_json_strict`, `get`, and `_as_list`.
    """
...
def _standards_registry_by_id(root: Path) -> dict[str, dict[str, Any]]:
    """
    Return standards registry by ID for the projections organ surface contract flow.
...
def _source_language_for_ref(ref: str) -> tuple[str, str]:
    """
    Produce the source language for ref value used by
    `microcosm_core.projections.organ_surface_contract`.

    Inputs are `ref`; notable helpers are `lower`, `get`, and `Path`.
    """
...
def _root_relative_ref(root: Path, ref: str) -> str:
    """
    Compute root relative ref from `root` and `ref`.

    Inputs are `root` and `ref`; notable helpers are `Path`, `is_absolute`, `startswith`,
    `removeprefix`, and 2 more.
    """
    path = Path(ref)
    if path.is_absolute():
        try:
...
def _mechanism_source_module_manifest_refs(root: Path, organ_id: str) -> list[str]:
    """
    Produce the mechanism source module manifest refs value used by
    `microcosm_core.projections.organ_surface_contract`.

    Inputs are `root` and `organ_id`; notable helpers are `read_json_strict`, `_as_list`,
    `is_file`, `get`, and 5 more.
    """
...
def _source_module_refs(root: Path, organ_id: str) -> list[str]:
    """
    Compute source module refs from `root` and `organ_id`.

    Inputs are `root` and `organ_id`; notable helpers are `is_dir`,
    `_source_module_manifest_refs`, `rglob`, `read_json_strict`, and 8 more.
    """
...
def _source_module_manifest_refs(root: Path, organ_id: str) -> list[str]:
    """
    Return source module manifest refs for the projections organ surface contract flow.

    Inputs are `root` and `organ_id`; notable helpers are `is_dir`,
    `_mechanism_source_module_manifest_refs`, `rglob`, `is_file`, and 3 more.
    """
    examples_root = root / "examples" / organ_id
...
def _edge_ref_token(*refs: str) -> str:
    """
    Compute edge ref token from `refs`.

    Inputs are `refs`; notable helpers are `encode`, `hexdigest`, `join`, and `sha1`.
    """
...
def _declared_target_ref(
    ...
    """
    Derive declared target ref without touching module import state.

    Inputs are `root`, `manifest_path`, and `module`; notable helpers are `strip`,
    `as_posix`, `is_absolute`, `startswith`, and 3 more.
    """
...
def _source_module_manifest_records(
    ...
    """
    Compute source module manifest records from `root` and `accepted_organ_ids`.

    Inputs are `root` and `accepted_organ_ids`; notable helpers are
    `_source_module_manifest_refs`, `read_json_strict`, `_as_list`, `get`, and 3 more.
    """
...
def _file_relationship_edge(
    ...
    **metadata: Any,
    ...
    """
    Compute file relationship edge from `edge_id`, `organ_id`, `relation_type`, `source_id`,
    `target_id`, and 3 more.

    Inputs are `edge_id`, `organ_id`, `relation_type`, `source_id`, `target_id`, and 3 more;
    notable helpers are `update`.
    """
    edge = {
        "edge_id": edge_id,
        "organ_id": organ_id,
...
def _shard_ref(file_ref: str, anchor: str) -> str:
    """
    Produce the shard ref value used by `microcosm_core.projections.organ_surface_contract`.

    Inputs are `file_ref` and `anchor`.
    """
...
def _source_module_file_graph(
    ...
    """
    Serialize `microcosm_core.projections.organ_surface_contract._source_module_file_graph`
    into the payload shape expected by projections organ surface contract.
    ...
    def append_edge(edge: dict[str, Any]) -> None:
        """
        Add one source-module file edge to both graph indexes.

        The closure keeps the flat edge list and the organ-keyed lookup in sync while
        `_source_module_file_graph` walks manifest records.
        """
        edges.append(edge)
...
def _source_language_inventory(
    ...
    """
    Serialize `microcosm_core.projections.organ_surface_contract._source_language_inventory`
    into the payload shape expected by projections organ surface contract.
...
def _source_language_adjacency(
    ...
    """
    Serialize `microcosm_core.projections.organ_surface_contract._source_language_adjacency`
    into the payload shape expected by projections organ surface contract.
...
def _relationship_edge(
    ...
    **metadata: Any,
    ...
    """
    Return relationship edge for the projections organ surface contract flow.

    Inputs are `organ_id`, `relation_type`, `target_id`, `evidence_class`,
    `source_projection`, and 1 more; notable helpers are `update`.
    """
    edge = {
...
def _standard_id_from_ref(ref: str) -> str:
    """
    Produce the standard ID from ref value used by
    `microcosm_core.projections.organ_surface_contract`.

    Inputs are `ref`; notable helpers are `Path` and `split`.
    """
    if not ref:
        return ""
...
def _organ_relationship_topology(
    ...
    """
    Serialize
    `microcosm_core.projections.organ_surface_contract._organ_relationship_topology` into
    the payload shape expected by projections organ surface contract.
    ...
    def append_edge(edge: dict[str, Any]) -> None:
        """
        Add one organ-topology edge to the accumulated relationship graph.

        The closure updates the flat edge stream and the organ-keyed index used in the
        topology payload.
        """
        edges.append(edge)
        edges_by_organ.setdefault(str(edge["organ_id"]), []).append(edge)
...
def _acceptance_plan_rows(root: Path) -> list[dict[str, Any]]:
    """
    Return acceptance plan rows for the projections organ surface contract flow.

    Inputs are `root`; notable helpers are `read_json_strict`, `is_file`, `_as_list`, and
    `get`.
    """
...
def _runtime_step_ids() -> set[str]:
    """
    Derive runtime step IDs without touching module import state.
...
def _cli_command_names(root: Path) -> set[str]:
    """
    Return cli command names for the projections organ surface contract flow.

    Inputs are `root`; notable helpers are `parse`, `walk`, `read_text`, and `add`.
    """
...
def _cli_command_from_first_command(first_command: str) -> str:
    """
    Compute cli command from first command from `first_command`.

    Inputs are `first_command`; notable helpers are `split`.
    """
    try:
...
def _expected_cli_command(
    *, organ_id: str, atlas_row: dict[str, Any], cli_command_names: set[str]
    ...
    """
    Compute expected CLI command from `organ_id`, `atlas_row`, and `cli_command_names`.

    Inputs are `organ_id`, `atlas_row`, and `cli_command_names`; notable helpers are
    `_cli_command_from_first_command`, `replace`, and `get`.
    """
...
def _validator_command_module(command: str) -> str:
    """
    Return validator command module for the projections organ surface contract flow.
...
def _global_doctrine_surfaces(root: Path) -> dict[str, Any]:
    """
    Serialize `microcosm_core.projections.organ_surface_contract._global_doctrine_surfaces`
    into the payload shape expected by projections organ surface contract.
...
def build_organ_surface_contract(root: str | Path | None = None) -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.projections.organ_surface_contract.build_organ_surface_contract` into
    the payload shape expected by projections organ surface contract.
...
def build_card(payload: dict[str, Any]) -> dict[str, Any]:
    """
    Serialize `microcosm_core.projections.organ_surface_contract.build_card` into the
    payload shape expected by projections organ surface contract.
...
def build_organ_relationship_topology_card(
    ...
    *,
    ...
    """
    Serialize
    `microcosm_core.projections.organ_surface_contract.build_organ_relationship_topology_card`
    into the payload shape expected by projections organ surface contract.
    ...
    def edge_matches(edge: dict[str, Any]) -> bool:
        """
        Return whether edge matches holds for the projections organ surface contract flow.

        The result is derived from `edge` with `get`; failing evidence is returned or raised
        exactly where the body says so.
        """
...
def main(argv: list[str] | None = None) -> int:
    """
    Run `microcosm_core.projections.organ_surface_contract` as a command-line entry point.
