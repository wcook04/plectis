"""
Implements private state scan for the public Plectis package.

Callers enter through `load_forbidden_classes`, `public_relative_path`,
`is_text_scan_candidate`, `classify_public_safe_macro_import`, `scan_text`, `scan_paths`,
and 1 more; constants such as `PASS`, `BLOCKED_PRIVATE`, `BLOCKED_PUBLIC_WRITE`,
`BLOCKED_CASE_REVIEW`, and 7 more pin local fixture names; dependencies include
`collections`, `json`, `pathlib`, `typing`, and 1 more. Importing it does not authorize
...
def _looks_like_public_root(path: Path) -> bool:
    """
    Return whether looks like public root holds for the private state scan flow.

    The result is derived from `path` with `is_file` and `is_dir`; failing evidence is
...
def load_forbidden_classes(path: str | Path) -> dict[str, Any]:
    """
    Load load forbidden classes for `microcosm_core.private_state_scan`.
...
def _resolved(path: Path) -> Path:
    """
    Return resolved for the private state scan flow.

    Inputs are `path`; notable helpers are `resolve` and `expanduser`.
    """
...
def _public_root_for_path(path: str | Path) -> Path | None:
    """
    Compute public root for path from `path`.

    Inputs are `path`; notable helpers are `Path`, `_resolved`, `resolve`, `is_dir`, and 3
    more.
    """
    raw_path = Path(path)
...
def public_relative_path(path: str | Path, *, display_root: str | Path | None = None) -> str:
    """
    Return public relative path for the private state scan flow.

    Inputs are `path` and `display_root`; notable helpers are `Path`, `_resolved`,
    `_public_root_for_path`, `as_posix`, and 2 more.
    """
    raw_path = Path(path)
...
def _infer_display_root(paths: Iterable[Path]) -> Path | None:
    """
    Produce the infer display root value used by `microcosm_core.private_state_scan`.

    Inputs are `paths`; notable helpers are `_public_root_for_path`.
    """
    for path in paths:
...
def _terms(forbidden_classes: dict[str, Any]) -> list[dict[str, str]]:
    """
    Produce the terms value used by `microcosm_core.private_state_scan`.

    Inputs are `forbidden_classes`; notable helpers are `get`, `strip`, and `append`.
    """
...
def _import_policy(forbidden_classes: dict[str, Any]) -> dict[str, Any]:
    """
    Produce the import policy value used by `microcosm_core.private_state_scan`.

    Inputs are `forbidden_classes`; notable helpers are `get`.
    """
...
def _scan_scope(forbidden_classes: dict[str, Any]) -> dict[str, Any]:
    """
    Scan whether scan scope holds for the private state scan flow.

    The result is derived from `forbidden_classes` with `get` and `update`; failing evidence
...
def _anti_claim(forbidden_classes: dict[str, Any]) -> str:
    """
    Return anti claim for the private state scan flow.
...
def _string_list(value: object) -> list[str]:
    """
    Produce the string list value used by `microcosm_core.private_state_scan`.
...
def is_text_scan_candidate(path: str | Path) -> bool:
    """
    Return whether is text scan candidate holds for the private state scan flow.

    The result is derived from `path` with `Path`; failing evidence is returned or raised
...
def _bool_from_row(row: dict[str, Any], key: str) -> bool:
    """
    Return whether bool from row holds for the private state scan flow.

    The result is derived from `row` and `key` with `get`; failing evidence is returned or
...
def classify_public_safe_macro_import(
    ...
    *,
    ...
    """
    Serialize `microcosm_core.private_state_scan.classify_public_safe_macro_import` into the
    payload shape expected by private state scan.
...
def _is_expected_negative_fixture_path(path: str) -> bool:
    """
    Return whether is expected negative fixture path holds for the private state scan flow.

    The result is derived from `path` with `lower`, `endswith`, and `startswith`; failing
...
def _allowed_synthetic_negative(path: str, text: str) -> bool:
    """
    Return whether allowed synthetic negative holds for the private state scan flow.

    The result is derived from `path` and `text` with `lower`, `endswith`,
    `_is_expected_negative_fixture_path`, and `startswith`; failing evidence is returned or
    raised exactly where the body says so.
    """
...
def _path_hit(path: str, source_context: str) -> dict[str, Any] | None:
    """
    Serialize `microcosm_core.private_state_scan._path_hit` into the payload shape expected
    by private state scan.
...
def _scan_text_with_terms(
    ...
    *,
    ...
    """
    Serialize `microcosm_core.private_state_scan._scan_text_with_terms` into the payload
    shape expected by private state scan.
...
def scan_text(
    ...
    *,
    ...
    """
    Scan whether scan text holds for the private state scan flow.

    The result is derived from `text`, `path`, `forbidden_classes`, and `source_context`
    with `_scan_text_with_terms` and `_terms`; failing evidence is returned or raised
    exactly where the body says so.
    """
...
def _merge_scan_hits(
    ...
    """
    Serialize `microcosm_core.private_state_scan._merge_scan_hits` into the payload shape
    expected by private state scan.
...
def _merge_scan_results(
    ...
    """
    Produce the merge scan results value used by `microcosm_core.private_state_scan`.

    Inputs are `results` and `forbidden_classes`; notable helpers are `_merge_scan_hits`,
    `extend`, and `get`.
    """
    hits: list[dict[str, Any]] = []
...
def _unreadable_text_result(
    *,
    ...
    """
    Serialize `microcosm_core.private_state_scan._unreadable_text_result` into the payload
    shape expected by private state scan.
...
def _scan_path_with_terms(
    ...
    *,
    ...
    """
    Serialize `microcosm_core.private_state_scan._scan_path_with_terms` into the payload
    shape expected by private state scan.
...
def scan_paths(
    ...
    *,
    ...
    """
    Scan whether scan paths holds for the private state scan flow.

    The result is derived from `paths`, `forbidden_classes`, `source_context`, and
    `display_root` with `_terms`, `_merge_scan_hits`, `Path`, `resolve`, and 8 more; failing
    evidence is returned or raised exactly where the body says so.
    """
    hits: list[dict[str, Any]] = []
    scanned = 0
...
def scan_json_payload(
    ...
    *,
    ...
    """
    Scan whether scan JSON payload holds for the private state scan flow.

    The result is derived from `payload`, `path`, `forbidden_classes`, and `source_context`
    with `dumps` and `scan_text`; failing evidence is returned or raised exactly where the
    body says so.
    """
