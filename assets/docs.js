(function () {
  ...
  // Glossary previews are useful on many pages but are not first-paint data.
  // Keep their governed prose in one small shared projection and fetch it only
  // when a reader first hovers or focuses a term. Native anchor hrefs remain
  ...
  function completeTermPreviews(terms) {
    mcTermPreviewState = terms && terms.length ? 'ready' : 'failed';
    var queued = mcTermPreviewCallbacks.slice();
    mcTermPreviewCallbacks = [];
    queued.forEach(function (cb) { cb(terms || []); });
  }

  function existingTermPreviewScript() {
    var tagged = document.querySelector('script[data-term-previews]');
    if (tagged) return tagged;
    var scripts = Array.prototype.slice.call(document.querySelectorAll('script[src]'));
    return scripts.filter(function (s) {
      return /(?:^|\/)term-previews\.js(?:[?#].*)?$/.test(s.getAttribute('src') || s.src || '');
    }).pop() || null;
  }

  function withTermPreviews(cb) {
    var terms = mcTermPreviewRecords();
    if (terms.length) {
      mcTermPreviewState = 'ready';
      cb(terms);
      return;
    }
    if (mcTermPreviewState === 'failed') {
      cb([]);
      return;
    }
    mcTermPreviewCallbacks.push(cb);
    if (mcTermPreviewState === 'loading') return;
    mcTermPreviewState = 'loading';

    var existing = existingTermPreviewScript();
    if (existing) {
      existing.addEventListener('load', function () { completeTermPreviews(mcTermPreviewRecords()); });
      existing.addEventListener('error', function () { completeTermPreviews([]); });
      return;
    }
    ...
    s.addEventListener('error', function () {
      if (window.console && console.warn) console.warn('Plectis: term-previews.js failed to load; falling back to the search index.');
      completeTermPreviews([]);
    });
    document.head.appendChild(s);
  }
  ...
  /* Glossary cards keep their long generated bodies inside <noscript> shells.
     With JS disabled the browser renders that source normally. With JS enabled
     it is inert text until this card is opened, which avoids constructing and
     styling tens of thousands of off-screen nodes during navigation. */
  function hydrateDeferredDetails(details) {
    if (!details || details.tagName !== 'DETAILS' || !details.querySelector) return false;
    var shell = details.querySelector('noscript[data-term-body]');
    if (!shell) return false;
    var holder = document.createElement('template');
    holder.innerHTML = shell.textContent || '';
    details.insertBefore(holder.content, shell);
    if (shell.parentNode) shell.parentNode.removeChild(shell);
    details.setAttribute('data-term-hydrated', '');
    return true;
  }
  ...
  document.addEventListener('toggle', function (event) {
    ...
    if (details && details.tagName === 'DETAILS' && details.open) {
      hydrateDeferredDetails(details);
    }
  }, true);
  ...
  (function viewState() {
    ...
    function pageTitle() {
      // Prefer the visible page heading: it is the reader-facing destination
      ...
      if (!heading) {
        t = cleanText(t.replace(/^\d+\.\s*/, '').replace(/^🐴\s*/, ''));
    ...
    function renderAffordance() {
      ...
      // next paint, and a document that is not painting -- backgrounded, or
      // otherwise throttled by the engine -- does not get one. Observed in the
      // wild: the button sat fully transparent for 51 seconds after load while
      // still hit-testing as the topmost element, i.e. an invisible control the
      // pointer could nonetheless press. A timer backstop cannot be throttled
      // the same way, and classList.add is idempotent, so whichever arrives
      // first reveals the pill and the other is a no-op.
      var reveal = function () { btn.classList.add('is-shown'); };
  ...
  (function pageExport() {
    ...
    function readablePageText(article) {
      ...
      // The wide-layout and landing views export a whole <main>, not a single
    ...
    buttons.forEach(function (btn) {
      ...
      // Narrow docs views own exactly one .docs-article, so the button can find
      // its own column. Wide views (component and paper-module details, the
      // architecture map) and the landing page are assembled from several
      // sections, and declare the root they mean with data-export-root.
      var rootSelector = btn.getAttribute('data-export-root');
      function exportRoot() {
        if (rootSelector) {
          var declared = document.querySelector(rootSelector);
          if (declared) return declared;
        }
        return btn.closest('.docs-article')
  ...
  (function search() {
    ...
    function paint() {
      ...
      for (var i = 0; i < items.length; i++) {
        ...
        // select a row; only that row's actions join the dialog's focus loop.
        var rowActions = items[i].querySelectorAll('.cmdk__action');
    ...
    function render() {
      ...
      results.forEach(function (rec, rowIndex) {
        ...
        if (rec.command) {
          ...
          copyBtn.tabIndex = -1; // paint() promotes actions on the active row
  ...
  (function graphMap() {
    ...
    // (records carry graph_node_id). Lazy-loaded on the first component preview
    // or pin, so the page does not pay this parse cost at startup.
    var recByNode = {};
    ...
    if (graphCap) {
      ...
      graphCap.textContent = fig.getAttribute('data-graph-consumer') === 'landing'
        ...
        : 'An interactive picture of the seven public areas, the shared path, and every public component. Hover a node to preview its summary and links; click to pin it; double-click (or the panel button) to open its page, the paper-module write-up where one exists, otherwise the component spec. Press Esc, or Whole map, to return to the overview.';
    }
    ...
    function nodeFromPoint(x, y) {
      ...
      if (!target || !fig.contains(target)) return null;
      return target.closest ? target.closest('.gnode') : null;
    }

    function resumeMapHoverAfterWheel() {
      mapWheelPause = false;
      mapWheelTimer = 0;
      var node = nodeFromPoint(lastPointerX, lastPointerY);
      if (node && node.getAttribute('data-id')) scheduleHover(node.getAttribute('data-id'));
      else restore();
    }

    // Suppress hover work WHILE the page is scrolling (nodes sliding under a
    // stationary cursor fire spurious mouseenter), then re-resolve from the real
    // pointer position once motion settles. Deliberately cheap: sets a flag and a
    // timer and nothing else. It must never touch the scroll itself -- scrolling
    // belongs to the browser (see the passive listener below).
    function beginMapWheelPause(ev) {
      lastPointerX = ev.clientX || lastPointerX;
      lastPointerY = ev.clientY || lastPointerY;
      mapWheelPause = true;
      cancelPendingHover();
      if (mapWheelTimer) window.clearTimeout(mapWheelTimer);
      mapWheelTimer = window.setTimeout(resumeMapHoverAfterWheel, 180);
    }

    function scheduleHover(id) {
      if (mapWheelPause) return;
      pendingHoverId = id;
    ...
    (function dock() {
      ...
      if (scroll) {
        ...
        // PASSIVE, and no preventDefault: this listener only pauses hover. Scrolling
        // over the map must feel exactly like scrolling anywhere else on the page,
        // and the only way to get that is to let the compositor do it. A non-passive
        // wheel listener forces every scroll event through the main thread before the
        // page may move, and re-driving it with window.scrollBy() throws away the
        // browser's own wheel smoothing -- together that is what made the map feel
        // stiff and steppy. Vertical fall-through is now a property of the box
        // (overflow-y: hidden in style.css), not something JS re-implements.
        scroll.addEventListener('wheel', beginMapWheelPause, { passive: true });
        var syncScrollAffordance = function () {
          // Horizontal only: the box no longer scrolls vertically by design.
          var overflows = scroll.scrollWidth > scroll.clientWidth + 2;
          if (overflows) {
            scroll.setAttribute('tabindex', '0');
            scroll.setAttribute('aria-label', 'Map, scrollable sideways — use arrow keys');
    ...
    function renderDefault() {
      ...
      panel.appendChild(el('p', 'gpanel__hint',
        'Hover a node to preview its summary and links. Click to pin it. Press Esc, or Whole map, to clear.'));
    ...
    function renderNode(id) {
      ...
      // previews and pins lazy-load that index and rerender; structural nodes keep
      // their build-time data-summary, so the resting page pays nothing.
      var jobRec = recByNode[id];
      if (!jobRec && graphSearchState !== 'failed' && id.indexOf('component:') === 0) {
        ensureGraphSearchIndex(function () {
          if ((pinnedId === id || activeGraphState === 'isolate:' + id) && recByNode[id]) {
            panelState = '';
            renderNode(id);
          }
        });
    ...
    // SVG's promised text equivalent. Pointing at an item previews the SAME
    // canonical node in the map, and a visible "current" marker stays in sync
    // across both surfaces; activating an item drills down to that area's card,
    // which is exactly what a double-click on the node does. Absent on docs (no
    // twin) -> the wiring is an empty no-op there.
    ...
    twinItems.forEach(function (item) {
      ...
      // Hover/focus previews the twinned node in the map (the same preview a
      // pointer over the node itself gives), so the card still says WHERE it is.
      item.addEventListener('mouseenter', function () { if (byId[nid]) scheduleHover(nid); }, { passive: true });
      item.addEventListener('mouseleave', function () { if (byId[nid]) restore(); }, { passive: true });
      item.addEventListener('focus', function () { if (byId[nid]) scheduleHover(nid); });
      item.addEventListener('blur', function () { if (byId[nid]) restore(); });
      // Activation drills down: the card's href IS the node's card, so a click
      // here does what a double-click on the node does. No preventDefault --
      // cancel any pending hover preview and let the link navigate.
      item.addEventListener('click', function () { cancelPendingHover(); });
    });
  ...
  // close, focus returns). Preview records live in one shared lazy asset instead
  // of being copied into every HTML page. The much larger site-wide search index
  // remains a compatibility fallback, not a page-load dependency. Older or
  // hand-authored pages with inline records continue to work during rollouts.
  (function () {
    /* The glossary already is the full definition surface. Its term links are
       native in-page jumps, so constructing another preview layer over its
    ...
    // index work so the CSS grading never waits on the payload. An anchor
    // inside a collapsed <details> or [hidden] region is not a "read", so it
    // never claims the first-mention slot from the first link a reader sees
    // (unless every mention of the term is hidden).
    function inHiddenContext(node) {
    ...
    function gradeTermAnchors() {
      ...
      for (var m = 0; m < anchors.length; m++) {
        if (inHiddenContext(anchors[m])) { hiddenAnchors.push(anchors[m]); continue; }
        var seenKey = anchors[m].getAttribute('data-term');
        anchors[m].classList.add(seenTerm[seenKey] ? 'is-term-again' : 'is-term-first');
        seenTerm[seenKey] = true;
      }
      for (var h = 0; h < hiddenAnchors.length; h++) {
        var hidKey = hiddenAnchors[h].getAttribute('data-term');
        hiddenAnchors[h].classList.add(seenTerm[hidKey] ? 'is-term-again' : 'is-term-first');
        seenTerm[hidKey] = true;
      }
    }
    /* Hundreds of editorial grade markers do not belong on the critical path.
       The ungraded CSS is already a complete link treatment, so apply the
       first/repeat refinement in the first idle slot. */
    if (window.requestIdleCallback) {
      window.requestIdleCallback(gradeTermAnchors, { timeout: 1200 });
    } else {
    ...
    function inlineTerms() {
      ...
      for (var n = 0; n < anchors.length; n++) {
        ...
        rows.push({
          object_id: 'term:' + key,
          preferred_label: anchor.getAttribute('data-term-label') || anchor.textContent || key,
    ...
    function fullTerms() {
      ...
      return (idx && idx.terms) || [];
    }

    function previewTerms() {
      return mcTermPreviewRecords();
    }

    function currentTerms() {
      var rows = previewTerms();
      if (rows.length) return rows;
      rows = fullTerms();
      return rows.length ? rows : inlineTermRows;
    }
    ...
    function ensureLayer(anchor, intent) {
      ...
      layerLoading = true;
      withTermPreviews(function (previewRows) {
    ...
    /* Delegated intent listeners replace listeners on every term link. Preview
       records stay out of the HTML and load only on genuine term intent. A
    ...
    function onTermIntent(event) {
      ...
      if (event.type === 'mouseover' && event.relatedTarget && anchor.contains(event.relatedTarget)) return;
      ensureLayer(anchor, event.type === 'mouseover' ? 'pointer' : 'focus');
    }
    ...
    function placeFloater(node, anchor) {
      ...
      var readingBlock = anchor.closest('p, li, dd, dt, blockquote, h1, h2, h3, h4');
    ...
    var tipRule = el('div', 'term-tip__rule');
    tipRule.hidden = true;
    ...
    var tipEscape = el('div', 'term-tip__escape', 'Esc to close');
    tip.appendChild(tipEscape);
    ...
    var tipHideTimer = 0;
    var tipFadeTimer = 0;  // pending fade-to-hidden (pairs with .is-leaving)
    ...
    function expandTip(anchor) {
      ...
      /* Expansion is intentionally local. Pulling the site-wide search index
         for one definition made an ordinary click perform megabytes of work.
         The explicit full-glossary link remains available when deeper fields
         are actually wanted. */
      return true;
    }
    function hideTip(immediate) {
      ...
      // Leave as a quick fade, not a blink: .is-leaving drops opacity on the
      // site ease and the card is removed from the layout when the fade lands.
      // A hover during the fade revives the same card (see the timer clears
      // in showTip/expandTip), which is what makes term-to-term movement read
    ...
    tipBack.addEventListener('click', function () {
      ...
      requestAnimationFrame(function () {
        ...
        requestAnimationFrame(function () {
          /* Some browsers deliver focus-in after the click task. Close once
             more after that delivery, then return to normal preview behavior. */
          hideTip(true);
    ...
    window.addEventListener('scroll', function () {
      ...
      if (!hit || (!tipFor.contains(hit) && !tip.contains(hit))) hideTip(true);
    }, { passive: true });

    /* The complete layer also stays delegated: five document listeners replace
       up to five listeners on each of hundreds of generated term anchors. */
    ...
    if (pendingAnchor && pendingIntent === 'click') {
      ...
      showTip(pendingAnchor, pendingIntent || (document.activeElement === pendingAnchor ? 'focus' : 'pointer'));
    }
    pendingAnchor = null;
    pendingIntent = null;
    }
...
/* Landing claim carousel: arrow buttons scroll the card track by one card.
   No-ops on pages without the carousel. */
(function () {
  'use strict';
  var wrap = document.querySelector('[data-claim-carousel]');
  if (!wrap) return;
  var track = wrap.querySelector('.claim-cards');
  if (!track) return;
  var buttons = wrap.querySelectorAll('[data-carousel-move]');
  for (var i = 0; i < buttons.length; i++) {
    buttons[i].addEventListener('click', function (ev) {
      var dir = ev.currentTarget.getAttribute('data-carousel-move') === 'next' ? 1 : -1;
      var card = track.querySelector('.claim-card');
      var step = card ? card.getBoundingClientRect().width + 18 : 380;
      track.scrollBy({ left: dir * step, behavior: 'smooth' });
    });
  }
})();

/* Landing demo-video carousel: same arrow-scroll as the claim carousel, plus a
   play affordance. Real playback is injected only when a card carries a
   non-empty data-video-src, so the static page ships no video tag and no
   third-party frame; a same-origin source plays under the page's own CSP. Until
   a source is wired, play reveals the pending note and opens the written primer.
   No-ops on pages without the carousel. */
(function () {
  'use strict';
  var wrap = document.querySelector('[data-video-carousel]');
  if (!wrap) return;
  var track = wrap.querySelector('.video-cards');
  if (track) {
    var buttons = wrap.querySelectorAll('[data-carousel-move]');
    for (var i = 0; i < buttons.length; i++) {
      buttons[i].addEventListener('click', function (ev) {
        ...
        var step = card ? card.getBoundingClientRect().width + 24 : 460;
        track.scrollBy({ left: dir * step, behavior: 'smooth' });
      });
    }
  }
  var players = wrap.querySelectorAll('[data-video-play]');
  for (var j = 0; j < players.length; j++) {
    players[j].addEventListener('click', function (ev) {
      var btn = ev.currentTarget;
      var card = btn.closest ? btn.closest('.video-card') : null;
      ...
      if (src) {
        ...
        if (media.play) { try { media.play(); } catch (err) {} }
        return;
      }
      if (card) {
        card.classList.add('is-pending');
        var primer = card.querySelector('.video-card__tldw');
        if (primer) primer.open = true;
      }
    });
  }
})();

/* Day / night toggle. Stamps data-theme on <html> (the CSS carries the flip),
   persists the choice in localStorage, and injects an animated slider at the
   right end of the primary nav. Runs on every page — docs.js is the shared
   runtime. A blocking theme-init.js in <head> does the pre-paint apply so a
   returning visitor never flashes the other theme; this stays idempotent with
   it and is the sole applier on any page that omits the head init. */
(function () {
  ...
  function mount() {
    var nav = document.querySelector('.site-header nav[aria-label="Primary"]') ||
              document.querySelector('.docs-topbar__links') ||
...
(function () {
  ...
  function reveal(hash) {
    ...
    if (!target) return null;
    // Any disclosure the target is nested inside, outermost included.
  ...
  window.addEventListener('hashchange', function () {
    var target = reveal(window.location.hash);
...
/* Warm navigation: prefetch the page a pointer is committing to.
   Every page here is one small same-origin document, so the moment a pointer
   rests on a link (140ms — a deliberate settle, not a pass-over) that document is asked
   for with prefetch semantics; by the time the click lands the HTML is
   usually already local, and the fingerprinted assets are already cached, so
...
(function () {
  'use strict';
  if (!document.querySelector || !window.URL || !document.createElement('a').closest) return;
  var conn = navigator.connection || {};
  if (conn.saveData) return;
  if (typeof conn.effectiveType === 'string' && conn.effectiveType.indexOf('2g') !== -1) return;
  try {
