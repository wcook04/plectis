...
For instance $`D_{1,12,16}=-143140`$, with residue $`53468`$ modulo $`2^{16}`$ and $`B=31`$, supplies a seed: $`31<53468<65536-31`$. The error estimate gives the quantitative separation
``` math
\|\Delta_1(12)\|_{\mathbb{R}/\mathbb{Z}}
 \ge\frac{\min(53468,65536-53468)-31}{65536}
 =\frac{12037}{65536}.
```
For this seed, two adjacent full-depth failures would force
``` math
\|\Delta_1(12)\|_{\mathbb{R}/\mathbb{Z}}
 \le\frac{144+20t}{2^{t+1}}
 \le\frac{43}{256}<\frac{12037}{65536}\qquad(t\ge10),
```
since the middle expression decreases with $`t`$. Thus every pair $`\{t,t+1\}`$ with $`t\ge10`$ contains an $`m`$ satisfying $`K(m,12,m)`$. This explicit threshold comes from the displayed estimates. The finite seed supplies infinitely many certificates on this one ray; it does not provide seeds for all $`d,N`$.
