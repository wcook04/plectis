...
\end{equation}
```
For example, take $`b=2`$, $`Q=6`$ and $`d=4`$. Since $`(Q,d)=2`$, the residues $`6m\bmod4`$ alternate between $`2`$ and $`0`$, and the weights alternate between $`4/15`$ and $`1/15`$. Their mean is $`1/6`$, exactly $`2/[4(2^2-1)]`$ as in <a href="#eq:gcd-orbit-mean" data-reference-type="eqref" data-reference="eq:gcd-orbit-mean">[eq:gcd-orbit-mean]</a>.
...
The two criteria below use different controls on the same positive displacement. The estimates named in this section are not independently checkable from the files in this checkout: the cited supplements and `MixedSupportSynchronisation.md` are absent from the public packet. The statements are retained as claimed extensions, not as results of this release.
