...
At $`N=2`$ the unique minimising tuple is $`(k_0,k_1)=(0,1)`$. Hence $`B_2=1`$ and $`C_2=(2!)^2 3!/2^2=6`$, so the theorem begins with the concrete expansion $`V_2^*(q)=6q+O(q^2)`$. Already here uniqueness does the work: every other increasing pair has larger weighted degree, so no second summand can cancel the coefficient $`6`$.
