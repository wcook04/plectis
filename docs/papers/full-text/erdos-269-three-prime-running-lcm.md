...
\end{aligned}
```
Both factors are positive. After removing these row and column factors, an entry is $`c`$ when $`x_i+y_j\ge1`$ and $`1`$ otherwise. We choose the column thresholds $`1-y_j`$ so that each successive row introduces exactly one new $`c`$. Subtracting consecutive rows will then isolate one nonzero entry at a time.

Distinct primes make $`\log_r p`$ and $`\log_r q`$ irrational, so each individual rotation is dense. For $`n\ge1`$, choose distinct row indices with $`0<x_{I(0)}<\cdots<x_{I(n-1)}<1`$. Choose $`1-y_{J(0)}`$ in $`(0,x_{I(0)})`$, and, for $`1\le b<n`$, choose $`1-y_{J(b)}`$ in $`(x_{I(b-1)},x_{I(b)})`$. The intervals are disjoint, so the column indices are distinct. These choices give
``` math
T_n(c)=\begin{pmatrix}
 c&1&\cdots&1\\
 c&c&\cdots&1\\
