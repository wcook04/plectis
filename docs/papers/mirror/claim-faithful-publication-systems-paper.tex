...
claim stronger. In the private workbench, an agent enters a world through a compiled comprehension
...
A compressed trace has an observation boundary. A missing path or worker
outcome in a summary is not evidence that no change occurred. Repository
state, command results, diffs, formal builds, and commit objects must settle
that question. Completeness is therefore an explicit field in the workbench's
design, not an impression created by fluent prose. Private activity traces
are production records; this paper does not use their event counts as public
evidence of throughput or successful mathematical progress.

The design interleaves search and validation, retains scoped failed routes,
and permits concurrency when work can be separated. Every claimed advance
must cross from a narrative of activity into an authority-bearing artefact and
receipt. A run may stop
...
as separate jobs with source-pinned hand-offs. These are instructions for an
agent or controller, not a resident scheduler. A single agent may execute the
two jobs sequentially; two tasks,
machines, models, or people may also hold the roles. The separation is logical,
not a requirement to buy two computers.

The prescribed coupling is event-driven. A newly landed theorem, counterexample,
authority change, paper correction, or external review outcome supplies the
trigger for a stewardship pass. Its output is a source-pinned appraisal and a changed
consumer or frontier disposition, not a recurring status report. That output
may in turn direct discovery toward a stronger target, a missing
...
counterexample, or a diagnosed failure. Accepted nodes and edges become inputs
to the next search; whether they improve it is an evaluation question.
Reusable process lessons can change heuristics and context, never mathematical
truth. When a mechanism comes from the literature, its annex
...
not the same as ``everything is understood.'' The
\repolink{docs/declaration_atlas.json}{declaration atlas} locates statements.
The \repolink{scripts/query_corpus.py}{dependency queries} distinguish module
imports from constant references extracted from elaborated Lean declarations;
an unavailable or stale extraction is not evidence that a dependency is absent.
The \repolink{docs/semantic/README.md}{authored semantic layer} explains a
statement's mathematical role, including specialisation, transport, and scoped
obstruction. The \repolink{docs/claims.json}{claim record} connects selected
results to reviewed wording and exact open propositions. These maps answer
different questions about the same source. A dependency can guide premise
selection without establishing an authored analogy; an analogy can suggest a
proof without supplying one.
...
evidence class and emits an omission receipt with an expansion route. This is
designed omission, not exhaustive loading: a bounded answer must expose how to
recover the source outside its selection. When an exact path is claimed, a local connection card can put its
...
lemma. A build also needs the public toolchain and dependency versions recorded
by \repolink{lean-toolchain}{lean-toolchain} and
\repolink{lake-manifest.json}{lake-manifest.json}; a fresh checkout may need to
download them. Self-contained here concerns source and authority, not an
offline installation. The larger workflow is provenance: it explains
...
must provide terms of those exact types. The
\repolink{verification/comparator.json}{configuration} names the permitted
axioms, modules, and runtime receipt; the
\repolink{docs/EXTERNAL_VERIFICATION.md}{verification guide} explains their
separate roles. A named altered statement must fail,
...
can say that the local packet satisfies its structural requirements. The
...
marks a coverage boundary, not a reliability score. The
...
make the registered test edits inspectable; they are not the missing original
run logs.
...
review. Path leases govern the second, validation admission governs the third, and
