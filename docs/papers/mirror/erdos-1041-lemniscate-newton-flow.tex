...
\[
  ...
  f(z(t))=e^{-t}f(z(0)) .
\]
A nonzero value therefore moves radially inward at exponential rate without
changing argument; a zero value stays zero.  Along every existing trajectory,
the lemniscate $\{|f|<1\}$ is therefore forward-invariant and the trajectory is
contained in the preimage of one ray from the origin.  This scalar identity
supplies no global existence theorem or description of an entire ray preimage.
...
This is the form needed by the topological route below.
...
\noindent For the constant perturbation $g=f+\beta$, one has $g'=f'$, so the
critical points stay fixed while each critical value changes from $f(c)$ to
$f(c)+\beta$.  Thus each pair of critical values contributes a one-real-parameter
