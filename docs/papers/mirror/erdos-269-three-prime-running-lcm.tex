...
\end{aligned}
\]
Both factors are positive.  After removing these row and column factors,
an entry is $c$ when $x_i+y_j\ge1$ and $1$ otherwise.  We choose the column
thresholds $1-y_j$ so that each successive row introduces exactly one new
$c$.  Subtracting consecutive rows will then isolate one nonzero entry at a
time.

Distinct primes make $\log_r p$ and
