...
For a numerical illustration, $2\mid8$, $3\mid9$, and $5\mid10$.
An integer sequence crossing from below $8$ in a step of at most $3$ must
land in $\{8,9,10\}$, even if it has previously decreased.  It therefore
hits a multiple of one of the three moduli.  The proof uses this mechanism
with moduli chosen from the given coprime family and a block placed beyond
the finite prefix where coprimality with them is not yet required.
