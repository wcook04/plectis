...
not an evaluation of reviewer performance.

A worked component calculates an
...
is the right account of audio level.

I chose the published fragments, cases,
...
\begin{paperresult}{The result a public rerun can support}
\textbf{Result.} A claim--evidence--limit contract can make disagreement about
one selected public test local: the reader can inspect the input, procedure,
expected answer, decision rule, run record, and stated inference limit.
\textbf{Evidence.} The worked audio component is rerunnable, and one expected
value can be derived independently from the published formula.
\textbf{Boundary.} A matching run does not establish origin, correctness,
representativeness, or reliability of the private system.
\end{paperresult}
...
The public record shows each boundary as declared at the
...
internal consistency, not of origin.
...
what the result should have been. An independent standard must be stated
separately from the implementation. The worked example makes the
