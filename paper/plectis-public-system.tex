...
\begin{abstract}
Plectis is a case study in testing public claims about software that cannot
itself be published.  The author releases selected code, fixed inputs, expected
...
components (separately testable parts) connects a prose claim to a fixed input,
command, saved output, pass rule, and stated limit.  I call this a
claim--evidence--limit contract.  A worked component calculates an audio level
from five small sample arrays.  A reader can repeat the calculation and,
...
it \cite[\S2.5, p.~138]{runeson2009}. I borrow only those two terms: the named repository
...
an operational oracle from a conceptual ground-truth oracle that always gives
the right answer, while observing that such ground truth is unknowable except
in trivial cases \cite[Defs.~2.4 and 2.6--2.8, pp.~509--510]{barr2015}. A
validator can therefore implement a published decision rule without
independently establishing that the rule is correct. The risk
