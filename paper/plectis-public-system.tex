...
The response names the audio component and offers a \code{no-write variant},
which leaves the repository's saved receipts unchanged. No launcher translation
is needed. Run the exact no-install command printed near the start of
Section~\ref{sec:run}; it writes only to \code{/tmp}. Under
\component{/tmp/plectis-audio-rms/}, open
\component{batch8_audio_level_rms_port_result.json};
JSON is a plain-text field-and-value format. Inside \code{exercise}, compare
\code{expected\_level} with \code{observed\_level} in each
\component{reference_cases} item. The top-level \component{anti_claim} says what a pass
...
reviewer checked their limits. For a challenge, copy the fixture to \code{/tmp},
change an input, and rerun with \code{--input} pointing to the copy. Compare the
validator's rule with the prose. Treat a pass as evidence only for the published
rule; report disputes through \code{CONTRIBUTING.md}.
