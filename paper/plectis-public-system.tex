...
runnable fragments drawn from work they cannot see. At the public version
examined here, each of the \componentcount{} registered components connects a
claim to a fixed input, a command, saved output, a pass rule, and a stated
limit. I call this arrangement a claim--evidence--limit contract and follow
one component from input to conclusion. The examination exposes five gaps:
a public run does not by itself establish private origin, correctness, support for the claim
as worded, behaviour beyond selected cases, or the absence of risks outside
a declared scan. I chose the selection, inputs, expected answers,
rules, and boundaries. The repository therefore lets a reader test claims
about its published files and outputs. It cannot establish the private
system's history, whether the selection represents the whole, or whether my
criteria are right. I report that I developed the private
...
This paper contributes a worked boundary analysis of one author-curated
software collection. It defines the component contract and follows one
ordinary component from input to limit. Five distinctions identify conclusions
that a passing run may appear to support but does not. Two tables classify all
\componentcount{} components by group and publication route. The proposed
evaluations identify the choices an evaluator would need to make independently
of me.
...
\emph{unit of analysis} (one item
examined). No subset of the components is used to estimate performance or
represent the private system.
...
can do to each part without asking anyone. Amber, here on the claim, the
limit, and the links asserting them, marks words and choices the project
itself supplies; the same two colours keep these meanings in every figure.
...
of assurance, testing, or reproducibility. The analysis here is narrower. It
applies these distinctions to one author-curated software collection and reads
...
The calculator check supplies an independent expected number for one row. It
does not establish the component's private origin, show that the validator
covers the full prose claim, predict untested inputs or unselected components,
or rule out publication risks beyond the project's declared scans. Even its
evidence for correctness is limited to that one calculation. The next section
treats these five gaps separately.

\section{Five distinctions}
\label{sec:distinctions}

Figure~\ref{fig:gaps} sets the five gaps side by side. Each begins with
something a reader can observe, then names the further evidence needed for a
broader conclusion. Individual component limits are instances of these gaps.
...
\node[flabel, fill=white, align=center, inner sep=2.5pt, text=authoramber] at (7.15,2.44) {selection and\\re-expression (mine)};
% public box
...
% a whisper of stranger-blue on the routes: each is evidence that lives
% outside the author's hands
\node[part, text width=2.15cm, minimum height=1.52cm, fill=strangerblue!8] (s1) at (1.2,0) {Independent repetition};
\node[part, text width=2.15cm, minimum height=1.52cm, fill=strangerblue!8] (s2) at (4.3,0) {Evaluator-controlled selection};
\node[part, text width=2.15cm, minimum height=1.52cm, fill=strangerblue!8] (s3) at (7.4,0) {Fresh inputs, independent expectations};
\node[part, text width=2.15cm, minimum height=1.52cm, fill=strangerblue!8] (s4) at (10.5,0) {Outside tasks, ordinary alternatives};
\node[part, text width=2.15cm, minimum height=1.52cm, fill=strangerblue!8] (s5) at (13.6,0) {Confidential provenance review};
