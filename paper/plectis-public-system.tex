...
\code{setuptools} to build it. No check proves the absence of network or model
calls, so inspect the source.

\paragraph{The worked example.} The Section~\ref{sec:run} command overwrites
\component{/tmp/plectis-audio-rms} on a rerun; a fresh run reproduced every
displayed pass and value. One stale field reports a copied private module, but
the saved import record reports none because public code replaced it. The
discrepancy remains.

\paragraph{A second Python version.} On 18~July~2026, a clean macOS clone using
Python 3.12.7 also matched the tour and example. Its offline install used the
already-present \code{setuptools} 80.9.0 (minimum 77.0.3) and no other runtime
library. Only versions 3.12.7 and 3.13.7 were tested.
...
(assumptions added without proof); compiled calculations whose acceptance
relies on more than Lean's small proof-checking core (the kernel);
\code{partial} definitions, which can run but cannot be unfolded in proofs;
\code{unsafe} definitions, which cannot be used in theorems; and removed
computation limits. This search cannot show that each formal
...
failed. They concerned an \code{AGENTS.md} line-break expectation, outdated
line or byte counts in copied-source manifests, and root-layout rules that did
not yet allow the \code{paper} directory or root PDF. A later change repairs
the last failure without altering this pinned run.
Exact test names, environment, and counts are in
\component{paper/public-test-receipt.json}. \code{make ci} would add smoke and
package-install checks, but was not green because \code{make test} failed.
These checks concern only the public checkout; this is my run,
