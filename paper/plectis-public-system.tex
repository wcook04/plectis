...
Answerability overlaps with reproducibility and transparency but is not a
synonym for either. A run can repeat perfectly with no stated claim attached,
...
ACM's artifact-review policy makes a related distinction between an artifact
audited as functional and results obtained by another team
\cite{acmartifact}. Plectis has received neither form of independent
review. This paper claims no new theory of assurance, testing, or
reproducibility. Its contribution is a worked boundary analysis of one
...
Disclosure alone cannot remove this concentration of control. Standards for
independent verification and validation distinguish independence from the
developers from independence over what and how to assess \cite{nasa8739}.
Plectis has not undergone such an evaluation. The narrower principle used in
Section~\ref{sec:stronger} is that evidence for a particular claim becomes
stronger as the choices bearing on that claim stop being mine.
...
was chosen. Adding more author-controlled components enlarges the inventory;
it does not strengthen the evidence. Stronger evidence requires consequential
choices to leave the author's hands.
...
larger tracked result contains one stale metadata field: it reports one
private source module copied into the public component. The pinned manifest,
the repository's list of source modules for this component, records zero
copied modules because the private module was omitted and replaced by public
code. That discrepancy does not change the displayed calculation, but it
remains part of the record.

\paragraph{A second interpreter check.} On 18~July~2026, a clean clone at the
same commit was tested on macOS with Python 3.12.7. A literal
...
setuptools 80.9.0 and telling \code{pip} to use that local build tool allowed
a network-free build;
after installation, the tour and worked example both passed, and the
displayed values matched exactly. Package records showed that Plectis had
installed no additional third-party library needed at runtime. This is
evidence for 3.12.7 and 3.13.7, not proof of every interpreter covered by the
package's declared Python 3.11-or-later range.
...
than smoothed over, which is what Section~\ref{sec:conclusion} asks a reader
