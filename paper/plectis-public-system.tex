...
\paragraph{A first review.} Use the appendix's no-install block to copy the
repository, select the version analysed here, and run its tour. Choose a claim
you doubt. Read its description and limit, run the listed command, compare the
new output with the stored receipt, and read the pass rule. Ask whether the
rule bears on the prose claim; change an input where practical; and, where a
refusal boundary does much of the work, ask when that boundary entered the
history. Report any discrepancy through
