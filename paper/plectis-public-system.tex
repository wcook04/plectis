...
folder) assembled from parts of a larger private system that cannot be
published. It asks a narrow question: what may a stranger reasonably conclude
from runnable fragments selected by the person whose work is being examined?
At the public version studied here, each of the \componentcount{} registered
components connects a claim to a fixed input, a command, saved output, a pass
rule, and a stated limit. I call this a claim--evidence--limit contract. One
component, for example, calculates an audio level from five small sample
arrays. A reader can repeat the calculation and challenge the expected
numbers; that run cannot show that the public code came from the private
application or will work on other inputs. The same boundary recurs across the
collection. A public run does not by itself establish private origin,
correctness, support for the claim as worded, behaviour beyond selected cases,
or the absence of risks outside a declared scan. I chose the published
fragments, inputs, answers, rules, and limits. The repository therefore lets a
reader test claims about its published files and outputs. It cannot establish
the hidden system's history, representativeness, or reliability. I report that
I developed the private system by directing AI coding tools; the argument does
not rely on that testimony.
\end{abstract}
...
\componentcount{} components by group and by the kind of public evidence they
supply. The proposed evaluations identify the choices an evaluator would need
to make independently of me.
...
result is justified independently of the implementation, as
...
Computer-checked mathematics & \mathcount & Check records and bounded examples from formal mathematics, including small Lean statements and one exact finite calculation. \\
...
local to those five gaps. Section~\ref{sec:stronger} lists independent evaluations
that address the dependencies this paper has identified.
...
and stated limit. No other repository's size or subject matter is evidence here.
This note reports no independent, randomly selected audit over fresh inputs,
...
this paper describes, none has happened.}
\label{fig:ladder}
\end{figure}
...
a version is frozen, and expected results worked out by a method that
...
The evaluations above may expose failures. Repairs should add facts rather
than erase them. The appendix therefore keeps three failures at the pinned
commit even though a later commit repairs one. That repair is a fact about the
later version; it does not subtract the first fact about this one. If outside
criticism shows a validator too weak or a claim too broad, the revision should
be legible as a revision, carrying its date and its cause, rather than the
project appearing always to have meant the narrower thing. A test rewritten after
...
no stronger than its rule or than the basis for the expected answer.
...
five reading orders, chose the README-first order
(\component{readme_onboarding_route}), wrote 21 files under
\code{.microcosm/}, and left the examined source files unchanged. Each
observation names its evidence, event, and limit, and the generated directory
can be rebuilt. The README gives a different file count from another day; both
counts describe dated runs, not stable project size.
...
The installed program needs no extra Python library; \code{pip} may fetch
\code{setuptools} to build it. No check enforces the claimed absence of network
or model calls, so inspect the source.

\paragraph{The worked example.} The command in Section~\ref{sec:run} writes
under \component{/tmp/plectis-audio-rms} and overwrites those files on a rerun.
A fresh run reproduced every displayed pass and value. One stale field reports
a copied private module, while the pinned manifest records none because public
code replaced it. The calculation is unchanged, but the discrepancy remains.
...
The tour and example matched without adding a runtime library. This tests 3.12.7
and 3.13.7, not the full Python 3.11-or-later range.

\paragraph{Project-wide checks.} At this commit, \code{make check} loads the
component registry in under a second.
The Lean trust checker searches the source text of \leanfilecount{} Lean files
for unfinished proof placeholders; custom axioms (assumptions added without
proof); native calculations relying on more than Lean's proof-checking kernel;
\code{partial} definitions, which Lean treats as opaque in its logic;
\code{unsafe} definitions, which Lean excludes from mathematical reasoning;
and removed computation limits. This search cannot show that each formal
statement says what its author intended \cite{leanvalidation}.
The registry and trust checks passed;
the \leanfilecount{} files and \mathcount{} mathematics components are different units.
At this commit, the bounded \code{make test} selection (not every test file)
ran 361 tests from 32 public test files: 356 passed, two were skipped, and three
failed. The failures were a line-break expectation in
\code{AGENTS.md}; outdated stored line or byte counts in copied-source manifests;
and root-layout rules that did not yet allow the \code{paper} directory or root
PDF. A later change repairs the last failure without altering this pinned run.
Exact test names, environment, and counts are in
\component{paper/public-test-receipt.json}. \code{make ci} would add smoke and
package-install checks, but its larger floor was not green because \code{make
test} failed. These checks concern only the public checkout; this is my run,
not an independent repetition.

\paragraph{The paper itself.}
\component{scripts/check_public_system_paper.py} checks counts, pinned facts, the
worked example, cautions, and citations, and fails on disagreement. Historical
facts come from the pinned commit; current prose is checked separately. Under
one maintainer, this is internal consistency, not external truth.
...
\bibitem{nasa8739}
NASA. \emph{Software Assurance and Software Safety Standard},
...
Lean Project. \emph{Lean Language Reference}: ``Validating a Lean Proof'' and
``Partial and Unsafe Definitions.'' Accessed 18 July 2026.
\href{https://lean-lang.org/doc/reference/latest/ValidatingProofs/}{Validation};
\href{https://lean-lang.org/doc/reference/latest/Definitions/Recursive-Definitions/#partial-and-unsafe-definitions}{definitions}.
