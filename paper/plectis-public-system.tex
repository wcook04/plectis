...
copy the fixture's \code{input} folder to \code{/tmp}. In
\component{batch8_audio_level_rms_port_probe_manifest.json}, change the first
case's second sample from \code{0.05} to \code{0.06}, but leave
\code{expected\_level} unchanged. Rerun with \code{--input} pointing to the copy.
The validator should return \code{blocked}; if it does not, report the finding
through \code{CONTRIBUTING.md}. Treat a pass as evidence only for the published rule.
