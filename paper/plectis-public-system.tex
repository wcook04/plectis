...
from runnable fragments chosen by the person whose work is being examined?
In the version studied here, each of the \componentcount{} registered
...
application or will work on other inputs. Across the collection, a public run
does not by itself establish private origin, correctness, support for the claim
as worded, behaviour beyond selected cases, or the absence of risks outside a
declared scan. I chose the published
...
the hidden system's history, whether the published selection resembles the
private whole, or whether the private system works reliably. I report that I
developed the private system by directing AI coding tools; the argument does
...
repository at the named commit is the single case, and each component is a
\emph{unit of analysis} (one item examined inside that case). No subset of the
...
a private mechanism is easiest to publish when it can be bounded, gives the
same output from the same input, and runs in isolation. A collection like this
...
its outcome is known is a different test and may be better. The known case can
show whether the repaired code later breaks in the same way; fresh confirmation
requires new cases whose answers were not used to design the repair.
Disappointment is in scope: an
...
but does not choose a component claim. Ask the repository to find the audio
example with
...
macOS 15.6.1 on Apple silicon (arm64) with Python 3.13.7. The commit's full
identifier is
...
private module; the saved import record reports none because public code replaced
it. The calculation is unchanged, but the discrepancy remains.

\paragraph{A second Python version.} On 18~July~2026, a clean clone on
macOS used Python 3.12.7. A network-disabled install could not fetch
\code{setuptools>=77.0.3}; setuptools 80.9.0 was already installed and allowed
a local build.
