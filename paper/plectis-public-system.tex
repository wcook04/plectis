...
and unfinished tools. Plectis asks what a stranger may reasonably conclude
from public tests when the author chose both what to publish and what counts as
a pass. In the version studied here, each of the \componentcount{} registered
components (separately testable parts) connects a claim to a fixed input,
command, saved output, pass rule, and stated limit. I call this a
claim--evidence--limit contract. One component calculates an audio level from
five small sample arrays; a reader can repeat it and challenge the expected
numbers. A matching run still does not establish where the code came from or
whether its answers are correct. It also cannot show whether the pass rule
tests the claim as written, how the code behaves on untested inputs, or whether
risks outside the declared scan are absent. I chose the published fragments,
inputs, answers, rules, and limits. The repository therefore lets a reader test
claims about its published files and outputs. It cannot establish the hidden
system's history, whether the published selection resembles the private whole,
or whether the private system works reliably. I also report that I developed
the private system by directing AI coding tools.
...
used to detect change \cite{nistfips1804}. Two different files can in
principle have the same fingerprint. NIST estimates SHA-256's collision
resistance at 128 bits; it defines collision resistance as making such a pair
computationally infeasible to find \cite{nisthashfunctions}. Collisions remain
mathematically possible. A match therefore supports a
...
command, receipt, limit, and \code{no-write variant}. That variant leaves stored
reference receipts untouched and puts fresh files under
\component{.microcosm/first\_action\_runs/}. Compare the receipts, read the
validator's rule, and change an input where practical. Ask whether the rule
...
17~July~2026: macOS 15.6.1 on Apple silicon (arm64), Python 3.13.7, and the
commit whose full identifier is

\begin{center}
\small\code{\snapshotcommit}
\end{center}

and is abbreviated \code{\snapshotshort} elsewhere. Later commits or other
machines may differ; the body does not depend on them.

\paragraph{Names.} Repository: \repo. Section~\ref{sec:contract}
explains why its code package is named \code{microcosm\_core}. The installed
command is \code{plectis}; the older alias \code{microcosm} also works.
...
\code{.microcosm/}, and left the examined source files unchanged. The run names
its evidence, event, and limit; the generated directory can be rebuilt. The
README's different file count records another dated run, not a stable project
size.
...
the saved import record reports none because public code replaced it. That
discrepancy remains.

\paragraph{A second Python version.} On 18~July~2026, a clean macOS clone using
Python 3.12.7 also matched the tour and example. Its offline install used
\code{setuptools} 80.9.0 already on the machine (minimum 77.0.3), with no other
runtime library. Only versions 3.12.7 and 3.13.7 were tested.
...
failed. The failures concerned an \code{AGENTS.md} line-break expectation,
outdated line or byte counts in copied-source manifests, and root-layout rules
that did not yet allow the \code{paper} directory or root PDF. A later change
repairs the last failure without altering this pinned run. Exact test names,
environment, and counts are in \component{paper/public-test-receipt.json}.
\code{make ci} would add smoke and package-install checks, but was not green
because \code{make test} failed.
These checks cover only the public checkout; this is my run, not an independent
repetition.
...
pinned commit and current prose is checked separately. With one maintainer,
this is internal consistency, not external truth.
