...
The runtime uses no third-party library; \code{pip} may fetch \code{setuptools}
only to build. Inspect the source rather than trusting the claim that components
avoid network or model calls. Before installation, the bootstrap script checks
the shipped inputs and limits.
...
Every displayed case passed; the fresh receipt and values matched the tracked
copies. One stale field in the larger result reports one copied private module,
while the pinned manifest records zero because public code replaced it. The
calculation is unchanged, but the discrepancy remains in the record.
...
The tour and example matched and added no runtime library. This covers 3.12.7
...
The Lean trust checker scans \leanfilecount{} Lean source files for unfinished proof
placeholders, custom axioms (assumptions added without proof), native
calculations that rely on more than Lean's proof-checking kernel, declarations
marked \code{unsafe} or \code{partial} that relax ordinary safeguards, and
instructions that remove computation limits. These are source-pattern checks.
They do not show that each formal statement says what its author intended
\cite{leanvalidation}. The registry and trust checks passed. The \leanfilecount{}
files and \mathcount{} mathematics components count different things.
\code{make ci} installs, tests, and runs two examples; continuous integration
runs the same checks. All concern the public checkout, not the private system.

The full public suite was not green: three tests failed, one in the front-door
profile and two in older documentation and record checks. A later commit repairs
the first; this record keeps all three, as Section~\ref{sec:conclusion} asks
readers to keep theirs.

\paragraph{The paper itself.}
\component{scripts/check_public_system_paper.py} recomputes counts, checks the
named commit and worked example, guards the main cautions, and fails on
disagreement. It reads the pinned commit for historical facts while checking
current prose and citations separately. Agreement under one maintainer is
...
Lean Project. ``Validating a Lean Proof.'' \emph{The Lean Language Reference}.
Accessed 18 July 2026.
\href{https://lean-lang.org/doc/reference/latest/ValidatingProofs/}{Lean proof validation}.
