...
searched for are the risks there are. The contract only makes each missing
premise explicit. Once named, a gap can be disputed, assigned a cost to
...
repository and select the version analysed here. The tour describes the checkout
but does not choose a component claim. Route the audio example with
\par\noindent{\scriptsize\Verb|PYTHONPATH=src python3 -m plectis comprehend --first-action "audio level calculation" --format text|}\par
For another claim, replace the quoted words. The response names a component,
command, receipt, limit, and \code{no-write variant}: stored reference receipts
stay untouched, while fresh files go under
\component{.microcosm/first\_action\_runs/}. Run it, compare the receipts, read
the validator's rule, and change an input where practical. Ask whether the rule
supports the prose and any refusal boundary; report discrepancies through
\code{CONTRIBUTING.md}. A pass licenses only the published rule.
Section~\ref{sec:stronger} lists broader evidence, most of it absent.
...
The program needs no extra Python library; \code{pip} may fetch
\code{setuptools} to build it. No check enforces the absence of network or model
calls, so inspect the source.

\paragraph{The worked example.} The command in Section~\ref{sec:run} writes
under \component{/tmp/plectis-audio-rms}, overwriting it on a rerun. A fresh run
reproduced every displayed pass and value. One stale field reports a copied
private module; the pinned manifest records none because public code replaced
it. The calculation is unchanged, but the discrepancy remains.

\paragraph{A second interpreter check.} On 18~July~2026, a clean clone on
macOS used Python 3.12.7. A network-disabled install could not fetch
\code{setuptools>=77.0.3}; the host's setuptools 80.9.0 allowed a local build.
The tour and example matched without a new runtime library. This tests 3.12.7
and 3.13.7, not every supported Python version.
...
\component{scripts/check_public_system_paper.py} checks counts, pinned facts,
the example, cautions, and citations, failing on disagreement. Historical facts
come from the pinned commit; current prose is checked separately. Under one
maintainer, this is internal consistency, not external truth.
