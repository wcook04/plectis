...
version analysed here. The tour describes the checkout
...
Change the quoted words for another claim. The response gives a main command
and a \code{no-write variant}. The latter starts with the installed command
\code{plectis}; because this route installs nothing, replace it with
\code{PYTHONPATH=src python3 -m} \code{microcosm\_core} and run it. The main
command overwrites stored receipts; the translated line writes fresh JSON under
\component{.microcosm/first\_action\_runs/}. In the directory named after
\code{--out}, open the file ending \component{\_result.json}. Inside
\code{exercise}, compare \code{expected\_level} and \code{observed\_level} in
each \code{reference\_cases} item. The top-level \code{anti\_claim} says what a
pass does not establish; \code{claim\_ceiling} inside \code{exercise} gives the
project's strongest conclusion. The project wrote both cautions; no independent
reviewer checked their limits. Then change an input where practical and compare
the validator's rule with the prose. Treat a pass as evidence only for the
published rule; report disputes through \code{CONTRIBUTING.md}.
