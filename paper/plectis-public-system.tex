...
published. It is a case study in a general question: what may a stranger
reasonably conclude from a curated set of small, runnable fragments of work
they cannot see? Each of the \componentcount{} public components joins a
claim to a fixed input, a command, saved output, a pass rule, and a stated
limit; the count is an inventory, not a score. This paper contributes a
concrete contract connecting each claim to its evidence and stated limit,
then uses that contract to audit one component from input to conclusion. The
audit exposes five gaps: a public run does not by itself establish private
origin, correctness, support for the claim as
worded, behaviour beyond selected cases, or the absence of risks outside a
declared scan. It also identifies which choices remain with the author and
what evidence would move each choice elsewhere. The public repository lets a
reader test claims about its published files and outputs. It cannot establish
the private system's history, whether the selection represents the whole, or
whether author-supplied success criteria are right. I report that I developed
the private system by directing AI coding tools, but the argument does not
depend on that account. One successful run supports only the conclusion
licensed by the published rule, which may still fall short of the prose
claim.
\end{abstract}
...
review. This paper adds no theory to those fields. Its narrower contribution
is the claim--evidence--limit contract and a worked analysis of the choices
that constrain inference from one author-curated software collection.
...
A first review can be small and still useful. Clone the repository; to compare
