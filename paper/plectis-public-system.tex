...
SHA-256 fingerprint: a fixed-length value calculated from its contents and
used to check for change. Two different files can in principle produce the
same value. SHA-256 is designed to make finding such a pair infeasible in
practice, not impossible \cite{nistfips1804}. A match therefore supports a
