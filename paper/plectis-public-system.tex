...
is a saved record of a prior run that the checker can read. A \emph{validator}
...
\node[part] (rec) at (9.55,-2.0) {Stored receipt\\(prior run)};
% flows
...
\paragraph{A second Python version.} On 18~July~2026, a clean macOS clone used
Python 3.12.7. Offline, the install used the already-present
\code{setuptools} 80.9.0 (the build requires version 77.0.3 or later). The tour
and example matched without another runtime library. This tests 3.12.7 and
3.13.7, not every supported version.
...
The registry and trust checks passed; the \leanfilecount{} files are not the
\mathcount{} mathematics components.
...
the example, cautions, or citations disagree. Historical facts come from the
pinned commit; current prose is checked separately. With one maintainer, this
is internal consistency, not external truth.
