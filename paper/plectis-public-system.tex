...
NASA defines independence for independent verification and validation (IV\&V),
a formal software check, through three separations: technical (the evaluator
did not develop the system); managerial (a separate organisation chooses scope,
methods, and schedule); and financial (an independent group controls the budget
without adverse financial pressure) \cite{nasa8739}.
These are criteria for NASA IV\&V, not a universal test. Plectis has undergone
no IV\&V and no other outside evaluation. I use this only for the paper's five
gaps: evidence about a choice depends less on me once the choice is no longer
mine. Section~\ref{sec:stronger} lists evaluations that address those
dependencies.
...
The clean-clone tour at this commit read 5{,}018 project files, found five
reading orders, chose the README-first order (internally
\component{readme_onboarding_route}), wrote 21 generated files under
\code{.microcosm/}, and left the examined source files unchanged. The generated
directory can be rebuilt. The README's different file count is another dated
run, not a stable project size.
...
machine's \code{setuptools} 80.9.0 (minimum 77.0.3) and no other runtime library.
Only versions 3.12.7 and 3.13.7 were tested.

\paragraph{Project-wide checks.} The Lean trust checker searches the source
text of \leanfilecount{} Lean files
...
failed: an \code{AGENTS.md} line-break expectation, outdated line or byte counts
in copied-source manifests, and root-layout rules that did not yet allow the
\code{paper} directory or root PDF. The last has since been repaired without
altering this pinned run. Exact test names,
