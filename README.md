...
the same command, and the same validator — matching the committed
demonstration — with no private-path leakage and no tracked source
...
digest-bound artifact subjects, the expectation policy (the committed
demonstration's owner, command, and validator that every context must match),
the failure-interpretation table, and what verification cannot prove. It is
generated by `scripts/build_release_review.py` from the same sources the proof
enforces and drift-gated in the public test floor; `make release-review` is
the one cold-review command — it regenerates the proof packet fresh, verifies
it with the strict no-rerun verifier (`make release-candidate-proof-verify`),
and prints its card.
