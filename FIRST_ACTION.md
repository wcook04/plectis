# Find a component command

<a id="first-correct-action"></a>
...
Plectis lists 88 components in [the component map](ORGANS.md). The `comprehend --first-action` command matches words in your question to the supplied component names and task descriptions, then prints a suggested command and the files relevant to it.

The lookup prints its answer in the terminal. It does not execute the suggested command or its validator, and it does not establish that the suggested command will answer your question. Read the selected component's input, source code and stated limits before deciding whether to run it.

Run the query from the clone root with Python 3.11 or newer; no package installation or model account is needed. Replace `<your goal>` with your question, or use the finance question shown below. [Quickstart](QUICKSTART.md) gives the clone command and shell setup.
...
The answer includes the selected component ID, suggested command, validator command, saved-result paths and limits. In the JSON, these fields together are called a *contract*. The `clean_run` variant, when present, writes fresh output under the ignored `.microcosm/` directory when you separately run it. Other commands may overwrite committed result files; the examples below name those output paths.

`scripts/build_first_action_demo.py` generates the contract blocks and table rows below by looking up the listed questions in the committed component records. Its `--check` mode compares this page and its JSON record with freshly generated versions; it does not execute the suggested component commands.
