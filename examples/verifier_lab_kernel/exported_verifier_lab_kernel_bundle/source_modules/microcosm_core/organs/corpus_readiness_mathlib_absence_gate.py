"""
Implements organs corpus readiness mathlib absence gate for the public Plectis package.

Callers enter through `runtime_lean_import_probe`, `validate_runtime_source_artifacts`,
`validate_corpus_readiness`, `validate_consumer_gate_cases`,
`validate_source_module_imports`, `write_receipts`, and 4 more; constants such as
`ORGAN_ID`, `FIXTURE_ID`, `VALIDATOR_ID`, `RESULT_NAME`, and 36 more pin local fixture
names; dependencies include `argparse`, `hashlib`, `json`, `shutil`, and 7 more. It builds
...
SOURCE_DIGESTS = {
    ...
    SOURCE_REFS[1]: "sha256:79c2420a86752405bed74464c8848d02db341149ec7e7bbbae905a503f732255",
    SOURCE_REFS[2]: "sha256:8c020f6884cda37338cb5216ded61722a9993fcd6d69aee1db655885738abbd1",
    SOURCE_REFS[3]: "sha256:87515caa99df6a18b8e3a3458ddc6a3e22784a79c32931376f1c6484ad01f267",
}
...
def _public_root_for_path(path: str | Path) -> Path:
    """
    Return public root for path for the organs corpus readiness mathlib absence gate flow.
...
def _display(path: Path, *, public_root: Path) -> str:
    """
    Return display for `microcosm_core.organs.corpus_readiness_mathlib_absence_gate`.
...
def _rows(payload: object, key: str) -> list[dict[str, Any]]:
    ...
    `microcosm_core.organs.corpus_readiness_mathlib_absence_gate._rows` from `payload[key]`.
...
def _load_payloads(input_dir: Path, *, include_negative: bool) -> dict[str, Any]:
    """
    Load load payloads for `microcosm_core.organs.corpus_readiness_mathlib_absence_gate`.
...
def _source_module_manifest_path(input_dir: Path) -> Path:
    """
    Return source module manifest path for the organs corpus readiness mathlib absence gate
    flow.
...
def _read_source_module_manifest(input_dir: Path) -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.corpus_readiness_mathlib_absence_gate._read_source_module_manifest`
    into the payload shape expected by organs corpus readiness mathlib absence gate.
...
def _source_module_source_path(public_root: Path, row: dict[str, Any]) -> Path | None:
    """
    Return source module source path for the organs corpus readiness mathlib absence gate
    flow.
...
def _source_module_rows_by_ref(input_dir: Path) -> dict[str, dict[str, Any]]:
    """
    Compute source module rows by ref from `input_dir`.

    Inputs are `input_dir`; notable helpers are `_source_module_rows`,
    `_read_source_module_manifest`, and `get`.
    """
...
def _input_paths(input_dir: Path, *, include_negative: bool) -> list[Path]:
    """
    Return input paths for `microcosm_core.organs.corpus_readiness_mathlib_absence_gate`.
...
def _sha256(path: Path) -> str:
    ...
    `microcosm_core.organs.corpus_readiness_mathlib_absence_gate._sha256`.
...
def _normalize_sha256(value: object) -> str:
    """
    Return the stable digest computed by
    `microcosm_core.organs.corpus_readiness_mathlib_absence_gate._normalize_sha256`.
...
def _line_count(path: Path) -> int:
    """
    Return line count for the organs corpus readiness mathlib absence gate flow.
...
def _finding(
    ...
    *,
    ...
    """
    Serialize `microcosm_core.organs.corpus_readiness_mathlib_absence_gate._finding` into
    the payload shape expected by organs corpus readiness mathlib absence gate.
...
def _record(
    ...
    *,
    ...
    """
    Record record for the organs corpus readiness mathlib absence gate flow.
...
def _merge_observed(*results: dict[str, Any]) -> dict[str, list[str]]:
    """
    Return merge observed for `microcosm_core.organs.corpus_readiness_mathlib_absence_gate`.
...
def _merge_findings(*results: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    `microcosm_core.organs.corpus_readiness_mathlib_absence_gate._merge_findings`.
...
def _unexpected_findings(
    ...
    """
    Create the finding rows emitted by
    `microcosm_core.organs.corpus_readiness_mathlib_absence_gate._unexpected_findings`.
...
def _source_ref_is_private(ref: str) -> bool:
    """
    Return whether source ref is private holds for the organs corpus readiness mathlib
    absence gate flow.

    The result is derived from `ref` with `lower` and `startswith`; failing evidence is
...
def _runtime_source_path(input_dir: Path, *, public_root: Path, source_ref: str) -> Path:
    """
    Return runtime source path for the organs corpus readiness mathlib absence gate flow.

    Inputs are `input_dir`, `public_root`, and `source_ref`; notable helpers are
    `startswith`, `is_file`, and `_strip_microcosm_prefix`.
    """
    if source_ref.startswith("microcosm-substrate/"):
...
def _runtime_corpus_rows(payload: object) -> dict[str, dict[str, Any]]:
    """
    Return runtime corpus rows for the organs corpus readiness mathlib absence gate flow.
...
def _runtime_mathlib_probe_status(payload: object) -> str:
    """
    Derive runtime mathlib probe status without touching module import state.
...
def _normalize_private_absolute_path_rewrite(text: str) -> str:
    """
    Compute normalize private absolute path rewrite from `text`.

    Inputs are `text`; notable helpers are `sub`.
    """
...
def _true_fields(payload: dict[str, Any], fields: tuple[str, ...]) -> list[str]:
    """
    Derive true fields without touching module import state.

    Inputs are `payload` and `fields`; notable helpers are `get`.
    """
...
def _command_result_card(
    ...
    *,
    ...
    """
    Serialize
    `microcosm_core.organs.corpus_readiness_mathlib_absence_gate._command_result_card` into
    the payload shape expected by organs corpus readiness mathlib absence gate.
...
def _skipped_command_result_card(
    ...
    *,
    ...
    """
    Serialize
    `microcosm_core.organs.corpus_readiness_mathlib_absence_gate._skipped_command_result_card`
    into the payload shape expected by organs corpus readiness mathlib absence gate.
...
def _probe_input_file_card(path: Path) -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.corpus_readiness_mathlib_absence_gate._probe_input_file_card`
    into the payload shape expected by organs corpus readiness mathlib absence gate.
...
def _runtime_probe_input_sources(
    ...
    """
    Produce the runtime probe input sources value used by
    `microcosm_core.organs.corpus_readiness_mathlib_absence_gate`.

    Inputs are `probe_input_dir`; notable helpers are `Path`, `read_text`,
    `_probe_input_file_card`, and `is_file`.
    """
...
def _runtime_probe_input_dir(input_dir: Path) -> Path | None:
    """
    Produce the runtime probe input dir value used by
    `microcosm_core.organs.corpus_readiness_mathlib_absence_gate`.

    Inputs are `input_dir`; notable helpers are `exists`.
    """
...
def runtime_lean_import_probe(
    ...
    """
    Serialize
    `microcosm_core.organs.corpus_readiness_mathlib_absence_gate.runtime_lean_import_probe`
    into the payload shape expected by organs corpus readiness mathlib absence gate.
...
def validate_runtime_source_artifacts(
    ...
    *,
    ...
    """
    Serialize
    `microcosm_core.organs.corpus_readiness_mathlib_absence_gate.validate_runtime_source_artifacts`
    into the payload shape expected by organs corpus readiness mathlib absence gate.
...
def validate_corpus_readiness(
    ...
    *,
    ...
    """
    Serialize
    `microcosm_core.organs.corpus_readiness_mathlib_absence_gate.validate_corpus_readiness`
    into the payload shape expected by organs corpus readiness mathlib absence gate.
...
def validate_consumer_gate_cases(
    ...
    *,
    ...
    """
    Serialize
    `microcosm_core.organs.corpus_readiness_mathlib_absence_gate.validate_consumer_gate_cases`
    into the payload shape expected by organs corpus readiness mathlib absence gate.
...
def validate_source_module_imports(
    ...
    *,
    ...
    """
    Serialize
    `microcosm_core.organs.corpus_readiness_mathlib_absence_gate.validate_source_module_imports`
    into the payload shape expected by organs corpus readiness mathlib absence gate.
...
def _build_board(
    *,
    ...
    """
    Serialize `microcosm_core.organs.corpus_readiness_mathlib_absence_gate._build_board`
    into the payload shape expected by organs corpus readiness mathlib absence gate.
...
def write_receipts(
    ...
    *,
    ...
    """
    Write write receipts for the organs corpus readiness mathlib absence gate flow.
...
def run(
    ...
    *,
    ...
    """
    Return run for `microcosm_core.organs.corpus_readiness_mathlib_absence_gate`.
...
def run_projection_bundle(
    ...
    """
    Return run projection bundle for the organs corpus readiness mathlib absence gate flow.
...
def _authority_ceiling_card(result: dict[str, Any]) -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.corpus_readiness_mathlib_absence_gate._authority_ceiling_card`
    into the payload shape expected by organs corpus readiness mathlib absence gate.
...
def _secret_scan_card(result: dict[str, Any]) -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.corpus_readiness_mathlib_absence_gate._secret_scan_card` into the
    payload shape expected by organs corpus readiness mathlib absence gate.
...
def _source_module_card(result: dict[str, Any]) -> dict[str, Any]:
    """
    Serialize
    `microcosm_core.organs.corpus_readiness_mathlib_absence_gate._source_module_card` into
    the payload shape expected by organs corpus readiness mathlib absence gate.
...
def result_card(result: dict[str, Any]) -> dict[str, Any]:
    """
    Serialize `microcosm_core.organs.corpus_readiness_mathlib_absence_gate.result_card` into
    the payload shape expected by organs corpus readiness mathlib absence gate.
...
def _parser() -> argparse.ArgumentParser:
    ...
    `microcosm_core.organs.corpus_readiness_mathlib_absence_gate._parser`.
...
def main(argv: list[str] | None = None) -> int:
    """
    Run the `microcosm_core.organs.corpus_readiness_mathlib_absence_gate` command-line entry
    point.
