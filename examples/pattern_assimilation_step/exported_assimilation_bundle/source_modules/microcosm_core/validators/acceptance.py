...
PUBLIC_RECEIPT_REF_SUFFIXES = frozenset({".json", ".jsonl"})
...
def _public_ref_path(ref: str, *, public_root: Path) -> Path | None:
    ...
    if ref_path.parts[:3] == ("state", "microcosm_portfolio", "reconstruction"):
        return public_root.parent / ref_path
    return public_root / ref_path


def _validate_public_safe_receipt_ref(
    ref: str,
    *,
    public_root: Path,
    subject_id: str,
    ...
    path = _public_ref_path(ref, public_root=public_root)
    allowed_state_root = repo_root / "state" / "microcosm_portfolio" / "reconstruction"
    if path is None:
        card.update({"status": "blocked", "defect": "unsafe_or_empty_ref"})
    else:
        resolved = path.resolve(strict=False)
        card["resolved_ref"] = _display_path(
            resolved,
            public_root=public_root,
            repo_root=repo_root,
        )
        if not (
            _is_relative_to(resolved, public_root)
            or _is_relative_to(resolved, allowed_state_root)
        ):
            card.update({"status": "blocked", "defect": "ref_outside_public_receipt_roots"})
        elif not path.is_file():
            card.update({"status": "blocked", "defect": "receipt_file_missing"})
        elif path.suffix not in PUBLIC_RECEIPT_REF_SUFFIXES:
            card.update({"status": "blocked", "defect": "unsupported_receipt_suffix"})
        else:
            ...
            card["private_state_scan_status"] = scan_result.get("status")
            card["private_state_blocking_hit_count"] = int(
                scan_result.get("blocking_hit_count") or 0
            )
            card["sha256"] = f"sha256:{_sha256_file(path)}"
            if (
                scan_result.get("status") != PASS
                or card["private_state_blocking_hit_count"] > 0
            ):
                card.update({"status": "blocked", "defect": "private_state_scan_blocked"})
    if card["status"] != PASS:
        findings.append(
            _bundle_finding(
                error_code,
                "Receipt reference must resolve to an existing public-safe JSON receipt file.",
...
def validate_exported_organ_landings(payload: object, public_root: Path) -> dict[str, Any]:
    ...
    landing_receipt_ref_files: list[dict[str, Any]] = []
    ...
    for row in rows:
        ...
        if receipt_ref:
            ...
            receipt_card, receipt_findings = _validate_public_safe_receipt_ref(
                receipt_ref,
                public_root=public_root,
                subject_id=organ_id or receipt_ref,
                subject_kind="organ_landing_receipt_ref",
                error_code="ASSIMILATION_BUNDLE_LANDING_RECEIPT_REF_UNRESOLVED",
            )
            landing_receipt_ref_files.append(receipt_card)
    ...
    return {
        ...
        "landing_receipt_ref_files": landing_receipt_ref_files,
        "resolved_landing_receipt_ref_count": len(
            [card for card in landing_receipt_ref_files if card["status"] == PASS]
        ),
...
def validate_exported_closeout_receipt_links(
    ...
    nothing_by_organ = dict(nothing_result.get("nothing_to_refine_result_by_organ", {}))
    closeout_links: list[dict[str, Any]] = []
    for organ_id, closeout_result in sorted(landing_result.get("closeout_by_organ", {}).items()):
        if not closeout_result:
            continue
        if organ_id not in refinement_by_organ and organ_id not in nothing_by_organ:
            continue
        expected = nothing_by_organ if closeout_result == "nothing_to_refine" else refinement_by_organ
        linked_result = str(expected.get(organ_id) or "")
        status = PASS if linked_result == closeout_result else "blocked"
        closeout_links.append(
            {
                "organ_id": organ_id,
                "closeout_refinement_result": closeout_result,
                "linked_receipt_result": linked_result,
                "status": status,
                "body_redacted": True,
            }
        )
        if status != PASS:
            findings.append(
                _bundle_finding(
                    "ASSIMILATION_BUNDLE_CLOSEOUT_RECEIPT_REF_UNRESOLVED",
                    "Organ closeout result must resolve to a matching refinement or nothing-to-refine receipt row.",
                    subject_id=organ_id,
                    subject_kind="organ_closeout_receipt_link",
                )
            )
    return {
        "status": PASS if not findings else "blocked",
        "findings": findings,
        "closeout_receipt_links": closeout_links,
        "resolved_closeout_receipt_link_count": len(
            [card for card in closeout_links if card["status"] == PASS]
        ),
    }
...
def _has_private_seed_payload_violation(missing_case: dict[str, Any]) -> bool:
    ...
    if missing_case.get("forbidden_payload_class") != "seed_origin_payload":
        return False
    evidence = missing_case.get("redacted_private_payload_evidence")
    if not isinstance(evidence, dict):
        return False
    forbidden_keys = [
        str(item)
        for item in evidence.get("forbidden_keys_detected", [])
        if isinstance(item, str)
    ]
    private_payload_present = any(
        key in {"body", "raw_seed_body", "private_payload", "matched_excerpt"}
        for key in forbidden_keys
    )
    return (
        evidence.get("body_redacted") is True
        and evidence.get("body_in_receipt") is False
        and private_payload_present
        and missing_case.get("body_redacted") is True
    )
...
def _validate_rows(payloads: dict[str, Any], public_root: Path) -> dict[str, Any]:
    ...
    valid_refinement_by_organ: dict[str, list[dict[str, Any]]] = defaultdict(list)
    valid_nothing_by_organ: dict[str, list[dict[str, Any]]] = defaultdict(list)
    for row in refinement_rows:
        receipt_id = str(row.get("receipt_id") or "")
        organ_id = str(row.get("landed_organ_id") or row.get("organ_id") or "")
        if (
            receipt_id
            and organ_id
            and receipt_id not in duplicate_receipt_ids
            and row.get("refinement_result") in {"fixture_manifest_refined", "validator_contract_refined"}
            and row.get("owner_surface")
            and not row.get("claims_global_doctrine_authority")
        ):
            valid_refinement_by_organ[organ_id].append(row)
    for row in nothing_rows:
        ...
        if (
            receipt_id
            and organ_id
            and row.get("refinement_result") == "nothing_to_refine"
            and row.get("stewardship_checked")
            and row.get("next_best_lane_checked")
            and row.get("reentry_condition")
        ):
            valid_nothing_by_organ[organ_id].append(row)
    landing_receipt_ref_files: list[dict[str, Any]] = []
    for row in landings:
        organ_id = str(row.get("organ_id") or "organ")
        closeout_result = str(row.get("closeout_refinement_result") or "")
        receipt_ref = str(row.get("landing_receipt_ref") or "")
        assimilation_ref = str(row.get("assimilation_receipt_ref") or "")
        ...
        if receipt_ref:
            receipt_card, receipt_findings = _validate_public_safe_receipt_ref(
                ...
                subject_id=organ_id,
                subject_kind="organ_landing_receipt_ref",
                error_code="PATTERN_ASSIMILATION_LANDING_RECEIPT_REF_UNRESOLVED",
            )
            landing_receipt_ref_files.append(receipt_card)
            blocking_findings.extend(receipt_findings)
        if not closeout_result:
            _record(
                findings,
                observed,
            ...
            continue
        valid_rows = (
            valid_nothing_by_organ[organ_id]
            if closeout_result == "nothing_to_refine"
            else valid_refinement_by_organ[organ_id]
        )
        if valid_rows and not any(
            str(valid_row.get("receipt_id") or "") == assimilation_ref
            and str(valid_row.get("refinement_result") or "") == closeout_result
            for valid_row in valid_rows
        ):
            blocking_findings.append(
                _finding(
                    "PATTERN_ASSIMILATION_CLOSEOUT_RECEIPT_REF_UNRESOLVED",
                    "Organ closeout must resolve to a matching refinement or nothing-to-refine receipt row.",
                    case_id="dead_or_fabricated_closeout_ref",
                    subject_id=organ_id,
                    subject_kind="organ_closeout_receipt_link",
                )
            )
    ...
    if _has_private_seed_payload_violation(missing_case):
        _record(
            findings,
            observed,
    ...
    return {
        ...
        "blocking_error_codes": sorted(
            {str(finding.get("error_code") or "") for finding in blocking_findings}
        ),
...
def run_assimilation_bundle(
    ...
    landing_result = validate_exported_organ_landings(
        payloads["organ_landing_summaries"],
        public_root,
    )
    ...
    closeout_link_result = validate_exported_closeout_receipt_links(
        landing_result,
        refinement_result,
        nothing_result,
    )
    ...
    result.update(
        {
            ...
            "landing_receipt_ref_files": landing_result["landing_receipt_ref_files"],
            "resolved_landing_receipt_ref_count": landing_result[
                "resolved_landing_receipt_ref_count"
            ],
            "closeout_by_organ": landing_result["closeout_by_organ"],
            "closeout_receipt_links": closeout_link_result["closeout_receipt_links"],
            "resolved_closeout_receipt_link_count": closeout_link_result[
                "resolved_closeout_receipt_link_count"
            ],
...
def validate_pattern_assimilation(input_dir: str | Path, out: str | Path, command: str | None = None) -> dict[str, Any]:
    ...
    closeout = _validate_rows(payloads, public_root)
    ...
    result.update(
        {
            "status": (
                ...
                and not closeout["blocking_findings"]
                else "blocked"
            ),
            "validator_id": VALIDATOR_ID,
            ...
            "findings": sorted(
                [*closeout["findings"], *closeout["blocking_findings"]],
                key=lambda item: (
