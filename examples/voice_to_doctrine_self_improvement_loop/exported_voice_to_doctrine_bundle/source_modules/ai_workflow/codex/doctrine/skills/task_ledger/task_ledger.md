...
- Preserve the structured capture receipt when compacting output. Keep `subject_id`/`cap_id`, command status, and the `visibility_receipt` together; a `grep`/`head`/`tail` summary that drops the id or hides nonzero status is not sufficient evidence to cite the capture later.
