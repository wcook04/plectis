...
| Work Ledger or Task Ledger append-log attribution risks oversized output | `std_work_ledger.json::claim_card_first_contract`, `std_task_ledger.json::event_model`, `task_ledger.md::Same-authority append-log landing`, and this skill | Do not inspect central append ledgers or generated projection maps with broad `git diff` / full JSON dumps. Start with compact owner commands such as `session-status --seed-speed`, `session-claims --refresh --session-summary --cards-only`, `task_ledger_apply.py organizer-report`, `validate`, `rebuild --check`, or `git diff --numstat`. Exact identifier search has the same failure mode when the path set includes compact single-line generated/read-model JSON, large generated views, or runtime JSONL logs such as `state/work_ledger/runtime_status.json`, `state/task_ledger/ledger.json`, `state/task_ledger/views/*.json`, and `tools/meta/control/*observability*.jsonl`; use `task_ledger_apply.py search`, Work Ledger session/card commands, selected `jq` over known files, or explicit source/docs globs with `--max-columns` and log/generated-state exclusions instead of repo/state-wide `rg`. When exact row attribution is needed, bound it with `git diff --unified=0 -- <exact-file>`, `tail -n <small-N>`, or a selected `jq`/event-id query, and increase output only after the bounded evidence cannot decide ownership. |
