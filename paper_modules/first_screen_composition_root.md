...
1. One shared first command: `microcosm tour --card <project>`.
2. Three reader branch handles after that shared card: safety/evals engineer,
   hiring reviewer, and peer developer.
3. Evidence counts framed as accounting, not maturity or progress scores.
4. A runnable-to-structural join: the folder-local command is one visible
   exercise of a larger source-open substrate.
5. An omission receipt: the card names the deeper route map, receipts,
   standards, workingness, authority, and observatory drilldowns instead of
   copying them.
6. An authority ceiling that rejects release, hosted publication, provider
...
single shared first command, the three reader route ids, evidence-accounting
