...
- macro source evidence is cited as path-level lineage, not body import;
- private material is omitted with explicit omission receipts;
- public replacements are fixtures, standards, paper modules, exported bundles,
  and receipt refs;
...
policy, authority ceiling, omitted material, private-state scan count, and
...
ready but unlanded cell from a public replacement, self-hosted protocol, or
runtime bridge that is already consumed.
...
`public_replacement_landed` for `formal_math_readiness_extensions`,
